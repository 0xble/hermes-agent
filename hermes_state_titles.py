...
class SessionTitlesMixin:
    ...
    def _next_title_in_lineage_locked(self, conn, base_title: str, session_id: str) -> str:
        """Choose a bounded free ``#N`` lineage alias inside the active write transaction.

        Allocating under the same transaction as the write is what makes concurrent claims
        serialize: two callers cannot both observe the same alias as free.
        """
        ...
        while True:
            suffix = f" #{number}"
            candidate = f"{base[:self.MAX_TITLE_LENGTH - len(suffix)].rstrip()}{suffix}"
            if conn.execute("SELECT 1 FROM sessions WHERE title = ? AND id != ?",
                            (candidate, session_id)).fetchone() is None:
                return candidate
            number += 1

    def _set_session_title_result(self, session_id: str, title: str, *, source: str,
                                  conflict_policy: str = "error") -> tuple:
        ...
        ``conflict_policy="lineage"`` allocates the next free ``#N`` alias instead of raising, so a
        Telegram topic lane can keep the visible label while the stored title stays unique. Returns
        ``(written, stored_title)``."""
        ...
        def _do(conn):
            ...
            if title:
                ...
                if conflict:
                    ...
                    if self._is_compression_ancestor(conn, ancestor_id=conflict_id, descendant_id=session_id):
                        ...
                        selected_title = self._next_title_in_lineage_locked(conn, title, session_id)
                    else:
            ...
            return conn.execute(
                ...
                (selected_title, source if selected_title else None, session_id,
                 current["title"], current["title_source"]),
            ).rowcount, selected_title
    ...
    def _set_session_title(self, session_id: str, title: str, *, source: str) -> bool:
        """Whether the title write landed; see :meth:`_set_session_title_result`."""
        return self._set_session_title_result(session_id, title, source=source)[0]

    def set_session_title_in_lineage(self, session_id: str, title: str) -> Optional[str]:
        """Set a user title, allocating a unique ``#N`` alias on collision.

        Lookup and reservation share one write transaction, so concurrent callers cannot claim the
        same alias. Returns the stored title, or None when the session does not exist.
        """
        written, stored_title = self._set_session_title_result(
            session_id, title, source=self.TITLE_SOURCE_USER, conflict_policy="lineage")
    ...
    def list_recent_session_titles(self, *, exclude_session_id: Optional[str] = None,
                                   limit: int = 24) -> List[str]:
        """Recent distinct titles, for advisory generation-time avoidance.

        Transactional uniqueness stays authoritative; this bounded read only improves title quality
        before the write, and deliberately returns titles rather than any conversation content.
        """
        bounded_limit = max(1, min(int(limit), 100))
        sql = "SELECT title FROM sessions WHERE title IS NOT NULL AND TRIM(title) != ''"
        params: List[Any] = []
        ...
        for row in rows:
            ...
            key = re.sub(r"\s+", " ", unicodedata.normalize("NFKC", title).casefold()).rstrip(".!,;:")
