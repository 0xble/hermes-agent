...
def test_surface_goal_state_matches_cli(surface, command, monkeypatch):
    ...
    for name in ('cli', surface):
        ...
        if state:
            ...
            state = asdict(state)
            # evidence_since is a fork-only field of the same class as these: every
            # GoalManager.set() stamps it with time.time(), so two managers built in
            # sequence always differ. Upstream's parity test cannot know about it.
            for key in ('created_at', 'updated_at', 'waiting_since', 'evidence_since'):
                state.pop(key, None)
