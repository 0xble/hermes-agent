...
def test_add_priority_preserves_success_when_saved_identity_is_unavailable(monkeypatch, capsys):
    ...
    def persist_without_pool_identity(_creds, **_kwargs):
        write_credential_pool("nous", rows)
        return None

    monkeypatch.setattr(auth_commands.auth_mod, "persist_nous_credentials", persist_without_pool_identity)
    auth_commands.auth_add_command(SimpleNamespace(
        provider="nous", auth_type="oauth", priority=0, label="saved"))
    assert [entry["id"] for entry in read_credential_pool("nous")] == ["row0", "row1"]
    assert "hermes auth priority nous <target> 0" in capsys.readouterr().err
