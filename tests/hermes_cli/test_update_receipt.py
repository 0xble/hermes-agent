...
class TestPreUpdateBackupStep:
    ...
    def _record(monkeypatch, *, args, snapshot_id, updates_cfg) -> dict:
        """Run the backup boundary and read its persisted receipt payload."""
        monkeypatch.setattr(update_cmd_maint, "_load_updates_cfg", lambda: dict(updates_cfg))
        ur.begin_update_receipt()
        monkeypatch.setattr(update_cmd_maint, "_run_quick_snapshots", lambda: snapshot_id)
        update_cmd_maint._run_pre_update_backup(args)
