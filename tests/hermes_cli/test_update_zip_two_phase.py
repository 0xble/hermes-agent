...
def test_no_open_coded_venv_layout_remains_in_hermes_cli():
    ...
    # rglob descends into generated caches before filtering its matches. Other
    # test workers create and remove those cache directories concurrently.
    # Prune only __pycache__ before descent; source-file read and walk errors
    # still surface as test failures.
    for dirpath, dirnames, filenames in os.walk(pkg, onerror=raise_walk_error):
