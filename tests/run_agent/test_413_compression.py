...
class TestPreflightCompression:
    ...
    def test_compress_context_emits_deferred_terminal_status_for_lcm_noop(self, agent):
        """An LCM no-op preserves the conversation without reporting failure."""
        agent.compression_enabled = False
        events = []
        agent.status_callback = lambda event, message: events.append((event, message))
        messages = [{"role": "user", "content": "hello"}]
        ...
        def _noop(*_args, **_kwargs):
            agent.context_compressor._last_compression_status = "noop"
            return messages

        with patch.object(agent.context_compressor, "compress", side_effect=_noop):
            compressed, prompt = agent._compress_context(
                messages,
                "system prompt",
                force=True,
