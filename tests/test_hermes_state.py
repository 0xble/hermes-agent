...
class TestFTS5Search:
    ...
    def test_search_projection_skips_context_enrichment_queries(self, db, monkeypatch):
        ...
        @contextmanager
        def traced_read_ctx():
            # The upstream reader pool can return a different connection for
            # each transaction, so instrument the connection actually leased
            # to every search step rather than one speculative reader.
        ...
        try:
            ...
            monkeypatch.setattr(db, "_read_ctx", original_read_ctx)
