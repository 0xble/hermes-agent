...
def test_native_cleanup_handles_rg_exiting_before_process_group_lookup(monkeypatch):
    """A short-lived rg may exit after poll() but before getpgid().

    Native rg is spawned with start_new_session=True, making its PID the group
    ID even after its leader exits.  Cleanup must use that ID and not leak the
    lookup race to the search caller.
    """
    class ExitedProc:
        pid = 424242
    ...
    monkeypatch.setattr(local.os, "getpgid", gone_before_lookup)
    monkeypatch.setattr(local.os, "killpg", missing_group)
