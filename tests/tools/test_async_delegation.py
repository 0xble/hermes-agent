...
def _restore_session_cwd_sentinel():
    """``clear_session_vars`` leaves ``_SESSION_CWD`` at the explicit-"" ISOLATION value, not the
    pristine "never set" sentinel. Left that way, every later test's ``resolve_tool_cwd()`` answers
    "no cwd" and TERMINAL_CWD-anchored suites silently fall back to $HOME."""
...
def _write_presentation_receipt(owner=None):
    ...
    db.append_message("parent", "user", "result", display_metadata={
        "delegation_deliveries": [{"delegation_id": "deleg_present", "owner": owner}]})
    db.close()
...
def test_canonical_compression_presentation_excludes_explicit_fork(tmp_path, monkeypatch):
    ...
    db.append_message("tip", "user", "result", display_metadata={
        "delegation_deliveries": [{"delegation_id": "deleg_present", "owner": None}]})
    db.close()
    assert not ad.mark_completion_presented("deleg_present")
    with ad._transaction() as conn:
