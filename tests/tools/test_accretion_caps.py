...
class TestCompletionConsumedPrune:
    def test_prune_drops_completion_entry_with_expired_session(self):
        ...
        reg = ProcessRegistry()
        # A finished, unowned session is eligible for TTL eviction. Use the
        # real record so result-observation and persistence state stay coherent.
        reg._finished["stale-1"] = ProcessSession(
            id="stale-1", command="true",
            started_at=time.time() - (FINISHED_TTL_SECONDS + 100), exited=True,
        )
