...
def test_resume_reports_only_mismatched_field_names(monkeypatch, field, value, diagnostic):
    ...
    creds[field] = value
    monkeypatch.setattr("tools.custom_subagents.resolve_named_credentials", lambda *a, **kw: (creds, None))
...
def test_resume_authority_resolution_errors_do_not_expose_values(monkeypatch):
    ...
    monkeypatch.setattr("tools.custom_subagents.resolve_named_credentials", refuse)
    with pytest.raises(ValueError, match="current_named_route") as error:
        delegate_tool._resolve_resume_launch({"resume_session_id": "child"}, definitions, parent)
...
def test_normal_resume_dispatch_restores_logical_identity(tmp_path, monkeypatch, corrupt, stopped):
    ...
    monkeypatch.setattr(delegate_tool, "_resolve_delegation_credentials", lambda *_: {"provider": "fixture", "model": "m", "base_url": "https://fixture/v1", "api_key": "secret", "api_mode": "chat_completions"})
