...
@pytest.mark.parametrize("tool_reason,user_stopped", [
    ("explicit stop requested", True), ("delegation cancelled", False),
])
def test_real_spawn_path_child_starts_interrupted(tmp_path, monkeypatch, tool_reason, user_stopped):
    ...
    try:
        mid.hard_interrupt("stop", tool_reason=tool_reason)
        ...
        try:
            ...
            assert grandchild._delegation_interrupt_reason == tool_reason
            assert bool(getattr(grandchild, "_delegation_user_stopped", False)) is user_stopped
        finally:
            grandchild.close()
    finally:
        mid.close()
