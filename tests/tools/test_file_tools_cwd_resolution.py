...
def _clear_recorded_session_cwd():
    """``_session_cwd`` is a process-global map keyed by task id, and several cases here record
    ``"default"``. Leaving one behind makes the NEXT test's relative-path resolution answer from
    this file's workspace — it silently fails unrelated suites (test_resolve_path,
    test_gateway_cwd_contract, ...) that legitimately expect the TERMINAL_CWD anchor."""
