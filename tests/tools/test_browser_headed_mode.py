...
class TestEffectiveHeadedPersistence:
    ...
    def _clear_runtime_tracking(self):
        """Per-identity runtime maps are process-global while the conftest gives each test a fresh
        HERMES_HOME, so an entry from an earlier case survives under a DIFFERENT (still unskipped)
        cache key and answers the next case's question instead of its own."""
        import tools.browser_tool as bt
