...
@pytest.mark.parametrize("fork_context", [None, "Quoted parent reference for this child"])
def test_child_run_sends_multimodal_goal_turn(tmp_path, fork_context):
    ...
    if fork_context is not None:
        child._delegation_fork_history = [{"role": "user", "content": fork_context}]
    run = _ChildRun(child, _Parent(), 0, "Describe it", None, None)
    ...
    assert "Describe it" in content[0]["text"]
    if fork_context is not None:
        assert content[0]["text"].startswith(fork_context + "\n\n")
        assert child._delegation_fork_history == [{"role": "user", "content": fork_context}]
