...
class Board:
    ...
    def env(self) -> dict[str, str]:
        # Service labels are host-wide on macOS. Keep the explicit profile outside
        # the child's native default HOME/.hermes so it gets its own label.
        os_home = self.home / "os-user"
        os_home.mkdir(exist_ok=True)
        env = {k: v for k, v in os.environ.items()
               if not k.startswith("HERMES_") and not k.endswith(("_API_KEY", "_TOKEN"))}
        env.pop("PYTEST_CURRENT_TEST", None)
        env.update({
            "HOME": str(os_home), "HERMES_HOME": str(self.hermes_home),
