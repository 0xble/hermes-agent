...
class TestRuntimeFtsRebuild:
    ...
    def test_same_holder_set_across_futile_window_names_holder_and_gateway_safe_remedy(
        ...
        def doctor_blob():
            # Fork rows are 4-tuples (kind, text, detail, issue); `issue` is an object/None, not
            # text, so join only the string fields rather than every element.
            return " ".join(
                " ".join(part for part in row if isinstance(part, str))
                for row in _render_state_db_stats(collect_state_db_stats(db_path))
            ).lower()
