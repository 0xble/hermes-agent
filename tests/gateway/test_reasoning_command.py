...
def test_prepare_reasoning_turn_event_preserves_multiline_prompt():
    runner = _make_runner()
    event = _make_event("/reasoning high Fix this\nand verify it")
...
def test_gateway_quick_alias_inherits_reasoning_turn_semantics():
    ...
    assert request is not None
    assert event.text == "Fix this"
    assert event.turn_reasoning_config["effort"] == "high"


def test_prepare_reasoning_turn_event_abstains_for_bare_effort():
    runner = _make_runner()
