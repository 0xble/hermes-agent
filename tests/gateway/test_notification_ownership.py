...
async def test_status_ownership_and_late_cleanup():
    ...
    for c in (c1, c2, c3):
        ...
        await a.pop_post_delivery_callback(c.session_key, generation=c.run_generation)()
...
@pytest.mark.asyncio
async def test_cancelled_accepted_progress_receipt_is_cleaned_after_final():
    w, a, g = setup()
    c, t = turn(g, "101")
    w.block_once = True
    c.progress_callback(
    ...
    await asyncio.wait_for(w.accepted.wait(), 5)
    # Overflow queued behind the ambiguous send must not trigger a second send either.
    c.progress_queue.put("additional " * 1000)
    task.cancel()
    await asyncio.wait_for(task, 5)
    assert len([e for e in w.events if e[0] == "send"]) == 1
    g._run_agent_schedule_bubble_cleanup({"final_response": "done"}, a, c)
    await a.pop_post_delivery_callback(c.session_key, generation=c.run_generation)()
    w.release.set()
    for _ in range(100):
