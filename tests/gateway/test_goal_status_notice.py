...
class FakeAdapter:
    ...
    async def send(self, chat_id, content, reply_to=None, metadata=None):
        ...
        return SimpleNamespace(success=True, message_id="status-message-1")
...
@pytest.mark.asyncio
async def test_side_goal_status_notice_keeps_route_and_reply_binding():
    ...
    runner.adapters = {Platform.DISCORD: adapter}
    runner.config = SimpleNamespace(group_sessions_per_user=True, thread_sessions_per_user=False)
    runner._record_side_delivery = AsyncMock()
    ...
    side_key = "agent:main:discord:channel:parent:side:side-root"
    route_metadata = {
        "gateway_session_key": side_key,
        "gateway_session_id": "side-session",
    ...
    assert side_key in adapter.callbacks
    _, callback = adapter.callbacks[side_key]
    await callback()
    recorded_event, recorded_ids = runner._record_side_delivery.await_args.args
    assert recorded_event.metadata == route_metadata
    assert recorded_ids == ["status-message-1"]
