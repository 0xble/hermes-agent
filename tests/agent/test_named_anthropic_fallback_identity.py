...
def test_effective_authority(key, fallback):
    ...
    try:
        ...
        # The constructor intentionally omits the unused channel. Its dormant
        # SDK attribute is not wire authority until that omission is removed.
        pin.validate_request(child, {"model": route.model}, client=client)
        from anthropic import Omit
        original_headers = dict(client._custom_headers)
        client._custom_headers = {
            name: value for name, value in original_headers.items()
            if not isinstance(value, Omit)
        }
        with pytest.raises(ValueError):
            pin.validate_request(child, {"model": route.model}, client=client)
        client._custom_headers = original_headers
