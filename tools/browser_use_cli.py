...
def _resolve_real_profile_cdp(
    ...
    if env.get("BU_CDP_WS") or env.get("BU_CDP_URL"):
        ...
        return None
    # Resolved through the ``tools.browser_tool`` facade, not the extraction modules: that is the
...
def _route_backend(
    ...
    """Resolve where the harness connects; return an error string or None."""
    rp_err = _resolve_real_profile_cdp(
        env, force_local=local, identity=identity, headed=headed
    )
    if rp_err:
        return rp_err
    ...
    if local and not _has_cdp_env(env) and not _real_profile_consented():
        return (
            "local=true was requested but browser.use_real_profile is off. Enable it in "
            "config.yaml (browser.use_real_profile: true) or the desktop Settings → "
            "Browser section, then retry."
        )
...
def _browser_exec(
    ...
    # On a SHARED browser (/browser connect CDP override) a fresh named daemon
