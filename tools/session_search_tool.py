...
def session_search(query: str = "", role_filter: str = None, limit: int = 3, db=None,
    ...
    if platform is None and current_session_id:
        platform = _get_session_meta(db, current_session_id).get("source")
    try:
...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "referring the user to a session, use clickable `link` references only on Hermes Desktop. "
        "On other surfaces, refer to the returned `title` as plain text and follow the response link_hint."
    ),
    "parameters": {
        "type": "object",
        "properties": {
