...
def _create_local_session(task_id: str, allow_real_profile: bool = True,
    ...
    ``identity`` (HERMES-064) pins one configured local real-profile alias and stamps the session
    with its opaque runtime key and owning Hermes home, so a later call can prove the task is still
    on the same cookie jar.
    """
    ...
    if identity:
        ...
        if resolved_identity is None:  # defensive: a non-empty alias always resolves or raises
    ...
    if allow_real_profile:
        try:
            cdp_url, err = (_bt._real_profile_cdp(identity) if identity else _bt._real_profile_cdp())
        except BrowserIdentityError as exc:
            raise RuntimeError(str(exc)) from exc
        if err:
        ...
        if cdp_url:
            ...
            if identity:
                ...
                info["browser_identity"] = identity
                info["browser_identity_key"] = browser_identity_scope_key(resolved_identity.runtime_key)
                info["browser_identity_home"] = hermes_home_key()
...
def _resolve_identity_key(alias: str, *, unresolved_error: Optional[str] = None) -> str:
    """Opaque runtime key for ``alias``; ``unresolved_error`` replaces the resolver's message."""
    from hermes_cli.browser_identity import (BrowserIdentityError, browser_identity_scope_key,
                                             resolve_browser_identity)
    try:
        resolved = resolve_browser_identity(alias)
    except BrowserIdentityError as exc:
        raise RuntimeError(unresolved_error or str(exc)) from exc
    if resolved is None:  # defensive: a non-empty alias always resolves or raises
        raise RuntimeError(unresolved_error or "browser identity could not be resolved")
    return browser_identity_scope_key(resolved.runtime_key)


_IDENTITY_SWITCH_ERROR = ("browser task is already bound to another identity; start a new task "
                          "instead of switching cookie jars")


def _get_session_info(task_id: Optional[str] = None, identity: Optional[str] = None) -> Dict[str, Any]:
    ...
    HERMES-064: a task's browser identity is bound ONCE and durably (a claim directory under the
    Hermes home), so it survives a process restart and can never be re-pointed at another cookie
    jar — not by an explicit argument, not by a config edit that remaps the alias, and not by
    another Hermes profile reusing the task id.
    """
    if task_id is None:
        task_id = "default"
    ...
    durable_identity = _bt._read_browser_identity_binding(task_id) if not force_local else None
    if durable_identity is not None:
        durable_alias, durable_key = durable_identity
        current_durable_key = _resolve_identity_key(
            durable_alias,
            unresolved_error="browser task identity binding no longer resolves; start a new task")
        if current_durable_key != durable_key:
            raise RuntimeError("browser task identity binding no longer matches its configured "
                               "profile; start a new task")
        if requested_identity_key and requested_identity_key != durable_key:
            raise RuntimeError(_IDENTITY_SWITCH_ERROR)
        identity, requested_identity_key = durable_alias, durable_key
    ...
    def _ensure_identity_binding(session_info: Dict[str, Any]) -> None:
        ...
        if not force_local and session_info.get("browser_identity"):
            _bt._claim_browser_identity_binding(task_id, str(session_info["browser_identity"]),
    ...
    # Captured BEFORE any teardown: a recycled task keeps its bound identity even when the
    # session object it was stamped on is replaced.
    recycled_identity = str((existing_session or {}).get("browser_identity") or "")
    recycled_identity_key = str((existing_session or {}).get("browser_identity_key") or "")
    ...
    if existing_session is not None:
        # Validate an explicit request BEFORE ensure_healthy() can tear the session down.
        ...
        elif not _bt._session_has_expired(existing_session) and not _local_backend_process_dead(existing_session):
    ...
    with _bt._cleanup_lock:
        if task_id in _bt._active_sessions:  # created concurrently during the network call — don't leak ours
            concurrent = _bt._active_sessions[task_id]
            _ensure_identity_binding(concurrent)
            return concurrent
        session_info = dict(session_info)
