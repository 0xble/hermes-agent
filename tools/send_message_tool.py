...
# HERMES-022: ``send_message`` is registered ONLY as the explicit opt-in ``messaging`` toolset (see the
# registration at the end of this module). Ordinary CLI, Telegram, API, ACP, webhook, delegated and
# default cron sessions still never receive it; a cron job gets it only with ``allow_messaging=true``.
...
def send_message_tool(args, **kw):
    ...
    cron_result = _maybe_handle_cron_outbound(args)
...
def _handle_send(args):
    ...
    try:
        ...
        # HERMES-022: bind the send to its owning profile from the TRUSTED session env, never from
        # model-authored tool args, and only when one is actually set — an unconditional
        # ``profile=None`` would change the exact built-in call contract for every ordinary send.
        try:
            from gateway.session_context import get_session_env
            trusted_profile = str(get_session_env("HERMES_SESSION_PROFILE", "") or "").strip()
        except Exception:
            trusted_profile = ""
        if trusted_profile:
...
async def _send_live_adapter_media(adapter, chat_id, message, media_files, *, thread_id=None, metadata=None,
    ...
    if len(media_files) > 1 and not force_document:
        batch_method = getattr(type(adapter), "send_multiple_images", None)
        if batch_method is not None and batch_method is not getattr(BasePlatformAdapter, "send_multiple_images"):
            images = []
            for index, descriptor in enumerate(media_files):
                ...
                if (not is_voice and isinstance(path, str)
...
def _check_send_message():
    """Gate send_message on a live gateway (always available on messaging platforms).

    Kanban workers also pass: the dispatcher sets ``HERMES_KANBAN_TASK`` on every spawned worker, but
    those workers run under the assignee profile's ``HERMES_HOME``, which has no ``gateway.pid`` — so the
    gateway-running check would fail even though the parent gateway is alive. Honoring the env var lets a
    worker deliver rich content straight to the originating chat instead of the kanban notifier's
    ~200-char first-line truncation.

    Opted-in cron jobs pass too. ``HERMES_CRON_ALLOW_MESSAGING=1`` is set only for that job's isolated
    run, so the tool stays hidden from ordinary cron, CLI and delegated-agent sessions.
    """
    ...
    if platform and platform != "local":
        return True
    try:
        # ``get_running_pid`` is the defining API. ``is_gateway_running`` exists only in
        # gateway/status.py's revert-scheduled PLUGIN-COMPAT block, which in-tree code must not
        # import (scripts/check_compat_pointers.py fails CI on it). See COMPAT_MANIFEST.md.
