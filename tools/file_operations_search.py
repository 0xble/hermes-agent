...
class SearchMixin:
 ...
 lane cannot express (the multi-root ``cd`` prefix); ``shell_prefix`` is shell-only."""
    ...
    def _search_files_rg(self, pattern: str, path: str | List[str], limit: int, offset: int,
                         ...
                         explicit_hidden_root: bool = False) -> SearchResult:
        ...
        # An explicit hidden root is allowed, not its hidden descendants.
        hidden_flags = " --hidden -g '!.*' -g '!**/.*'" if explicit_hidden_root else ""
        ...
        rg_cmd = (f"{rg} --files{sort_arg} -g {self._escape_shell_arg(glob_pattern)}"
                  f"{hidden_flags}{exclusion_args} -- {root_args}")
