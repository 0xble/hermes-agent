...
def _resolve_config_cwd(env_type: str, mount_docker_cwd: bool) -> tuple:
    ...
    # HERMES-015: read through the session cwd authority, not raw TERMINAL_CWD. A concurrent cron
    # workdir job (or a gateway turn on another profile) binds its cwd on the ContextVar and never
    # mutates the process env, so a raw read here resolves some other execution context's directory.
