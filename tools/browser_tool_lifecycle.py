...
def _emergency_cleanup_all_sessions():
    ...
    # Real-profile Chrome is launched directly (not by agent-browser), so session cleanup never
    # reaps it; the sweep is per-identity across every profile at exit.
    _best_effort("Real-profile runtime cleanup on exit",
                 lambda: _bt._close_all_real_profile_runtimes(all_profiles=True))
...
def cleanup_all_browsers() -> None:
    ...
    # Shutdown reaps EVERY profile's identity runtimes: per-identity tracking supersedes the
    # global real-profile chrome sweep, and each tracked snapshot browser must be stopped.
    _bt._close_all_real_profile_runtimes(all_profiles=True)
