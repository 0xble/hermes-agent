...
def _resolve_child_cwd(mode: str, staging_dir: str, task_id: str = "") -> str:
    ...
    if task_id:
        ...
        if session_cwd and os.path.isdir(session_cwd):
            return session_cwd
    # HERMES-015: resolve_tool_cwd, not scope_terminal_cwd — the turn-bound logical cwd (including an
    # explicit empty isolation scope) outranks the process-wide value, which a concurrent cron turn
    # in the same process may own.
