...
def _check_skill_security(name: str, skill_md: Path, content: str, all_dirs, active_skills_dir):
    """Refuse a skill whose RESOLVED path escapes every configured root; warn on
    suspected prompt injection. Returns a failure JSON string, or None to proceed.

    Escaping the roots is a refusal, not a warning: a symlink planted in the
    skills directory would otherwise load arbitrary file content into the model's
    context under a trusted skill name. The check is on the RESOLVED path: every
    candidate is built as ``<search_dir>/...`` so a lexical test can never fire,
    and a SKILL.md symlinked to a file outside every root is exactly what this
    guards against. Injection patterns stay advisory because they are heuristic
    and fire on legitimate content.
"""
    ...
    if any(p in content.lower() for p in _INJECTION_PATTERNS):
        logger.warning("Skill security warning for '%s': %s", name,
...
def skill_view(
    ...
    try:
        ...
        if (refusal := _check_skill_security(name, skill_md, content, all_dirs, active_skills_dir)) is not None:
            return refusal
...
def _skill_view_with_bump(args, **kw):
    ...
    with suppress(Exception):
        ...
        if isinstance(parsed, dict) and parsed.get("success"):
            ...
            # A delegated child's read must not move the parent's usage counters:
            # the curator's stale timer keys off last_used_at, so borrowed views
            # would keep a dead skill looking alive.
            from agent.delegation_context import is_read_only_knowledge_context
            resolved = parsed.get("name") or name  # qualified forms return the canonical name
