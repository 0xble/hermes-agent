...
# ---------------------------------------------------------------------------
# HERMES-064/069/091/092 — named real-profile identities
#
# Every credential-bearing and process-bearing resource is keyed by an opaque
# identity/browser/source-profile/Hermes-home tuple, so two aliases never share a
# snapshot, an agent-browser session, a CDP cache entry, or a Chromium process.
# ---------------------------------------------------------------------------
...
_real_profile_browser_processes: Dict[str, tuple] = {}


def _migrate_real_profile_runtime_state(state, previous_key: str, cache_key: str) -> None:
    """Carry one runtime map's entry from a pre-home-scoped key to the current key."""
    if previous_key not in state or cache_key in state:
        return
    state[cache_key] = state.pop(previous_key)


def _real_profile_runtime_resources(identity) -> tuple:
    """``(session_name, lock, cache_key)`` owned by ``identity`` under the active Hermes home.

    ``identity`` None is the legacy unconfigured path, which keeps its own reserved key so an
    unconfigured install behaves byte-for-byte as before.
    """
    ...
    with _real_profile_cdp_locks_guard:
        ...
        lock = _real_profile_cdp_locks.setdefault(lock_key, previous_lock or threading.Lock())
        # Only adopt pre-home-scoped state that belongs to THIS home; another profile's
        # runtime must never be inherited across the rename.
        if _real_profile_session_homes.get(previous_cache_key) == home_key:
            for state in (_real_profile_cdp_cache, _real_profile_headed_modes,
                          _real_profile_session_names, _real_profile_session_homes,
                          _real_profile_browser_processes):
                _migrate_real_profile_runtime_state(state, previous_cache_key, cache_key)
        session_name = (f"hermes-rp-{runtime_key}" if identity is not None
...
def _track_real_profile_session(cache_key: str, session_name: str) -> None:
    """Record the agent-browser session owning ``cache_key`` and the home that owns it."""
    from hermes_constants import hermes_home_key
...
def _reload_browser_use_runtime(runtime_key: str) -> bool:
    """Stop this identity's Browser Use daemons so they cannot keep a stale CDP attachment."""
    try:
        from tools.browser_use_cli import _reload_browser_exec_daemons_for_runtime
        from hermes_constants import hermes_home_key

        return _reload_browser_exec_daemons_for_runtime(runtime_key, home_key=hermes_home_key())
    except Exception as exc:
...
def _close_all_real_profile_runtimes(*, all_profiles: bool = False) -> None:
    ...
    Per-identity reaping supersedes the global ``_terminate_real_profile_chrome()`` sweep: a
    profile-scoped cleanup must never close another Hermes home's authenticated browser.
    """
    from hermes_constants import hermes_home_key
    ...
    with _real_profile_cdp_locks_guard:
        selected_keys = tuple(key for key in _real_profile_session_names
                              if all_profiles or _real_profile_session_homes.get(key) == active_home)
    ...
    if not all_profiles:
        # Managed snapshot Chromium intentionally survives gateway restarts, so process-local
        # maps may be empty when consent is revoked. Discover each credential-bearing runtime
        # from its ownership marker before cleanup.
        try:
            from hermes_cli.browser_connect import stop_snapshot_browser_processes
...
def _preserve_browser_between_turns() -> bool:
    ...
    try:
        from hermes_cli.browser_connect import (UNSUPPORTED_CHANNEL, detect_default_chromium,
                                                real_profile_copy_dir)
        from hermes_cli.browser_identity import (configured_identity_aliases,
        ...
        identities = (tuple(resolve_browser_identity(alias, browser_cfg=browser_cfg)
                            for alias in aliases) if aliases else (None,))
        recovered_modes: list = []
        ...
        for identity in identities:
            ...
            if mode is None:
                ...
                continue
            session_name, _lock, cache_key = _real_profile_runtime_resources(identity)
...
def _browser_identity_binding_dir(task_id: str) -> Path:
    """Profile-scoped immutable identity claim directory for ``task_id``."""
    import hashlib
...
def _read_browser_identity_binding(task_id: str):
    """``(alias, owner_key)`` durably bound to ``task_id``, or None. Corrupt claims fail closed."""
    ...
    try:
        ...
        raise RuntimeError("browser task identity binding is unreadable; start a new task") from exc
    if (not alias or not owner or len(alias) > 128 or len(owner) > 128
            or any(ch.isspace() for ch in alias) or any(ch.isspace() for ch in owner)):
        raise RuntimeError("browser task identity binding is corrupt; start a new task")
    return alias, owner
...
# ---------------------------------------------------------------------------
# Identity-routing seam. The extracted modules reach these names back through
# ``_bt`` (tools.browser_tool_origin), so the facade stays the single place a
# caller — or a regression that has to stub one boundary — rebinds them.
# ---------------------------------------------------------------------------
from tools import browser_tool_real_profile as _real_profile  # noqa: E402

# Each seam DELEGATES at call time rather than binding the sibling's function object, so a caller
# (or a regression) may stub either the facade name or the owning module's and both take effect.
def _seam(module, name: str):
    def call(*args, **kwargs):
        return getattr(module, name)(*args, **kwargs)
    call.__name__ = call.__qualname__ = name
    call.__doc__ = f"Identity-routing seam for :func:`{module.__name__}.{name}`."
    return call


_use_real_profile = _seam(_cloud, "_use_real_profile")
_is_headed_mode = _seam(_cloud, "_is_headed_mode")
_get_cloud_provider = _seam(_cloud, "_get_cloud_provider")
_get_cdp_override_raw = _seam(_cdp, "_get_cdp_override_raw")
_get_cdp_override = _seam(_cdp, "_get_cdp_override")
_resolve_cdp_override = _seam(_cdp, "_resolve_cdp_override")
_find_agent_browser = _seam(_install, "_find_agent_browser")
_start_browser_cleanup_thread = _seam(_lifecycle, "_start_browser_cleanup_thread")
_update_session_activity = _seam(_lifecycle, "_update_session_activity")
_session_has_expired = _seam(_lifecycle, "_session_has_expired")
_cleanup_single_browser_session = _seam(_lifecycle, "_cleanup_single_browser_session")
_cdp_http_ready = _seam(_real_profile, "_cdp_http_ready")
_cdp_on_data_dir = _seam(_real_profile, "_cdp_on_data_dir")
_cdp_owned_by_data_dir = _seam(_real_profile, "_cdp_owned_by_data_dir")
_owned_profile_cdp = _seam(_real_profile, "_owned_profile_cdp")
_read_real_profile_headed_mode = _seam(_real_profile, "_read_real_profile_headed_mode")
_agent_browser_get_cdp = _seam(_real_profile, "_agent_browser_get_cdp")
_agent_browser_close_session = _seam(_real_profile, "_agent_browser_close_session")
_real_profile_cdp = _seam(_real_profile, "_real_profile_cdp")
_cleanup_real_profile_state = _seam(_real_profile, "_cleanup_real_profile_state")
_create_local_session = _seam(_session, "_create_local_session")
_get_session_info = _seam(_session, "_get_session_info")
...
def browser_navigate(url: str, task_id: Optional[str] = None, identity: Optional[str] = None) -> str:
    ...
    ``identity`` (HERMES-064) is only accepted here, at session entry: it binds the task to one
    ...
    # Only pass ``identity`` when one actually resolved: the unconfigured lane must keep calling
    # the session entry point with its historical signature.
    session_info = (_get_session_info(nav_session_key, identity=resolved_identity.alias)
                    if resolved_identity is not None else _get_session_info(nav_session_key))
...
def _browser_navigate_schema_overrides() -> dict:
    ...
    Follow-up tools never take ``identity``: the task's identity is immutable once bound, so
    offering the argument again would advertise a switch that must fail.
    """
    from hermes_cli.browser_identity import configured_identity_aliases, read_browser_identity_config
    ...
    properties["identity"] = {
        "type": "string",
        "enum": list(aliases),
        "description": ("Configured local real-profile browser identity. It binds this task's "
...
def _browser_navigate_handler_entry(args, **kw):
    return _browser_navigate_handler(args, kw)
...
for _name, _emoji, _check_fn, _defaults, *_extra in _BROWSER_TOOL_TABLE:
    ...
    _extra_register = ({"dynamic_schema_overrides": _browser_navigate_schema_overrides}
                       if _name == "browser_navigate" else {})
    _handler = (_browser_navigate_handler_entry if _name == "browser_navigate"
                else _routed_handler(_name, _fallback_call(_name, _defaults, *_extra)))
    registry.register(name=_name, toolset="browser", schema=_BROWSER_SCHEMA_MAP[_name],
                      handler=_handler, check_fn=_check_fn, emoji=_emoji, **_extra_register)
