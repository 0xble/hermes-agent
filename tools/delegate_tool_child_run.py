...
def _attach_child(parent_agent: Any, child: Any) -> None:
    ...
    if hard is None or hard.is_set():
        _signal_child_stop(
            child, message or "parent agent interrupted",
            tool_reason=getattr(parent_agent, "_tool_interrupt_reason", None) or "delegation cancelled",
        )
    else:
        ...
        def _run_with_thread_capture():
            ...
            with delegated_child_context(
                ...
                return child.run_conversation(
                    user_message=_build_child_goal_message(goal, _images, child) if _images else goal, task_id=self.child_task_id, stream_callback=self.relay_text,
                )
