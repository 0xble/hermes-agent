...
def _worktree_path_warning(command: str, command_cwd: Optional[str]) -> Optional[str]:
    """HERMES-105 advisory when a direct command creates a worktree outside ``.worktrees/``."""
    try:
        from tools.worktree_path_guard import nonstandard_worktree_add_warning
        return nonstandard_worktree_add_warning(command, command_cwd or "")
    except Exception:
    ...
    *, command: str, result: dict, env: Any, env_type: str, effective_task_id: str,
    ...
    optional_fields: list[tuple[str, Any]] = [
        ...
        *_redact_spill_file(result.get("full_output_path"), result.get("output_total_chars"), command),
        ...
        # HERMES-105: advisory only — Git accepts worktrees anywhere, but trees outside
        # `.worktrees/` escape Hermes's lifecycle and are hard for an agent to inventory or clean.
        # Never blocks the command; the warning rides alongside its result.
        ("warning", _worktree_path_warning(command, command_cwd)),
