...
def read_file_tool(path: str, offset: int = 1, limit: int = DEFAULT_READ_LIMIT, task_id: str = "default") -> str:
    ...
    try:
        ...
        # HERMES-015: pass the path already resolved for this task. File backends may be shared
        # across sessions, so re-resolving a relative path against the backend environment's
        # mutable cwd can read another session's file.
        result = _get_file_ops(task_id).read_file(resolved_str, offset, limit)
