...
class _EmbeddedCuaDaemon:
    ...
    # Upper bound for ONE readiness probe (HERMES-047). Upstream's fixed 2s permanently fails a
    # healthy-but-slow status client (cua-driver 0.21's eager attestation): every probe is killed
    # before it can answer. Each probe gets headroom, still bounded by the remaining budget above.
    ...
    def start(self) -> None:
        ...
        while True:
            # One clock read per iteration: it bounds BOTH the overall budget and this probe, so a
            # second read could hand the probe a negative timeout after a slow prior probe.
            now = time.monotonic()
            if now >= deadline:
                break
            ...
            # HERMES-047: give each probe real headroom, bounded by the remaining startup budget.
            # A fixed short per-probe timeout permanently fails a healthy-but-SLOW status client —
            # every probe is killed before it can answer, so startup times out though the daemon is fine.
            if self._socket_ready(env, min(self._STATUS_PROBE_TIMEOUT_SECONDS, deadline - now)):
                self._running = True
                return
            time.sleep(0.1)
        self.stop()
    ...
    def _socket_ready(self, env: Dict[str, str], timeout: Optional[float] = None) -> bool:
        """``cua-driver status --socket`` exits 0 once the private daemon accepts connections."""
        probe = _cb()._run_quiet(
            [self._command, "status", "--socket", self.socket_path],
            timeout=self._STATUS_PROBE_TIMEOUT_SECONDS if timeout is None else timeout,
            env=env, swallow=_QUIET_ERRORS)
