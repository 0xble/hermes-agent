...
def _authoritative_workspace_root(task_id: str = "default") -> str | None:
    ...
    Order: (1) the session's own cwd record (per-session, so one session's ``cd`` never leaks into
    another); (2) a registered raw-keyed cwd override (TUI/Desktop/ACP); (3) the turn-scoped logical
    cwd; (4) a sentinel-free absolute ``$TERMINAL_CWD``.
    """
    try:
    ...
    if recorded:
        return recorded
    if registered := _registered_task_cwd_override(task_id):
        return registered
    # HERMES-015: a gateway turn binds its logical workspace on a ContextVar. A FRESH turn has run no
    # terminal command yet, so there is no per-session record above — and the process-wide
    # TERMINAL_CWD below can belong to a concurrent cron turn in the same process. Resolving the
    # scoped value first is what stops a cron job's cwd mutation from redirecting an interactive turn.
    try:
