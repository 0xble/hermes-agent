...
| HERMES-028 | Active | `fix: harden gateway runtime boundaries`; `fix(terminal): avoid remote Python lifecycle dependency`; `fix(terminal): skip known remote shell binaries`; `fix(terminal): trust only system shell paths`; `fix(terminal): parse guarded shell grammar safely` | Inspect referenced remote scripts through the POSIX shell contract without requiring Python, while distinguishing inert Bash grammar from genuine unresolved executable references. |
