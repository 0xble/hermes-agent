...
| HERMES-031 | Active | `fix(output): preserve answers before verification receipts`; `fix(output): preserve repeated verification candidates durably`; `fix(output): keep verification candidates provisional`; `fix(output): persist one canonical verification row`; `fix(output): preserve replacement preview and tool ordering`; `fix(output): preserve authored verification reports` | Keep a substantive answer when a verify-on-stop continuation returns only a verification receipt. |
