...
- **Surfaces:** `agent/title_generator.py`; `gateway/run.py`; `hermes_state.py`; `hermes_cli/main.py`; `hermes_cli/telegram.py`; `hermes_cli/telegram_user.py`; `hermes_cli/telegram_topic_icons.py`; `plugins/platforms/telegram/adapter.py`; `plugins/platforms/telegram/user_transport.py`; `plugins/platforms/telegram/mtproto_telethon.py`; `plugins/platforms/telegram/topic_icons.py`; `plugins/platforms/telegram/topic_icon_service.py`; `plugins/platforms/telegram/topic_icon_gateway.py`; `plugins/platforms/telegram/topic_icon_runtime.py`; `pyproject.toml`; `uv.lock`; `cli-config.yaml.example`; Telegram messaging documentation; focused CLI, policy, transport, catalog, service, state, title, and gateway tests.
- **Upstream tracking:** Independent 2026-08-15 incident hypothesis was frozen before searching upstream: the LLM icon chooser can return `None` after provider failure or an out-of-allowlist response, while the gateway silently performs a title-only rename. The correction belongs in the selector boundary: a deterministic allowed-list fallback plus warning-level reason logging, while preserving manual-icon ownership and title independence. Runtime evidence on 2026-08-20 additionally showed that completion-only title scheduling leaves long-running topics generic despite useful visible assistant prose; the bounded single-claim interim path belongs at the gateway turn boundary. Current upstream `main` at `f43eabee5f36e11448086ee8ee17c499958e81bf` has no `choose_topic_icon`, response-aware Telegram title scheduler, or interim title timer; open source PR `#66353` and related PR `#35737` retain model-only icon selection without this deterministic failure fallback or interim scheduling contract.
- **Upstream PR:** Source: #66353; related: #35737 (both open; no deterministic failure fallback equivalent; checked 2026-08-15).
- **Regression:** `scripts/run_tests.sh tests/hermes_cli/test_telegram_namespace.py tests/hermes_cli/test_telegram_user.py tests/hermes_cli/test_telegram_topic_icons.py tests/gateway/test_telegram_user_policy.py tests/gateway/test_telegram_user_transport.py tests/gateway/test_telegram_custom_topic_icons.py tests/gateway/test_telegram_topic_icon_runtime.py tests/gateway/test_telegram_topic_icon_service.py tests/agent/test_title_generator.py tests/gateway/test_session_title_rename_lane.py tests/gateway/test_telegram_topic_mode.py tests/test_hermes_state.py tests/test_telegram_topic_status_ptb.py tests/hermes_cli/test_backup.py tests/test_project_metadata.py -q`. Live release proof requires attended identity authorization, exact disposable topic mutation/readback/cleanup, and two unique cases per configured Apple pack with zero default IDs or readback mismatches.
