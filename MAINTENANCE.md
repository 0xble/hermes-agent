...
- **Summary:** Opt-in semantic native Telegram topic icons resolve against the live allowed sticker set, honor exact emoji overrides, preserve observed manual choices, validate bindings immediately before mutation, and avoid the 24 most recently selected icons with durable per-chat least-recently-used history. For an eligible default-owned topic with a nonempty live allowlist, malformed or failed model selection falls back to a deterministic semantic/least-recent choice and logs the fallback reason; icon transport failure still never blocks session-title persistence or topic renaming. Topic title/icon generation uses the opening request plus useful assistant output: a completed reply is preferred, while a long-running turn may claim the single rename lane after 20 seconds of visible streamed prose so delayed or non-terminating work does not leave the topic generic. The combined title context is bounded before it crosses the auxiliary-provider boundary.
- **Surfaces:** `agent/title_generator.py`; `gateway/run.py`; `hermes_state.py`; `plugins/platforms/telegram/adapter.py`; `website/docs/user-guide/messaging/telegram.md`; `tests/gateway/test_session_title_rename_lane.py`; focused state, selector, adapter, title, and gateway tests.
...
- **Regression:** `scripts/run_tests.sh tests/agent/test_title_generator.py tests/gateway/test_session_title_rename_lane.py tests/gateway/test_telegram_topic_mode.py tests/test_hermes_state.py tests/test_telegram_topic_status_ptb.py -q` plus one disposable live Telegram topic canary proving an eligible topic receives an allowed icon when model selection fails. Separately prove a long-running turn with at least 20 characters of visible streamed assistant prose claims exactly one title lane and includes bounded excerpts from both request and response.
