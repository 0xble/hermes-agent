...
| HERMES-031 | Active | `fix(output): preserve answers before verification receipts`; `fix(output): preserve repeated verification candidates durably` | Keep a substantive answer when a verify-on-stop continuation returns only a verification receipt. |
...
| HERMES-041 | Active | `fix(gateway): fail closed on inherited restart marker`; `fix(safety): block destructive gateway launchctl verbs`; `fix(safety): narrow live lifecycle guard to executable actions`; `fix(safety): close executable wrapper bypasses` | Prevent gateway-derived contexts from bypassing lifecycle self-control guards. |
...
- **Regression:** `pytest -q tests/run_agent/test_verification_continuation_budget.py`; Desktop `hidden-history-messages.spec.ts` live verify-on-stop case with interim assistant messages disabled, followed by a page reload proving the composed answer remains durable.
