...
| HERMES-041 | Active | `fix(gateway): fail closed on inherited restart marker`; `fix(safety): block destructive gateway launchctl verbs`; `fix(safety): narrow live lifecycle guard to executable actions`; `fix(safety): close executable wrapper bypasses`; `fix(safety): block dynamic lifecycle executables` | Prevent gateway-derived contexts from bypassing lifecycle self-control guards. |
