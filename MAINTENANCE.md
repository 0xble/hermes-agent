...
| HERMES-017 | Active | `feat(title): add compact titles and canonical aliases`; `feat(titles): configure concise distinct session titles`; `feat(titles): support configurable casing`; `fix(titles): reject malformed auxiliary output`; `fix(titles): reject incomplete markdown fences`; `fix(titles): harden Gemini fallback generation`; `docs(maintenance): track related title fixes`; `fix(titles): reuse healthy auxiliary routes`; `fix(auxiliary): preserve route attribution with complete responses`; `test(auxiliary): stabilize timeout deadline`; `fix(telegram): harden response-aware topic titles`; `fix(titles): make omitted character cap intentional`; `fix(titles): preserve canonical brand spelling`; `docs(maintenance): record upstream title divergence` | Make title shape configurable while preserving durable, race-safe uniqueness and the usable derived title when model output is malformed or truncated. |
| HERMES-018 | Active | `feat(telegram): add semantic topic icons and robust auto-renames`; `feat(telegram): remember 24 recent topic icons`; `fix(fork): preserve reconciled patch contracts`; `fix(telegram): fall back deterministically for topic icons`; `fix(titles): keep completed responses in primary provider`; `feat(telegram): title topics after completed replies`; `fix(telegram): suppress duplicate topic title updates`; `fix(telegram): defer topic metadata until response`; `fix(telegram): suppress derived topic rename`; `fix(telegram): title topics from interim responses`; `fix(telegram): bound response-aware title context`; `fix(telegram): harden response-aware topic titles`; `docs(maintenance): record upstream title divergence`; `feat(telegram): add user-authorized custom topic icons`; `fix(telegram): create MTProto sessions privately`; `fix(telegram): repair transport safety regressions`; `fix(telegram): retry new-topic propagation once` | Select live Telegram topic icons without repeating the 24 most recent choices, overwriting manual icons, or silently leaving an eligible topic iconless after model-selection failure; title long-running topics from useful interim assistant output, with opt-in bounded request/response excerpts routed only through configured title-generation and fallback routes. |
...
| HERMES-028 | Active | `fix: harden gateway runtime boundaries`; `fix(terminal): avoid remote Python lifecycle dependency`; `fix(terminal): skip known remote shell binaries`; `fix(terminal): trust only system shell paths`; `fix(terminal): parse guarded shell grammar safely`; `fix(terminal): preserve shell dialect boundaries` | Inspect referenced remote scripts through the POSIX shell contract without requiring Python, while distinguishing inert Bash grammar from genuine unresolved executable references. |
...
- **Summary:** Allows a healthy private cua-driver status client up to five seconds within the existing overall startup deadline, retains the last readiness failure in diagnostics, and launches bounded or unrestricted private daemons through the exact signed CuaDriver.app on macOS so Accessibility and Screen Recording grants remain attached to the runtime. LaunchServices receives the sanitized Cua permission and telemetry variables explicitly. Forced cleanup identifies only the unique verified `serve --embedded --socket <private path>` process, excludes MCP proxies, re-verifies before each signal, handles a dead `open -W` wrapper, and preserves the recovery socket whenever discovery is inconclusive. Linux and Windows retain the raw executable path.
- **Surfaces:** `tools/computer_use/cua_backend.py`; `tests/tools/test_computer_use_cua_0_10_permissions.py`; `tests/tools/test_computer_use_browser_authorization.py`; this record.
- **Upstream tracking:** Direct Hermes issue #93312 covers the readiness timeout. Direct Cua issue trycua/cua#3347 covers cua-driver 0.21's eager Computer History attestation latency. Related Hermes issue #84033 and open PR #76433 cover the macOS TCC launch identity; PR #76433's architecture is relevant but its current signature validator accepts a suffixed identifier and any nonempty signing team. Related open PR #76686 bounds startup but leaves the inner two-second probe unchanged. Checked 2026-08-23 against upstream `4a3e5c4094d96c2e184dead99bccb4f4aabb2e1b`, which still contains both defects.
- **Upstream PR:** None for the complete two-part contract after checked 2026-08-23. Related only: #76433 and #76686.
- **Regression:** `323 passed, 3 skipped` across all computer-use and maintenance-validator tests; `67 passed` across the two focused permission/authorization files; Ruff clean; Python compilation clean; `git diff --check` clean. Sabotage runs proved the slow-probe, signed-app launch, and exact identity tests fail on the old behavior. The isolated live macOS cua-driver 0.21 canary returned 117 apps, six windows, a 1456×887 Chrome capture with a 247,879-byte PNG and 666 AX elements, observed all three explicit `CUA_DRIVER_*` variables, and confirmed the private daemon was absent after shutdown. Independent Claude Max review addressed four lifecycle/security rounds and approved the final diff with no P0/P1 findings.
- **Published commit identity:** Stable subject `fix(computer-use): preserve private macOS runtime readiness`; source and manifest ship together in the same fork commit.
- **Activation:** Source publication is separate from runtime promotion. The active gateway keeps its imported pre-patch adapter until a separately authorized restart.
- **Rollback:** Revert `fix(computer-use): preserve private macOS runtime readiness` in a follow-up commit. Restore the two-second readiness subprocess bound and raw executable launch on macOS while preserving unrelated computer-use patches, including HERMES-046. Expect cua-driver 0.21 private startup and macOS TCC capture failures to return.
- **Retirement:** Retire after released upstream both tolerates healthy private readiness clients beyond two seconds with actionable diagnostics and launches macOS private runtimes with verified signed-app TCC identity, while preserving private sockets, launch-time authorization, lifecycle cleanup, strict bundle identity, and focused plus live regressions.
...
- **Shell-grammar correction hypothesis (2026-08-21):** HERMES-028 deliberately fails closed when the executable position of a recursively inspected shell script contains unresolved expansion. Its line-oriented `shlex` walk incorrectly promotes inert Bash grammar into that position: a `case` branch pattern such as `*)` and a wildcard operand after `||` inside `[[ ... ]]` become fake dynamic executables. The correction must preserve raw quote provenance because quoted data such as `'[['` and `';;'` must not poison parser state or hide a later lifecycle command. Heredoc bodies require a separate boundary: mask every well-formed body from the outer parser, recursively inspect every lifecycle heredoc body and executable substitution, and fail closed on malformed heredocs. Dynamic executable arrays such as `${name[@]}` remain blocked; repository-owned scripts should invoke a known executable directly when no real indirection is required. Arbitrary `$RUNNER`, `bash $SCRIPT`, command/process substitution, quoted-grammar bypasses, heredoc-state poisoning, and referenced lifecycle scripts remain blocked. No state, configuration, dependency, schema, or platform migration is required.
- **Summary:** Keeps the gateway self-control guard fail-closed while reading bounded referenced scripts through the target's POSIX `sh` and `dd` contract. Supported SSH, container, and sandbox targets no longer need Python merely to run an otherwise valid shell script. Canonical absolute shell paths under immutable system directories (`/bin`, `/usr/bin`) are treated as interpreters, while their script arguments remain recursively inspected; user-writable `/usr/local` and Homebrew shells are inspected as executable content. Valid `case` patterns and `[[ ... ]]` operands are distinguished from executable expansion using quote-aware tokens. Heredoc bodies are partitioned from outer shell grammar and recursively scanned according to their execution boundary without relaxing unresolved dynamic command references.
- **Surfaces:** `cron/lifecycle_guard.py`; `tools/shell_heredoc.py`; `tools/terminal_tool.py`; `tests/hermes_cli/test_gateway_restart_loop.py`; `tests/tools/test_terminal_heredoc_background_guard.py`.
- **Upstream tracking:** The exact regression is fork-only because released upstream does not carry HERMES-028's unresolved-executable fail-closed contract. Related open issue #85557 and PR #85804 address multiline tokenization; open PR #88336 broadly reduces lifecycle-guard false positives; issue #91433 and PR #91604 address inline interpreter payloads; closed issue #87401 addressed a distinct directory-token trigger. Current upstream and those candidate heads allow `$RUNNER` and `bash $SCRIPT`, so none is an equivalent replacement for this correction.
- **Upstream PR:** None for the fork-only strict-contract correction after checked 2026-08-21. Related: #85804, #88336, and #91604 are open and not safely backportable as replacements for HERMES-028.
- **Regression:** `scripts/run_tests.sh tests/hermes_cli/test_gateway_restart_loop.py tests/tools/test_terminal_heredoc_background_guard.py -q`; direct guard readback of all LPG and Brianle `bin/check`, `bin/setup`, and `bin/doctor` wrappers; and exact execution of both `./bin/check --fast` gates.
- **Rollback:** Remove `_ShellToken`, `_lex_shell_line`, quote-aware substitution detection, the `case`/`[[ ... ]]` token-state handling, `partition_heredoc_bodies`, its recursive lifecycle integration, and the associated shell-grammar/heredoc tests. Restore the preceding `_iter_command_segments` implementation while preserving HERMES-028 remote bounded reads, trusted system-shell handling, all unresolved and unreadable-script fail-closed behavior, HERMES-041 lifecycle verb protection, the existing inert-heredoc background guard, and unrelated scanner hardening.
- **Retirement:** Retire the shell-grammar correction only after released upstream distinguishes equivalent inert Bash grammar from executable positions while retaining HERMES-028-equivalent fail-closed unresolved executable references and passing both the benign-wrapper and adversarial regressions. Retire all of HERMES-028 only after released upstream also performs equivalent bounded remote script inspection without adding runtime dependencies beyond the supported remote shell contract.
...
- **Regression:** `scripts/run_tests.sh tests/agent/test_title_generator.py tests/agent/test_gemini_native_adapter.py tests/test_hermes_state.py tests/hermes_cli/test_aux_config.py -q` plus a live Google-fallback replay proving native JSON-schema delivery, a non-truncated finish, and a complete title. Focused title tests require explicit hard limits, omitted profile character limits, instruction-only stylistic targets, canonical brand case/punctuation restoration, and word-boundary/grapheme-safe safety truncation.
...
- **Summary:** Opt-in semantic Telegram topic icons support two explicit providers. `telegram_default` preserves the live Bot API allowlist behavior. `telegram_custom_packs` loads only configured custom packs through Bot API, resolves Unicode selectors to typed Apple-pack document IDs, and uses a profile-scoped Telethon 1.44.0 user transport limited to `topic.read` and `topic.icon.write`. The native path verifies the expected Premium user, exact bot peer, capability, binding, manual ownership, and exact post-write document ID before recording automatic ownership or recent history. It has no generic RPC method, message access, update listener, or default-provider fallback. Exact overrides remain portable Unicode, model options are bounded for large catalogs, and title delivery remains independent when native icon transport is unavailable. The `hermes telegram` namespace provides setup, local/live status and doctor, attended QR plus private-2FA authorization, bounded peer/capability inspection, effective catalog and resolution, exact topic verification, and one `--yes`-gated manual icon mutation.
- **Surfaces:** `agent/title_generator.py`; `agent/agent_init.py`; `run_agent.py`; `gateway/platforms/base.py`; `gateway/run.py`; `hermes_state.py`; `hermes_cli/main.py`; `hermes_cli/telegram.py`; `hermes_cli/telegram_user.py`; `hermes_cli/telegram_topic_icons.py`; `plugins/platforms/telegram/adapter.py`; `plugins/platforms/telegram/user_transport.py`; `plugins/platforms/telegram/mtproto_telethon.py`; `plugins/platforms/telegram/topic_icons.py`; `plugins/platforms/telegram/topic_icon_service.py`; `plugins/platforms/telegram/topic_icon_gateway.py`; `plugins/platforms/telegram/topic_icon_runtime.py`; `pyproject.toml`; `uv.lock`; `cli-config.yaml.example`; Telegram messaging documentation; focused CLI, policy, transport, catalog, service, state, title, delivery, stream-observer, and gateway tests.
- **Upstream tracking:** Independent 2026-08-15 incident hypothesis was frozen before searching upstream: the LLM icon chooser can return `None` after provider failure or an out-of-allowlist response, while the gateway silently performs a title-only rename. The correction belongs in the selector boundary: a deterministic allowed-list fallback plus warning-level reason logging, while preserving manual-icon ownership and title independence. Runtime evidence on 2026-08-20 additionally showed that completion-only title scheduling leaves long-running topics generic despite useful visible assistant prose; the bounded single-claim interim path belongs at the gateway turn boundary. Production evidence on 2026-08-22 showed the one-shot 20-second timer misses text that becomes sufficient only after its sample and that a deleted private topic can leave a completed response undelivered while a later title callback still attempts to rename the dead topic. The corrections belong at a two-condition gateway latch and the Telegram final-delivery boundary. Current upstream `main` at `13f4cfebfafbce8ac9d1bf29f66731858ed638b5` and release `v2026.8.19` still title from the user message at turn start and have no interim-response latch. Merged PR `#3390` supplies root fallback only for ordinary invalid thread IDs; merged PR `#50990` prunes confirmed stale DM-topic bindings. Merged PR `#32270` establishes the contrary containment policy for named private topics by recreating or refreshing them instead of falling back to root, and current delivery still calls `ensure_dm_topic(..., force_create=True)`. Closed-unmerged PR `#51011` proposed private-topic root recovery and was rejected specifically for violating that containment invariant. Open PR `#51085` only extends stale-binding pruning to media/anchor fallback; issue `#75789` tracks ambiguous recovery routing. None provides the complete interim-latch, final-response recovery, and rename-suppression contract. The no-recreation plus explicit root-recovery behavior is an intentional fork divergence chosen by Brian, not an upstream-compatible default.
...
- **Regression:** `scripts/run_tests.sh tests/hermes_cli/test_telegram_namespace.py tests/hermes_cli/test_telegram_user.py tests/hermes_cli/test_telegram_topic_icons.py tests/gateway/test_telegram_user_policy.py tests/gateway/test_telegram_user_transport.py tests/gateway/test_telegram_custom_topic_icons.py tests/gateway/test_telegram_topic_icon_runtime.py tests/gateway/test_telegram_topic_icon_service.py tests/agent/test_title_generator.py tests/gateway/test_session_title_rename_lane.py tests/gateway/test_telegram_stale_topic_recovery.py tests/gateway/test_telegram_topic_mode.py tests/test_hermes_state.py tests/test_telegram_topic_status_ptb.py tests/hermes_cli/test_backup.py tests/test_project_metadata.py -q`. Focused gateway tests require both eligibility orderings, one title claim, stale-topic root recovery only for a completed final response, and rename suppression after stale-topic confirmation. Live release proof requires attended identity authorization, exact disposable topic mutation/readback/cleanup, and two unique cases per configured Apple pack with zero default IDs or readback mismatches.
- **Activation:** Source and disposable live QA are verified but not promoted. The active gateway must not be restarted or configured for `telegram_custom_packs` until separate runtime-promotion approval. Attended login uses a local QR plus private 2FA, and ordinary backups exclude `state/telegram-user` session credentials.
- **Rollback:** Set `user_transport.enabled: false` and `auto_topic_icons: false`, or restore `topic_icon_provider: telegram_default`. Disabling the user transport must stop MTProto calls without affecting Bot API messaging or title delivery. To roll back only the 2026-08-22 correction, remove the interim eligibility latch and streamed-text observer, restore the one-shot timer, remove stale-topic final-response root recovery and rename suppression, and delete only their focused regressions while preserving stale-binding pruning and ordinary thread fallback. Revoke with `hermes telegram user logout` if credential containment is required. Full source retirement removes only the optional dependency, Telegram CLI namespace, user transport, custom provider, docs, and focused tests while preserving existing default-provider selection, state, and title behavior.
...
- **Summary:** Treats any inherited gateway marker as tainted for self-control and blocks executed gateway lifecycle actions, destructive launchctl verbs, and hidden executable shell actions from gateway-origin terminal execution. Live terminal inspection classifies actual argv and executable shell payloads while allowing benign test runners, dynamic interpreter variables, and lifecycle vocabulary used only as data; broader fail-closed referenced-script inspection remains available for future cron specifications.
- **Surfaces:** `hermes_cli/gateway.py`; `cron/lifecycle_guard.py`; `tools/terminal_tool.py`; `tests/cron/test_gateway_lifecycle_guard_launchctl.py`; `tests/hermes_cli/test_gateway_restart_loop.py`.
