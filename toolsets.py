...
_HERMES_CORE_TOOLS = [
    ...
    "session_search",
    "clarify",
    "execute_code", "delegate_task", "review_changes",
    "cronjob_manage",
...
def _bundle(description, extras=()):
    ...
    return _ts(description, _HERMES_INTERACTIVE_TOOLS + list(extras))
...
TOOLSETS = {
    ...
    "goal": _ts("Inspect and control a persistent per-session goal", ["set_goal"]),
    "skill_management": _ts("Create, edit, and manage shared skill documents", ["skill_manage"]),
    "messaging": _ts("Native outbound send_message. Default-off. Cron requires allow_messaging=true.", ["send_message"]),
    ...
    "review": _ts("Dispatch candidate-bound independent review", ["review_changes"]),
    ...
    # Full Hermes toolsets (CLI + messaging platforms). Outbound messaging is
    # separately opt-in, never part of core. hermes-acp is the
    ...
    # Noninteractive core only. `hermes tools` platform config filters it down and
    # _get_platform_tools() drops _DEFAULT_OFF_TOOLSETS unless user-enabled.
    "hermes-cron": _ts("Default cron toolset - core tools gated by `hermes tools`", _HERMES_CORE_TOOLS),
    ...
    "hermes-yuanbao": {
        ...
        "tools": _HERMES_INTERACTIVE_TOOLS + _YUANBAO_TOOLS,
