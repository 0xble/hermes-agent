...
class GatewayStatusCommandsMixin:
    ...
    async def _handle_context_command(self, event: MessageEvent) -> str:
        ...
        if used > 0 and context_length > 0:
            ...
            breakdown = await asyncio.to_thread(self._context_breakdown_block, agent, event, source, expanded) if agent else []
    ...
    def _context_breakdown_block(self, agent, event, source, expanded: bool) -> list[str]:
        ...
        try:
            ...
            try:
                payload = self._session_context_breakdown(agent, event, source)
            except TranscriptReadError:
    ...
    def _session_context_breakdown(self, agent, event, source) -> dict:
    ...
    def _context_breakdown_lines(self, agent, event, source) -> list[str]:
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        if agent and hasattr(agent, "session_total_tokens") and agent.session_api_calls > 0:
            ...
            breakdown_lines = await asyncio.to_thread(self._context_breakdown_lines, agent, event, source)
            if breakdown_lines:
