        ...
        try:
            ...
            compressed, _ = await self._run_in_executor_with_context(
                lambda: tmp_agent._compress_context(
                    ...
                    # Scope the dedup reset to the LIVE session row id — the same task_id the
                    # main gateway turn passes to run_conversation. Without it the reset falls
                    # back to the "default" task and a post-compression skill_view can return a
                    # dedup stub pointing at pruned context (#98206).
