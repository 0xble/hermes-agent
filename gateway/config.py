...
def _normalize_restart_resume_policy(value: Any, *, allow_none: bool, key: str) -> Optional[str]:
    ...
    Unlike ``_normalize_choice``, an unrecognized value RAISES instead of falling back to a
    default: a typo'd resume policy must fail loudly at config construction, not silently
    resume with the opposite behavior after a restart.
    """
...
class GatewayConfig:
    ...
    # Empty-message restart recovery policy for adapters that can either ask the user or continue
    # autonomously. None preserves each adapter's native safety default. Per-platform
    # ``extra.restart_resume_policy`` wins.
    ...
    _SCALAR_DICT_FIELDS = (
        "write_sessions_json", "always_log_local", "restart_resume_policy",
        "filter_silence_narration", "stt_enabled",
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        return cls(
            ...
            **{name: _coerce_bool(data.get(name), default) for name, default in _TOPLEVEL_BOOL_DEFAULTS.items()},
            ...
            # Raw: ``__post_init__`` validates and raises on a typo rather than defaulting.
            # ``pick`` supplies the fork's top-level-wins-over-``gateway.``-nested lookup.
