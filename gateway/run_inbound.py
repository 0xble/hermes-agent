...
class GatewayInboundMixin:
    ...
    async def _hm_handle_running_session_message(
        ...
        if self._draining:
            ...
            if queue_during_drain:
                # Durable, not the in-memory pending slot: this process is being replaced.
                queue_during_drain = await self._persist_restart_inbox_event(_quick_key, event)
    ...
    def _quick_command_alias_text(
        ...
        """Return a configured alias expansion without mutating the event."""
    ...
    def _prepare_reasoning_turn_event(self, event: "MessageEvent"):
        ...
        if request.prompt.lstrip().startswith("/"):
            raise ReasoningTurnError("One-turn reasoning cannot wrap another slash command.")
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        # Not every event exposes get_command() (estop/synthetic events); a hard attribute
        # access here raises before the paused-reply path is ever reached.
        _get_reasoning_command = getattr(event, "get_command", None)
        reasoning_command = _get_reasoning_command() if callable(_get_reasoning_command) else None
        reasoning_definition = _resolve_command(reasoning_command) if reasoning_command else None
        if reasoning_definition and reasoning_definition.name == "reasoning":
            denied = self._check_slash_access(source, reasoning_definition.name)
    ...
    def _prepend_inbound_document_notes(cls, event: MessageEvent, message_text: str) -> str:
        ...
        for i, path in enumerate(event.media_urls):
            ...
            # Probe the cached file: a note for a file we cannot find or open must say so
            # rather than let the agent claim it read the attachment.
