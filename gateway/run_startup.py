...
class GatewayStartupMixin:
    ...
    async def _finish_startup_restore(self) -> None:
        ...
        if durable_drained:
            logger.info(
                "Dispatched %d durable restart-drain inbound message(s)", durable_drained)
        if drained:
            ...
            with _log_suppressed(logging.DEBUG, "delivery ledger update failed", exc_info=True):
                if result is not None and getattr(result, "success", False):
                    ...
                    if (
                        ...
                        and getattr(adapter, "_send_path_recovery_generation", 0) > recovery_generation
                    ):
                        # Re-enter ONLY when polling proved a newer recovery during this attempt.
                        # Reusing the same health edge burns the full retry budget in one degraded
                        # sweep and strands the obligation.
    ...
    def _schedule_deferred_obligation_redelivery(
        self, platform: Platform, *, profile: Optional[str] = None, delay: float,
    ) -> None:
        """Re-drive this identity's failed obligations once a wait elapses.

        The reconnect sweep cannot help a flood-controlled send: nothing disconnects, so no
        ...
        key = "%s:%s" % (getattr(platform, "value", platform), profile or "default")
        delay = max(0.0, float(delay))
        # Bound the wait so a hostile or garbled retry_after cannot park an answer
        # indefinitely; the ledger's staleness/attempt caps still apply.
        try:
        ...
        async def _run_after_delay() -> None:
            try:
                ...
                logger.debug("deferred delivery-ledger sweep failed for %s", key, exc_info=True)
            finally:
