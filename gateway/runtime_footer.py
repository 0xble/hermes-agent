...
def _resolved_cwd(cwd: Optional[str]) -> str:
    """Resolve the footer's cwd. ``None`` means "work it out"; an explicit ``""`` means the
    caller has none and must NOT be silently replaced by the process-global env var, which a
    cron job mutates out from under interactive turns."""
    if cwd is not None:
        return cwd
    try:
        from agent.runtime_cwd import resolve_tool_cwd

        # Empty means nothing is bound for this context (``clear_session_cwd`` binds "" rather
        # than unsetting), so the env is still the next authority.
        if resolved := resolve_tool_cwd():
            return resolved
    except Exception:
        pass
    try:
...
def format_runtime_footer(*, model: Optional[str], context_tokens: int,
    ...
    renderers = {
        ...
        "cwd": lambda: _home_relative_cwd(_resolved_cwd(cwd)),
    }
