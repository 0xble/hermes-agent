...
class GatewayModelCommandsMixin:
    ...
    def _live_agent_for_session_control(self, session_key: str):
        """The running agent a request-boundary control may update, else None. The pending
        sentinel reserves the turn before construction: its build path consumes the session
        override, and it is never itself a mutable agent."""
    ...
    def _apply_reasoning_selection(
        ...
        *, source: SessionSource | None = None,
        ...
        def _activate(reasoning_config) -> None:
            """Replace (never mutate) the live agent's config so an in-flight API request keeps
            the snapshot it already owns; the next iteration reads this one."""
    ...
    def _apply_fast_selection(self, session_key: str, value: str, persist: bool = False) -> str:
        ...
        if live_agent is not None:
            ...
            selection_model = str(getattr(live_agent, "model", "") or "")
            request_overrides = dict(getattr(live_agent, "request_overrides", {}) or {})
            request_overrides.pop("service_tier", None)
            request_overrides.pop("speed", None)
            if tier:
                request_overrides.update(resolve_fast_mode_overrides(selection_model) or {})
            live_agent.service_tier = tier
            # Copy-on-write: an already-built API request keeps its snapshot; the next model
            # iteration reads this replacement.
            live_agent.request_overrides = request_overrides
