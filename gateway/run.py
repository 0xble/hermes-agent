...
def build_resume_recovery_note(
    ...
    ``restart_resume_policy`` is the resolved HERMES-075 policy. When omitted it falls back to the
    adapter-derived default: non-interactive platforms (webhook, API server — adapters with
    ``interactive_resume = False``) continue, everything else asks. The continue guidance is
    deliberately platform-neutral: an interactive platform can now opt into ``continue``, so the
    wording must not claim the platform is non-interactive (#57056).
    """
    reason_phrase = (
...
# Immutable gateway baseline.  Cron jobs temporarily mutate process-global TERMINAL_CWD for
# legacy tool consumers, so interactive turns must never read that variable live.  Snapshot
# taken after config/placeholder resolution and before the scheduler can run (#81451).
...
def _build_document_context_note(
    ...
    resolution_status: str = "resolved") -> str:
    ...
    ``resolution_status`` is deliberately small and machine-testable: ``resolved``, ``absent`` or
    ``inaccessible``. A cached file the gateway could not find or open is reported as such so the
    agent never claims to have read it."""
    if resolution_status == "absent":
        return (
            f"[The user sent a document: '{display_name}', but its cached file could not be "
            f"found at the expected path: {agent_path}. Attachment resolution failed; do not "
            "claim to have read it.]")
    if resolution_status == "inaccessible":
        return (
            f"[The user sent a document: '{display_name}', but its cached file is inaccessible "
            f"at: {agent_path}. Attachment resolution failed; do not claim to have read it.]")
...
def _load_gateway_runtime_config(config: dict | None = None) -> dict:
    """Load gateway config for runtime reads, expanding supported ``${VAR}`` and model-preset refs.

    ``config`` is the raw per-turn config passed by gateway callers. Expansion stays at this runtime-read
    boundary so authoring loaders retain named references while every primary-route consumer sees inline fields.
...
def _format_gateway_process_notification(evt: dict) -> "str | None":
    ...
    if evt_type in {
        ...
        # Every model-facing process notification stays on the ONE shared formatter, so a
        # watch event carries the same silence contract a completion does.
        from tools.process_registry import format_process_notification
        return format_process_notification(
            evt,
            include_no_reply_contract=evt_type != "async_delegation",
            # Preserve the gateway watch text exactly: the pre-refactor watch formatter
            # never exposed subagent ownership attribution.
...
class GatewayRunner(
    ...
    async def _clear_resume_pending_for_claimed_obligations(
        ...
        """Discharge recovery only with the matching agent-final obligation."""
        sendable = []
        for row in claimed:
    ...
    def _init_registries_and_clocks(self) -> None:
        ...
        # Per-adapter-identity deferred delivery-ledger sweeps (flood-control waits),
        # coalesced so one penalty window schedules one sweep.
        # Progress-edit throttle shared by every session in one chat. Telegram rate-limits per
        # CHAT while the progress consumer runs per SESSION, and DM topics all share one chat_id
        # — on per-session clocks N concurrent sessions multiply the edit rate by N and trip
        # flood control. Bare monotonic floats, keyed "platform:chat_id"; trimmed
        # opportunistically so a long-lived gateway cannot accumulate an entry per chat forever.
    ...
    def _thread_metadata_for_source(
        ...
        if getattr(source, "platform", None) == Platform.TELEGRAM:
            # Every reply on a Business conversation must carry its connection id, or the
            # send lands on the bot's own chat instead of the Business account's.
    ...
    def _get_executor(self) -> concurrent.futures.ThreadPoolExecutor:
        ...
        with lock:
            ...
            if executor is None or getattr(executor, "_shutdown", False):
                # An agent turn occupies a worker for its full lifetime. At ten workers
