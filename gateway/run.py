...
def _load_gateway_runtime_config(config: dict | None = None) -> dict:
    """Runtime route expansion; the shared loader already owns env/managed expansion.

    Keep this boundary for callers supplying an explicit per-turn config and let invalid
    named routes raise. Never re-expand env refs in an already-effective managed layer.
    """
