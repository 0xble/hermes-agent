...
class GatewayStreamConsumer(StreamTransportMixin, StreamFallbackMixin, StreamThinkFilterMixin):
    ...
    def __init__(
        ...
        # persistent preview pauses later tool updates; durable confirmation commits
        # the split, while retraction resumes the current progress bubble. Native
    ...
    def _notify_content_boundary(self, source: DurableContentSource, *,
        ...
        self._publish_content_boundary(DurableContentBoundary(
            boundary_id=boundary_id, source=source, message_id=normalized_message_id))
    ...
    def _open_preview_boundary(self) -> None:
        ...
        event = ProvisionalContentBoundary(
            boundary_id=f"{self._turn_id}:preview:{self._content_boundary_sequence}")
    ...
    def _confirm_pending_preview_boundary(
        ...
        self._publish_content_boundary(DurableContentBoundary(
            boundary_id=pending.boundary_id, source=source,
            message_id=(str(message_id) if message_id is not None else pending.message_id)))

    def _confirm_or_notify_content_boundary(self, source: DurableContentSource, *,
                                            message_id: Optional[str] = None) -> None:
        """Resolve the active preview, otherwise publish a direct boundary."""
        if self._pending_preview_boundary is not None:
            self._confirm_pending_preview_boundary(source=source, message_id=message_id)
        else:
    ...
    def on_delta(self, text: str) -> None:
        ...
        if text:
            # Open synchronously on the agent thread: the async consumer may not reach
            # adapter.send() until after the next tool event under platform latency.
