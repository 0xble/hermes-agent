...
class StreamTransportMixin:
    ...
    async def _delete_previews(self, stale_ids, *, skip=None, label: str,
        ...
        Returns False when any delete failed, so a caller can tell whether the preview is
        still a persistent chat entry (silence-marker retraction)."""
    ...
    async def _first_send(self, text: str, *, finalize: bool) -> bool:
        ...
        if getattr(self, "_egress_declined", False):
            ...
            # so the check belongs here rather than at each caller. Checked BEFORE the
            # boundary is opened: a suppressed send must not leave one parked.
            logger.warning(
        ...
        if finalize:
            ...
            # A successful send without an editable id cannot be retracted, so it is
            # already a durable timeline entry.
            self._confirm_pending_preview_boundary(
    ...
    async def _edit_existing(self, text: str, *, finalize: bool, is_turn_final: bool) -> bool:
        ...
        if ((getattr(result, "continuation_message_ids", ()) or ())
            ...
            # A new visible message: tool progress must linearize below it.
