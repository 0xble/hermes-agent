...
class GatewaySlashCommandsMixin(
    ...
    def _gateway_command_cwd(self) -> str:
        """The IMMUTABLE gateway cwd for pre-session commands. Never the live TERMINAL_CWD: a
        cron job mutates that process-global for legacy tool consumers."""
    ...
    async def _session_entry_for_event(
        ...
        if force_new:
            return await self.async_session_store.get_or_create_session(
                effective_source, force_new=True)
    ...
    def _background_reply_anchor(self, event: MessageEvent, source) -> "str | None":
        """Return a reply anchor only when it belongs to the NORMALIZED route. A Telegram
        topic command arrives thread-stripped; anchoring the reply to the raw message would
        pin the answer to the wrong thread."""
        ...
        if raw_thread != normalized_thread:
            return None
        return self._reply_anchor_for_event(event)
    ...
    async def _handle_background_command(self, event: MessageEvent) -> str:
        ...
        # Recover the topic a stripped Telegram topic command lost, so the background
        # session and its reply land on the route the user actually typed in.
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        try:
            launch = launch_native_update(
                home=_hermes_home, hermes_cmd=hermes_cmd, pending=pending, spawn=_spawn_detached_update)
        except Exception as e:
