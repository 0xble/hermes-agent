...
def _is_api_interrupt_sentinel(result: Dict[str, Any], text: Any) -> bool:
    """Whether *text* is Hermes' internal "waiting for model" interrupt status (HERMES-007)."""
    return (
        bool(result.get("interrupted")) and isinstance(text, str)
        and text.strip().startswith(INTERRUPT_WAITING_FOR_MODEL_PREFIX))


def _api_final_response_text(result: Dict[str, Any]) -> str:
    """User-facing assistant text, keeping an interrupt sentinel as metadata instead of content."""
...
def _api_interrupt_flags(result: Any) -> tuple:
    """``(completed, interrupted)`` for an agent result; an interrupted turn is not completed."""
    if not isinstance(result, dict):
        return True, False
    interrupted = bool(result.get("interrupted"))
    return bool(result.get("completed", not interrupted)), interrupted
...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Freeze the gateway baseline before an in-process workdir cron can temporarily
        # replace process-global TERMINAL_CWD (#81451).
        ...
        # Per-process secret proving a wake/notification request came from Hermes itself, not a
        # client: the turn is then marked internal and its user message persisted as such.
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        final_response = _api_final_response_text(result) if is_dict else ""
        completed, interrupted = _api_interrupt_flags(result)
        headers = self._session_headers(effective_session_id or session_id, gateway_session_key)
        return web.json_response(
            {"object": "hermes.session.chat.completion",
             "session_id": effective_session_id or session_id,
             "message": {"role": "assistant", "content": final_response},
             "completed": completed, "interrupted": interrupted, "usage": usage,
        ...
        async def _run_and_signal() -> None:
            try:
                ...
                await queue.put(_event_payload("assistant.completed", {
                    ...
                    "content": final_response, "completed": completed,
                ...
                completed_payload = {
                    "session_id": effective_session_id, "message_id": message_id, "completed": completed,
                    "interrupted": interrupted, "messages": turn_messages, "usage": usage,
    ...
    def _cron_error_response(exc: BaseException) -> "web.Response":
        """A cron ValueError is caller input (bad schedule, unknown IANA timezone) — 400, not a
        500 that reads as a server fault the client cannot fix."""
        status = 400 if isinstance(exc, ValueError) else 500
        return web.json_response({"error": _redact_api_error_text(exc)}, status=status)
    ...
    def _bind_api_server_session(
        self, *, chat_id: str = "", session_key: str = "", session_id: str = "", profile: str = "",
        ...
        return set_session_vars(
            ...
            # Frozen startup cwd: a cron job mutating TERMINAL_CWD must not redirect
            # an API request's tools.
    ...
    async def _run_agent(
        ...
        internal_notification: bool = False, session_history_delivery: str = "", turn_author: Optional[Dict[str, Any]] = None,
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    agent._current_turn_is_internal = bool(internal_notification)
                    ...
                    if internal_notification:
                        conversation_kwargs["persist_user_display_kind"] = "internal_notification"
                    ...
                    conversation_kwargs = dict(**conversation_kwargs,
                    )
                    if relay_metadata:
