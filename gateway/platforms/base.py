...
class MessageEvent:
    ...
    # Explicit reasoning override consumed by this event's turn only, plus the notice echoed to the user.
    turn_reasoning_config: Optional[Dict[str, Any]] = None
    turn_reasoning_notice: Optional[str] = None
...
class BasePlatformAdapter(ABC):
    ...
    async def _finalize_delivery_obligation(
        ...
        """Mark the ledger row delivered/failed (best-effort). A ``flood_control:<wait>`` rejection is
        retryable but not yet — park a coalesced sweep for when the server's stated wait elapses, else
        the completed answer stays terminally failed with attempts=0 for the life of the process. On
        ``send_path_degraded`` with a replacement adapter live, trigger another redelivery sweep (the
        watcher's may have run before this failure landed; atomic claiming keeps it idempotent), or wake
        the ledger in place when the same adapter recovered without being replaced."""
        try:
            from gateway.delivery_ledger import mark_delivered, mark_failed, parse_flood_retry_after
            ...
            flood_wait = parse_flood_retry_after(error)
