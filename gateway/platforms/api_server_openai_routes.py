...
class OpenAICompatRoutesMixin:
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # X-Hermes-Internal-Notification carries this process's own token: a wake / delegation
        # notification Hermes sent itself, never a client turn. Constant-time compare.
        ...
        internal_notification = bool(_internal_token) and hmac.compare_digest(
            request.headers.get("X-Hermes-Internal-Notification", "").strip(), _internal_token)
        ...
        run_kwargs = dict(
            ...
            gateway_session_key=gateway_session_key, internal_notification=internal_notification,
    ...
    def _turn_transcript_messages(
        ...
        if not isinstance(agent_messages, list) or not agent_messages:
            return []
        from gateway.platforms.api_server import _is_api_interrupt_sentinel
        ...
        for msg in agent_messages[start:]:
            if not isinstance(msg, dict) or msg.get("role") not in {"assistant", "tool"}:
                continue
            # HERMES-007: the internal "waiting for model" status is interrupt metadata, never assistant
            # content — it must not reach a client through the transcript either.
