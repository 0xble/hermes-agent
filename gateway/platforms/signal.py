...
class SignalAdapter(BasePlatformAdapter):
    ...
    async def _send_attachment_batch(self, scheduler, params: Dict[str, Any], n: int, label: str) -> bool:
        """Send one attachment batch with rate-limit pacing and a single transient retry; True when the
        server validated the send. Tokens are deducted only on validated success (None = server never
        accepted it); 429s feed the scheduler."""
