...
class GatewayBusySessionMixin:
    ...
    async def _persist_restart_inbox_event(self, session_key: str, event: MessageEvent) -> bool:
        """Durably accept drain-time inbound BEFORE promising the user it is queued. The
        in-memory pending slot dies with this process; the restart inbox survives the
        replacement gateway's boot."""
        try:
            from gateway.restart_inbox import record_event

            adapter = self._adapter_for_source(event.source)
            await asyncio.to_thread(
                record_event, session_key, event, getattr(adapter, "_owner_profile", None))
            return True
        except Exception:
            logger.exception(
                "Could not persist restart-drain inbound for %s; rejecting instead", session_key)
            return False
    ...
    async def _send_busy_drain_notice(self, event: MessageEvent, session_key: str, effective_mode: str) -> None:
        ...
        if not adapter:
            return
        queued = self._queue_during_drain_enabled(effective_mode)
        if queued:
            # Durable, not the in-memory slot: this process is going away.
            queued = await self._persist_restart_inbox_event(session_key, event)
        message = self._build_drain_busy_reply(queued=queued)
        await self._send_busy_reply(event, adapter, str(message))
    ...
    # /reasoning, /fast and /title carry ``busy_policy="dispatch"``: they take effect from the
    # next request boundary without touching the in-flight snapshot, so they must be reachable
    # from the busy path too — otherwise dispatch falls through to the busy-reject text.
    _PLAIN_COMMANDS = (
        "status", "context", "restart", "approve", "deny", "pause", "agents", "bg", "btw",
        "kanban", "subgoal", "heartbeat", "busy", "yolo", "verbose", "reasoning", "fast",
        "footer", "help", "commands", "profile", "wisdom", "login", "update", "version", "title",
    )
    # Dispatched only on the idle path (busy dispatch has its own allowlist).
    _IDLE_COMMANDS = (
        "topic", "whoami", "platform", "stop", "memory", "skills",
    ...
    async def _busy_goal_command(self, event: MessageEvent, quick_key: str, source):
        ...
        if is_goal_control(_goal_arg):
            # Activation fence: a mid-turn control mutation bumps the persisted revision so the
            # in-flight turn's goal activation is invalidated instead of racing the new state.
            # Read-only control verbs (status/show/gate list) deliberately do NOT fence.
            _goal_mutates = (
