...
class SessionSource:
    ...
    # Telegram Business connection that owns this conversation. A routing discriminator,
    # not an authorization claim. Kept last to preserve positional construction
    # compatibility for older downstream integrations.
    ...
    _OPTIONAL_POST_SCOPE = ("parent_chat_id", "message_id", "business_connection_id", "profile")
...
class SessionEntry:
    ...
    # Token of the interrupted turn whose final delivery may discharge this
    # recovery obligation.  It is intentionally distinct from the live active
    # marker: startup promotion clears active_turn_token after copying it here.
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result["resume_turn_token"] = self.resume_turn_token
...
def build_session_key(
    ...
    Layout: ``<ns>:<platform>:<chat_type>[:<slack scope_id>][:business:<id>][:<chat_id>]``
    ``[:<thread_id>][:<user>]``.
    ...
    # Telegram Business: two Business conversations can present the same chat_id, so the
    # connection id must isolate them or they collide on one session key.
    if source.platform == Platform.TELEGRAM and getattr(source, "business_connection_id", None):
        parts.extend(("business", str(source.business_connection_id)))
    if chat_id:
