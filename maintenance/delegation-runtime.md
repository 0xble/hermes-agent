...
- Fully recovery-exhausted, positively terminal (including `budget_exhausted`) and provably unlabelled notifications settle as dropped, never delivered, under existing seven-day/count history retention. Card obligations, unknown/malformed records, held claims and unused recovery budgets remain retained. Recoverable work remains intentionally uncapped. Regression: `tests/tools/test_async_completion_retention.py`. This does not promise a global database-size bound or recover already expired history after rollback.

- Durable primary, fallback and MoA launch metadata exposes only URL origins (scheme, host and port), known numeric request limits, and full normalized URL/override authority fingerprints. Userinfo, arbitrary route paths, query values and opaque overrides remain runtime-only. Fingerprint-free legacy resumes retain strict historical path-bearing comparisons through trusted reauthorization, then sanitize the next snapshot and fingerprint the current authorized full route. They do not establish retrospective proof of previously redacted URL components. Same-origin path changes remain distinct authority. Regressions: `tests/tools/test_delegate_launch_authority.py`, `tests/tools/test_delegation_resume_route_identity.py`, `tests/tools/test_delegation_primary_pool_resume.py`, `tests/tools/test_delegation_custom_fallback_resume.py`, and `tests/agent/test_delegation_frozen_runtime.py`. Read-only resume does not rewrite historical rows.
...
- **Gateway schema refresh and detached history:** Changes to named-role definitions, inherited delegation defaults, preset allowlists and schema limits invalidate the cached parent on its next turn; unchanged configuration reuses it. Typed mapping keys and structurally tagged containers make nested cache signatures order-stable without mapping/sequence collisions. Invalid registry diagnostics remain authoritative. Rebuilds deep-copy the current conversation history before closing the cached agent, preserving tool-call/result continuity without aliasing the old agent's mutable messages. In-flight schemas and existing children's frozen routes/history remain unchanged. Surfaces: `gateway/run.py`, `gateway/run_agent_cache.py`, `gateway/run_turn_runner.py`. Regression: `tests/gateway/test_delegation_schema_refresh.py`, `tests/gateway/test_agent_cache.py`, `tests/gateway/test_agent_signature_containers.py`. Roll back these cache/rebuild changes together without rewriting persisted history or altering running children; retire with HERMES-108 only when released upstream preserves these boundaries. Source-only; no runtime activation.
