...
- **Rotation trigger is not the failure name (2026-09-18):** `_transient_credential_retry_reason` flattens overloaded / server_error / timeout into ONE rotation trigger, and `classify_api_error` puts a connection-refused error in the same `timeout` bucket. It therefore decides only WHETHER to alternate; the ordered `_FALLBACK_REASONS` predicates name the failure. They resolve a label for exactly the timeout and connection shapes and deliberately leave a genuine capacity error unnamed, which is what still reads "provider overloaded". Without that split every full-budget timeout and every unreachable endpoint logged as an overloaded provider, which is the confusion upstream #89445 exists to remove: a slow local model looked like a dead endpoint and the deadline knob never reached the log.
- **Surfaces:** `agent/auxiliary_client.py`; `tests/agent/test_auxiliary_client.py`; `tests/agent/test_auxiliary_title_timeout_bound.py`.
- **Upstream tracking:** Local fork behavior; upstream equivalence has not yet been established. Upstream `f85cd964e17` (2026-09-17) bounds auto-title timeouts to one window and requires them to be NAMED as timeouts; that contract is adopted and pinned by `tests/agent/test_auxiliary_title_timeout_bound.py`.
- **Upstream PR:** None.
- **Regression:** `pytest -q tests/agent/test_auxiliary_client.py -k 'AuxiliaryTransientCredentialRetry or AuxiliaryOverloadFallback'`; `scripts/run_tests.sh tests/agent/test_auxiliary_title_timeout_bound.py`.
...
- **Read-only loader scope, narrowed 2026-09-18:** `read_pool_snapshot` reads the ACTIVE store only. Upstream [PR #111724](https://github.com/NousResearch/hermes-agent/pull/111724), commit `93889b770da3dc641c2ed55eecffef90da9c358f`, made named profiles independent islands and deleted the global-root resolver (`hermes_cli.auth._global_auth_file_path`) the loader's root fallback called. A profile holding no rows of its own therefore reports no accounts instead of inspecting, and reporting quota for, the root profile's. The byte-preserving no-write invariant is unchanged. The same integration dropped `load_pool_read_only` and `read_pool_snapshot` from `agent/credential_pool.py` entirely, which made `hermes_cli/auth_quota.py` unimportable; both are restored.
