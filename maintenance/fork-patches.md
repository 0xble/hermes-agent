...
| Camofox accounts and vault | browser account/vault tests and `tests/agent/test_vault_connect.py` | Account aliases are profile-specific. Adopted source: [upstream PR 114414](https://github.com/NousResearch/hermes-agent/pull/114414), open at `d8a374630aef825ad3d86c1e41defa57a4874247` on 2026-09-19. Fork adaptation is delivered; upstream merge is separate. Shadow-DOM login forms (web-component inputs such as AA's `adc-text-input`): adopted [upstream PR 109425](https://github.com/NousResearch/hermes-agent/pull/109425) at `988a691a1d122561dc3809840357090b27612287`, open on 2026-09-19; proof `tests/tools/test_vault_shadow_dom_live.py` (real headless Chrome). Rollback: revert the single fork commit; it touches only `agent/vault_login_classifier.py`, `tools/browser_vault_tool.py`, and vault tests. Service-account `op item get` requires `--vault`: adopted [upstream PR 109456](https://github.com/NousResearch/hermes-agent/pull/109456) at `90a17768fd85cc40153a2dbcc2d8f21f2f6dc748`, open on 2026-09-19; the Connect read path is preserved ahead of the CLI selector; proof `tests/agent/test_vault_onepassword_selector.py`. |
...
| Telegram topic titles and icons | `tests/agent/test_title_generator.py`, `tests/agent/test_topic_icons.py`, `tests/hermes_state/test_title_lineage.py`, gateway title/topic tests | Own fork adaptation sourced from [upstream PR 66353](https://github.com/NousResearch/hermes-agent/pull/66353) and the single-call idea in [PR 35737](https://github.com/NousResearch/hermes-agent/pull/35737); Bot API only, retire when a released tag provides equivalent behavior. |
...
| Quick-command aliases while busy | `tests/gateway/test_command_bypass_active_session.py` (quick-alias cases), `tests/gateway/test_session_race_guard.py` (quick-alias cases) | Local narrow patch: alias `quick_commands` (for example `s` → `/steer`) expand before the adapter and runner busy-session guards, per routed-profile snapshot; aliases never enter the /stop-/new-/reset cancellation handoff. Nearest upstream tracking is [issue 25783](https://github.com/NousResearch/hermes-agent/issues/25783) (exec type only; its PR 25804 closed unmerged). Retire when upstream expands aliases on the busy path. |
