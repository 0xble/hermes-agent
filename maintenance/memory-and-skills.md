...
- **Summary:** Preserves an explicit empty inner scope (`[[]]`) for text retention so Hindsight performs one shared consolidation pass instead of silently reverting to the combined default. The pinned 0.9.1 raw-file API has no observation-scope field, so attachments retain existing server behavior without changing bank policy.
...
- **Attachment tag parity:** File uploads merge configured retention tags with source tags through the same normalizer as text retains, so configured recall filters can find both. Native cache/discovery/provider tests with a fake SDK cover CSV/list normalization, deduplication, explicit empty defaults and automatic/explicit filtered recall. Attachment consent, verified bytes, metadata and source identities remain unchanged.
...
- **Upstream PR:** None after checked 2026-08-18.
- **Regression:** `scripts/run_tests.sh tests/agent/test_prompt_builder.py tests/agent/test_api_content_sidecar.py tests/agent/test_gateway_turn_sidecar.py tests/agent/test_turn_context.py tests/plugins/memory/test_hindsight_provider.py`. The curation correction is covered by `tests/plugins/memory/test_hindsight_provider.py -k TestMemoryCuration`. `scripts/run_tests.sh tests/plugins/memory/test_hindsight_recall_controls.py` verifies registered controls through the actual pinned SDK with a fake transport, filter-preserving expansion fallback, local slicing, and unchanged automatic recall defaults.
