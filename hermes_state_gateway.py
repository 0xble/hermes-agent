...
"""
# Peer recovery must return the gateway PARENT, never a lineage child. A spawn/branch/delegate
# child can carry the same peer tuple (and, for a spawn, the same session_key) as its parent, and
# it usually has the newer ``last_activity_at`` — so without this fence the ranking hands back the
# child and the peer resumes into a subordinate transcript. Mirrors the exclusion the orphan-donor
# queries already apply (see _ORPHAN_DONOR / _ORPHAN_LINEAGE_DONOR_SQL).
_PEER_LINEAGE_FENCE_SQL = """
                  AND json_extract(COALESCE(s.model_config, '{}'),
                                   '$._spawned_from') IS NULL
                  AND json_extract(COALESCE(s.model_config, '{}'),
...
_PEER_BY_KEY_SQL = f"""{_PEER_SELECT_HEAD}                WHERE s.session_key = ?
                  AND s.source = ?{_PEER_LINEAGE_FENCE_SQL}
                ...
                LIMIT 1
                """
_PEER_BY_TUPLE_SQL = f"""{_PEER_SELECT_HEAD}                WHERE s.source = ?{_PEER_LINEAGE_FENCE_SQL}
