...
from hermes_state_repair import (
    _backup_db_file, _claim_repair_attempt, _fsync_directory, preflight_db_writability,
        ...
        while True:
            ...
            try:
                with self._lock:
                    ...
                    # HERMES-079: SQLite's connection-level busy handler must not run its own
                    # deterministic wait inside BEGIN IMMEDIATE — that wait can outlive THIS
                    # operation's application patience budget before the jitter/deadline loop below
                    # gets control, which is what makes sub-second budgets (activity writes)
                    # ineffective and hides the lock-attribution error. Acquire the lock
                    # non-blocking, then restore the configured timeout for the transaction body.
                    _timeout_row = self._conn.execute("PRAGMA busy_timeout").fetchone()
                    _configured_timeout_ms = (
                        int(_timeout_row[0]) if _timeout_row and _timeout_row[0] is not None else 0)
                    _override_busy_timeout = _configured_timeout_ms != 0
                    if _override_busy_timeout:
                    ...
                    try:
                        ...
                        if _override_busy_timeout:
                            try:
                                self._conn.execute(f"PRAGMA busy_timeout={_configured_timeout_ms}")
                                ...
                                if _began:
                                    try:
                                        ...
                                        pass
                                raise RuntimeError(
                                    "failed to restore the SQLite busy timeout") from restore_exc
                    try:
                        fn_started = True
