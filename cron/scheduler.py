...
def _total_run_budget_error(
    job_name: str, budget_seconds: Optional[float],
) -> CronTotalRunBudgetExpired:
    budget = float(budget_seconds or 0.0)
...
def _total_run_budget_failure(
    job: dict, job_name: str, budget_seconds: Optional[float],
    ...
    alert = (
        ...
        f"after {float(budget_seconds or 0.0):g}s."
    )
    return False, doc, alert, error


# Kept as a scheduler binding because completion-verification tests and the live
# re-read intentionally patch this seam. The implementation remains in the job store.
...
def _resolve_cron_disabled_toolsets(job: Any, cfg: Optional[dict] = None) -> list[str]:
    """Policy/config denylist. One-argument calls remain the global-config form."""
    if cfg is None:
        cfg = job or {}
        job = {}
...
# HERMES-057: job ids whose worker thread outlived its timeout and is still running. The claim must
# stay held (and heartbeated) until the future actually completes — releasing it while the worker
# still runs is the double-dispatch case this contract exists to prevent.
_retained_worker_job_ids: set = set()
...
def release_running_job(job_id: str) -> None:
    ...
    scheduler dispatch the same job again alongside the still-live worker (HERMES-057).
    """
    with _running_lock:
...
def _run_no_agent_job(
    ...
    *, total_run_deadline: Optional[float] = None,
    ...
    try:
        script_kwargs = {"workdir": _job_workdir, "cancel_event": cancel_event}
        remaining = _remaining_run_budget(total_run_deadline)
        if remaining is not None:
            script_kwargs["timeout_seconds"] = remaining
        ok, output = _run_job_script_with_claim_heartbeat(job, script_path, **script_kwargs)
    except Exception as exc:
        logger.exception("Job '%s': script execution raised unexpectedly", job_id)
        ok, output = False, f"Script execution failed: {exc}"

    if _remaining_run_budget(total_run_deadline) == 0:
        return _total_run_budget_failure(job, job_name, total_run_budget)
...
def _apply_monitor_gate(
    ...
    *, total_run_deadline: Optional[float] = None,
    ...
    remaining = _remaining_run_budget(total_run_deadline)
    _mon = check_monitor(job) if remaining is None else check_monitor(job, timeout_seconds=remaining)
    if _remaining_run_budget(total_run_deadline) == 0:
        return _total_run_budget_failure(job, job_name, total_run_budget), extra_prompt
...
def _open_cron_session_db(job: dict, timeout_seconds: Optional[float] = None):
    ...
    _session_db_timeout = float(_get_session_db_timeout())
    if timeout_seconds is not None:
        remaining = max(0.0, float(timeout_seconds))
        _session_db_timeout = (
            min(_session_db_timeout, remaining) if _session_db_timeout > 0 else remaining
        )
    try:
...
def _run_agent_with_watchdog(
    ...
    *, total_run_deadline: Optional[float] = None,
    ...
    def _wait_for_cron_worker_exit() -> None:
        """Bound the join, then retain and heartbeat a lingering worker (HERMES-057).

        A timeout tears down the agent but cannot kill the worker thread. Until that future
        completes the job is still executing, so its run claim must keep being heartbeated —
        otherwise the claim expires, the scheduler treats the job as free, and dispatches a second
        worker alongside the first. The bounded join covers the common case where the worker exits
        promptly; the fence thread covers the one where it does not.
        """
    ...
    try:
        ...
        _must_poll = (
            ...
            or total_run_deadline is not None
            or _is_oneshot
        ...
        if not _must_poll:
            ...
            while True:
                if _cron_future.done():
                    ...
                    result = _cron_future.result()
                    break
                remaining_total = _remaining_run_budget(total_run_deadline)
                if remaining_total is not None and remaining_total <= 0:
                    _total_budget_timeout = True
                    break
                wait_timeout = _POLL_INTERVAL
                if remaining_total is not None:
                    wait_timeout = min(wait_timeout, remaining_total)
                done, _ = concurrent.futures.wait({_cron_future}, timeout=wait_timeout)
                if done:
                    _abort_if_fire_claim_lost()
                    result = _cron_future.result()
                    break
...
def _completion_configuration_failure(job: dict[str, Any]) -> Optional[str]:
    """Reject a verifier whose protected job configuration changed during the agent run."""
...
def _capture_job_script_snapshot(script_path: str) -> tuple[bool, bytes | str]:
    """Read one trusted verifier before the agent can mutate its backing file."""
    scripts_dir = (_get_hermes_home() / "scripts").resolve()
    try:
        raw_path = Path(script_path).expanduser()
        path = (raw_path if raw_path.is_absolute() else scripts_dir / raw_path).resolve()
        path.relative_to(scripts_dir)
    except (OSError, RuntimeError, ValueError):
        return False, f"Blocked: script path resolves outside the scripts directory: {script_path!r}"
    if not path.is_file():
...
def _finalize_cron_session(
    ...
    *,
    ...
    _end_reason = "cron_completion_failed" if completion_failed else "cron_complete"
...
def _prepare_job_prompt(
    ...
    *, total_run_deadline: Optional[float] = None,
    ...
    if job.get("no_agent"):
        return _run_no_agent_job(
            job, job_id, job_name, cancel_event,
            total_run_deadline=total_run_deadline,
            total_run_budget=total_run_budget), None
    ...
    script_path = job.get("script")
    if script_path:
        script_kwargs = {"cancel_event": cancel_event}
        remaining = _remaining_run_budget(total_run_deadline)
        if remaining is not None:
            script_kwargs["timeout_seconds"] = remaining
        prerun_script = _run_job_script_with_claim_heartbeat(job, script_path, **script_kwargs)
        if _remaining_run_budget(total_run_deadline) == 0:
            return _total_run_budget_failure(job, job_name, total_run_budget), None
...
class _CronRunScope:
    ...
    def __init__(self, job: dict, job_id: str, execution_id: Optional[str]):
        from cron.outbound import _ensure_cron_session_context_vars
        ...
        # Native cron messages are restricted to the adapter identity of the profile that owns the
        # job. Bind that identity before the agent runs, and fail CLOSED on an unresolved or failed
        # lookup: an empty profile makes send_message refuse, while None would inherit the default
        # profile's adapter and send as the wrong principal.
        try:
            from hermes_cli.profiles import get_active_profile_name
            job_profile = get_active_profile_name() or ""
        except Exception:
            job_profile = ""
...
def _reload_dotenv_and_publish_delivery_target(job: dict) -> None:
    ...
    run_id = str(claim.get("run_id") or "").strip() if isinstance(claim, dict) else ""
    fire_owner = str(claim.get("by") or "").strip() if isinstance(claim, dict) else ""
    _VAR_MAP["HERMES_CRON_JOB_ID"].set(job_id)
    _VAR_MAP["HERMES_CRON_RUN_ID"].set(run_id or str(uuid.uuid4()))
    _VAR_MAP["HERMES_CRON_FIRE_OWNER"].set(fire_owner)
...
def _construct_cron_agent(
    AIAgent, job: dict, _cfg: dict, setup: _CronAgentSetup, *, workdir,
    session_id, session_db, run_budget_seconds: Optional[float] = None,
):
    ...
    if run_budget_seconds is not None:
        kwargs["run_budget_seconds"] = run_budget_seconds
...
def run_job(
    ...
    try:
        total_run_budget = _normalize_run_budget_seconds(job.get("run_budget_seconds"))
    ...
    total_run_deadline = (
        time.monotonic() + total_run_budget if total_run_budget is not None else None
    )
    ...
    try:
        if completion_script:
            snapshot_ok, snapshot_value = _capture_job_script_snapshot(completion_script)
            if not snapshot_ok or not isinstance(snapshot_value, bytes):
                raise RuntimeError(
                    f"Completion verifier unavailable before agent run: {snapshot_value}"
                )
            expected_sha = str(job.get("completion_script_sha256") or "").lower()
            observed_sha = hashlib.sha256(snapshot_value).hexdigest()
            if not re.fullmatch(r"[0-9a-f]{64}", expected_sha):
                raise RuntimeError("Completion verifier has no valid CLI-pinned content hash")
            if observed_sha != expected_sha:
                raise RuntimeError("Completion verifier content changed after CLI configuration")
            completion_snapshot = snapshot_value
        scope.enter()
        if scope.workdir:
        ...
        if remaining is not None and remaining <= 0:
            raise _total_run_budget_error(job_name, total_run_budget)
        _session_db = _open_cron_session_db(job, timeout_seconds=remaining)
        remaining = _remaining_run_budget(total_run_deadline)
        if remaining is not None and remaining <= 0:
            raise _total_run_budget_error(job_name, total_run_budget)
        ...
        def _construct_agent():
            return _construct_cron_agent(
                AIAgent, job, _cfg, setup, workdir=scope.workdir,
                session_id=_cron_session_id, session_db=_session_db,
                run_budget_seconds=remaining)

        if remaining is None:
            agent = _construct_agent()
        else:
            agent_future: concurrent.futures.Future = concurrent.futures.Future()
            agent_context = contextvars.copy_context()

            def _initialize_agent() -> None:
                try:
                    agent_future.set_result(agent_context.run(_construct_agent))
                except BaseException as error:
                    agent_future.set_exception(error)
            ...
            try:
                agent = agent_future.result(timeout=remaining)
                ...
                agent_future.add_done_callback(_teardown_late_agent)
                raise _total_run_budget_error(job_name, total_run_budget) from error

        if total_run_deadline is not None:
            setattr(agent, "_cron_total_run_deadline", total_run_deadline)
        ...
        completion_output = ""
        if completion_script:
            completion_error = _completion_configuration_failure(job)
            if completion_error:
                completion_ok = False
                completion_output = completion_error
            else:
                completion_ok, completion_output = _run_job_script_with_claim_heartbeat(
                    job,
                    completion_script,
                    workdir=scope.workdir,
                    cancel_event=cancel_event,
                    script_snapshot=completion_snapshot,
                )
            if not completion_ok:
                completion_failed = True
                error_msg = f"Completion verification failed: {completion_output}"
                output = (
                    _run_doc_header(job, f"{job_name} (FAILED)", job_id, prompt)
                    + f"## Agent Response\n\n{logged_response}\n\n"
                    + f"## Completion Verification Failed\n\n{completion_output}\n"
                )
                _audit.write(
                    dict(result, response_silent=False),
                    error_msg,
                )
                return (
                    False,
...
from cron.scheduler_delivery import (  # noqa: E402
    ...
    _resolve_delivery_targets, _resolve_origin,
)
from cron.scheduler_script import (  # noqa: E402
    _drain_script_pipes, _get_script_timeout, _get_session_db_timeout,
    _run_job_script, _run_job_script_with_claim_heartbeat,
    _start_heartbeat_thread, _terminate_cron_script_process,
