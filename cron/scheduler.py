...
# Job ids whose worker thread outlived an inactivity timeout and are still running. The claim must
...
def try_register_running_job(job_id: str, *, reservation_token: Optional[object] = None) -> bool:
    ...
    with retirement.work() as admitted, _running_lock:
        if not admitted or job_id in _running_job_ids or job_id in _running_fire_owners:
            return False
