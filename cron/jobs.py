...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if schedule_lower.startswith("in "):
        ...
        run_at = (now.astimezone(dt_timezone.utc) + timedelta(minutes=minutes)).astimezone(now.tzinfo)
...
def _effective_cron_timezone(timezone_name: Optional[str]) -> tzinfo:
    return ZoneInfo(timezone_name) if timezone_name else _hermes_now().tzinfo
...
def _cron_next_run_matches_expr(
    schedule: Dict[str, Any], next_run_dt: datetime, timezone: Optional[str] = None,
) -> bool:
    ...
    try:
        if timezone:
            next_run_dt = next_run_dt.astimezone(_effective_cron_timezone(timezone))
...
def compute_next_run(
    ...
    if kind == "interval":
        ...
        next_run = base_time.astimezone(dt_timezone.utc) + timedelta(minutes=minutes)
    ...
    if kind == "cron":
        ...
        zone = _effective_cron_timezone(timezone)
...
def _compute_next_run_for_job(
    job: Dict[str, Any], last_run_at: Optional[str] = None,
) -> Optional[str]:
    timezone_name = job.get("timezone")
    if timezone_name:
        return compute_next_run(job["schedule"], last_run_at, timezone=timezone_name)
...
def create_job(
    ...
    *,
    ...
    if parsed_schedule.get("kind") == "once" and next_run_at is None:
        raise _oneshot_past_grace_error(parsed_schedule.get("run_at") or schedule)
    ...
    *,
    ...
    def apply(jobs, i, job):
        ...
        if "completion_script" in updates:
            updates["completion_script"] = _normalize_job_optional_text(
        ...
        if {
            ...
            _validate_job_mode_invariants(
                ...
                _normalize_job_optional_text(updated.get("completion_script")))
...
def claim_job_for_fire(
    ...
    manual: bool = False, preserve_paused: bool = False, return_job: bool = False,
    ...
    def apply(jobs, _i, job):
        ...
        # A deferred fire carries the occurrence it already owns, so it is exempt.
        if (instant is not None and not deferred
...
def _recover_missing_next_run(job: Dict[str, Any], scan: _DueScan) -> Optional[str]:
    ...
    if not recovered_next and kind in {"cron", "interval"}:
        recovered_next = _compute_next_run_for_job(job, scan.now.isoformat())
        if recovered_next:
            recovery_kind = kind
...
class _DueJob:
    ...
    def recompute_next(self) -> Optional[str]:
        return _compute_next_run_for_job(self.job, self.scan.now.isoformat())

    @property
    def now(self) -> datetime:
        return self.scan.now.astimezone(_effective_cron_timezone(self.job.get("timezone")))
...
def _reanchor_stale_cron(d: _DueJob) -> bool:
    ...
    stale_class = _classify_stale_cron_next_run(
        d.schedule, d.raw_next_run_dt, d.next_run_dt, d.job.get("timezone"))
...
def _evaluate_due_job(job: Dict[str, Any], scan: _DueScan, run_claim_ttl: float) -> bool:
    ...
    now = scan.now.astimezone(_effective_cron_timezone(job.get("timezone")))
