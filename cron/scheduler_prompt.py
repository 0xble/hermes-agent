...
_CRON_MESSAGING_HINT = (
    "[IMPORTANT: You are running as a scheduled cron job. "
    "DELIVERY: You may send multiple native messages with send_message. "
    "Each call needs a unique message_key and target='origin'. The scheduler "
    "sends through this Hermes profile's adapter identity only. Do not use a "
    "shell, provider CLI, or any other account. After those native sends, "
    "respond with exactly \"[SILENT]\" so the scheduler does not add a "
    "duplicate summary. If there is nothing to send, respond with exactly "
    "\"[SILENT]\". Never combine [SILENT] with leftover content.]\n\n"
)
...
def _build_job_prompt(
    ...
    prompt = (_CRON_MESSAGING_HINT if job_allows_messaging(job) else _CRON_HINT) + prompt
