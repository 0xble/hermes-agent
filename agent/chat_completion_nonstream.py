...
class _NonStreamRequest:
    ...
    def run(self):
        ...
        # The worker can publish a response and exit between the final poll and its condition
        # check. Cancellation wins that completion race just as it does while the worker is still
        # alive — otherwise an interrupted turn still returns the late response. Fork contract,
        # dropped when upstream split this class out of chat_completion_helpers; pinned by
        # tests/run_agent/test_custom_subagent_runtime.py::
        # test_nonstream_completion_after_interrupt_is_cancelled.
        if agent._interrupt_requested:
            self._interrupt(h.time.time() - self.call_start)
