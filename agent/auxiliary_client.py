...
# Structured auxiliary responses must be complete before any physical retry is
# accepted. ContextVar keeps sync and async fallback chains request-local.
_AUX_REQUIRE_COMPLETE_RESPONSE: contextvars.ContextVar[bool] = contextvars.ContextVar(
    "auxiliary_require_complete_response", default=False
)
...
def _is_invalid_aux_response_error(exc: Exception) -> bool:
    ...
    incomplete = (
        "llm returned an incomplete structured response" in msg
        and "finish_reason=" in msg
    )
    return "auxiliary " in msg and (malformed or incomplete)
...
def _validate_llm_response(
    ...
    *,
    ...
    if require_complete and finish_reason not in complete_finish_reasons:
        raise RuntimeError(
    ...
    route = {
        "provider": str(provider or relay_context.get("provider") or "").strip(),
        "model": str(relay_context.get("model") or getattr(response, "model", "") or "").strip(),
        "base_url": str(base_url or "").strip(),
        "api_mode": str(relay_context.get("api_mode") or "").strip(),
    }
    ...
    if route:
        try:
            ...
            try:
                object.__setattr__(response, "_hermes_auxiliary_route", route)
            except Exception:
                pass
