...
_AUX_REQUIRE_COMPLETE_RESPONSE: contextvars.ContextVar[bool] = (
    contextvars.ContextVar("auxiliary_require_complete_response", default=False)
)
...
def _build_xai_oauth_aux_client(
    model: str, *, explicit_api_key: Optional[str] = None,
...
def _build_codex_client(
    ...
    if explicit_api_key:
        ...
        base_url = explicit_base_url or codex_override or _CODEX_AUX_BASE_URL
    else:
        pool_present, entry = _select_pool_entry("openai-codex")
...
def _is_invalid_aux_response_error(exc: Exception) -> bool:
    """HTTP-200 empty/malformed/truncated structured response."""
    ...
    if "auxiliary " not in msg:
        return False
    return (
        "incomplete structured response" in msg
        or (
...
def _transient_credential_retry_reason(
    ...
    """Return the bounded same-provider alternate-credential reason, if any."""
    try:
        from agent.error_classifier import FailoverReason, classify_api_error
        classified = classify_api_error(exc, provider=provider or "", model=model or "")
        ...
        logger.debug("Auxiliary transient credential classification failed", exc_info=True)
    return None


def _select_transient_aux_alternate(
    provider: str, *, failed_api_key: str, reason: str,
) -> Optional[Tuple[str, Optional[str], str]]:
    """Select one alternate runtime credential without durable exhaustion."""
    normalized = _normalize_aux_provider(provider)
...
def _validate_llm_response(
    ...
    require_complete_response: Optional[bool] = None,
) -> Any:
    ...
    try:
        ...
        choices = response.choices
    require_complete = (
        _AUX_REQUIRE_COMPLETE_RESPONSE.get()
        if require_complete_response is None
        else require_complete_response
    )
    finish_reason = str(getattr(choices[0], "finish_reason", "") or "").lower()
    if require_complete and finish_reason not in {
        "", "stop", "end_turn", "completed", "success", "eos", "eos_token", "finished",
    }:
        raise RuntimeError(
            f"Auxiliary {task or 'call'}: LLM returned an incomplete structured "
            f"response (finish_reason={finish_reason})"
        )
    # Retain the provider-reported model and request-side route for terminal attribution.
    context = _RELAY_AUX_CALL_CONTEXT.get()
...
def _ladder_credential_rungs(
    ...
    if pool_provider and transient_reason:
        ...
        if alternate is not None:
            ...
            logger.info(
                "Auxiliary %s%s: transient %s on %s; trying alternate credential %s",
                task or "call", tag, transient_reason, pool_provider, alternate_label,
            )
            alternate_overrides = dict(
                resolved_base_url=alternate_base or route.resolved_base_url,
                resolved_api_key=alternate_key,
            )
            try:
                return (yield _LadderStep("retry_same_provider", (
                    pool_provider, route.resolved_model or route.final_model,
                    alternate_overrides,
                ))), None
            except Exception:
                logger.info(
                    "Auxiliary %s%s: alternate credential %s failed; continuing fallback",
                    task or "call", tag, alternate_label,
                )
                # Keep the primary transient classification so provider
                # fallback remains a capacity recovery even if the alternate
                # failed with an unrelated stale-credential error.
                first_err = original_err
    return None, first_err
...
def call_llm(
    ...
    response_format = (extra_body or {}).get("response_format")
    require_complete = require_complete_response or (
        isinstance(response_format, dict)
        and response_format.get("type") in {"json_object", "json_schema"}
    )
    completeness_token = _AUX_REQUIRE_COMPLETE_RESPONSE.set(require_complete)
    ...
    try:
        ...
        _AUX_REQUIRE_COMPLETE_RESPONSE.reset(completeness_token)
        if semaphore is not None:
            semaphore.release()
...
def _ladder_step_call(
    ...
    if step.kind == "retry_same_provider":
        retry_provider, retry_model, *overrides = step.args
        retry_call_kwargs = dict(
            retry_kwargs, resolved_provider=retry_provider, resolved_model=retry_model,
        )
        if overrides:
            retry_call_kwargs.update(overrides[0])
        return "retry", (), retry_call_kwargs
