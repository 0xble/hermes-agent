...
def apply_stop_gates(
    ...
    def _continue(nudge: str, flag: str) -> StopGateVerdict:
        ...
        candidate_previewed = agent._interim_content_was_streamed(final_response or "")
        if flag not in {"_verification_stop_synthetic", "_pre_verify_synthetic"}:
            return StopGateVerdict(
                continue_turn=True, final_response=None,
                pending_verification_response=final_response,
                pending_verification_response_previewed=candidate_previewed,
            )

        from agent.turn_finalizer import (
            _merge_verification_candidate, _verification_candidate_replaces_pending,
        )

        previous = pending_verification_response
        merged = _merge_verification_candidate(previous, final_response)
        composed = bool(previous and merged != previous and merged != final_response)
        if composed:
            previewed = False
        elif _verification_candidate_replaces_pending(previous, final_response):
            previewed = candidate_previewed
        else:
            previewed = pending_verification_response_previewed or candidate_previewed
        return StopGateVerdict(
            continue_turn=True, final_response=None,
            pending_verification_response=merged,
            pending_verification_response_previewed=previewed,
        )
