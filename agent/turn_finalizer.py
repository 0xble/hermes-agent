...
def _compose_verification_receipt_with_answer(
    pending_response: str | None, final_response: str | None,
) -> str | None:
    """Keep the substantive answer when a continuation returns only a receipt."""
    if not pending_response or not final_response:
        return final_response
    pending, final = pending_response.strip(), final_response.strip()
    if pending == final or not _is_verification_receipt_only(final):
        return final_response
...
def _merge_verification_candidate(
    pending_response: str | None, candidate_response: str | None,
) -> str | None:
    """Accumulate a candidate without allowing a receipt to erase the answer."""
    return (
        candidate_response
        if not pending_response
        else _compose_verification_receipt_with_answer(pending_response, candidate_response)
    )
...
def _verification_candidate_replaces_pending(
    ...
    """Whether a complete later candidate supersedes prior visible text."""
    return bool(
        pending_response
        and candidate_response
        and pending_response.strip() != candidate_response.strip()
        and not _is_verification_receipt_only(candidate_response)
    )
...
def _collapse_verification_candidates(messages, final_response, agent) -> bool:
    """Collapse current-turn provisional answers into one canonical assistant row."""
    ...
    if last_candidate < len(messages) - 1:
        for index in reversed(candidate_indices):
            messages.pop(index)
        canonical = {"role": "assistant", "content": final_response}
...
def _apply_output_hooks(
    ...
    messages, verification_pre_transform=None, interrupted=False,
) -> Tuple[Any, bool, Optional[Any], bool]:
    """Fire ``transform_llm_output`` then ``post_llm_call`` once per turn after the tool loop.
    Returns the response, aggregate transform metadata, and whether the output hook
    itself changed the response. Partial/interrupted text is transformed too because
    a fallback delivery path may already have made it user-visible."""
    transformed = bool(verification_pre_transform)
    pre_transform = verification_pre_transform
    output_hook_transformed = False
    try:
        from hermes_cli.lifecycle import transform_llm_output

        transformed_response, did_transform = transform_llm_output(
            final_response,
        ...
        if did_transform:
            ...
            transformed = True
            output_hook_transformed = True
    except AttributeError as exc:
        # Compatibility with plugin runtimes that expose only the generic hook
        # dispatcher. First string wins there; the dedicated lifecycle helper,
        # when present, owns sequential composition and per-callback isolation.
        hook_results = _invoke_hook_safely(
        ...
        for hook_result in hook_results:
            if isinstance(hook_result, str) and hook_result:
                ...
                break
        if not hook_results:
            logger.debug("transform_llm_output compatibility fallback: %s", exc)
    except Exception as exc:
    ...
    return final_response, transformed, pre_transform, output_hook_transformed
...
def finalize_turn(
    ...
    # A complete later answer supersedes its provisional predecessor, but a
    # receipt-only continuation must be attached to—not erase—the substantive
    # answer. On interrupt, keep the last complete pending candidate rather than
    # exposing a partial continuation.
    _verification_delivery_pre_transform = None
    if interrupted and final_response and _pending_verification_response:
        if final_response != _pending_verification_response:
            _verification_delivery_pre_transform = final_response
        final_response = _pending_verification_response
    elif not preserved_verification_fallback:
        _raw_final_response = final_response
        final_response = _compose_verification_receipt_with_answer(
            _pending_verification_response, final_response,
        )
        if _raw_final_response and final_response and final_response != _raw_final_response:
            # The streamed receipt is not the canonical response; delivery must
            # publish the composed answer in full.
    ...
    def _persist_step():
        ...
        # Interim candidates may already exist as separate durable rows. A
        # replacement snapshot is required to delete those provisional rows;
        # an append-only flush cannot perform the collapse.
        if (
            _verification_candidates_collapsed
            and getattr(agent, "_session_db", None) is not None
            and agent.session_id
        ):
            agent._session_db.replace_messages(agent.session_id, messages, active_only=True)
    ...
    if final_response:
        (
            ...
            verification_pre_transform=_verification_delivery_pre_transform,
            interrupted=interrupted,
        )

    # Keep the canonical transcript synchronized with an output transform. Scope
    # the search to this turn so equal text from an earlier turn is never rewritten.
                ...
                try:
                    if getattr(agent, "_session_db", None) is not None and agent.session_id:
                        agent._session_db.replace_messages(
                            agent.session_id, messages, active_only=True,
                        )
                    _save_log = getattr(agent, "_save_session_log", None)
                    if callable(_save_log):
                        _save_log(messages)
                except Exception as exc:
                    _cleanup_errors.append(f"persist_transformed_response: {exc}")
                    logger.error(
