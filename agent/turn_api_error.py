...
def handle_api_error(
    ...
    transient_reason = classified.reason in {
        FailoverReason.overloaded,
        FailoverReason.server_error,
        FailoverReason.timeout,
    }
    if transient_reason and recover_transient_credential(
        agent,
    ...
    _recovered, recovered_with_pool = recover_after_classification(
        ...
        status_code=status_code, error_context=error_context,
        messages=messages, api_messages=api_messages,
    )
    if _recovered:
        return _verdict("continue")

    if transient_reason and _retry.transient_credential_retries_used > 0:
        # Every credential retry used a distinct account. Exhaust the generic
        # retry budget rather than repeat the last failed account indefinitely.
