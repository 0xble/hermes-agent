...
class MemoryProvider(ABC):
    ...
    def supports_read_only_prefetch(self) -> bool:
        """Whether ``prefetch`` is safe in a read-only agent context. Fail-closed by default:
        legacy prefetch implementations may have incidental side effects, so providers opt in."""
        return False
    ...
    def get_read_only_tool_names(self) -> set[str]:
        """Enumerate pure-retrieval tools; unknown operations are withheld in read-only mode."""
        return set()
