...
# control-plane mutations behind a real user-authored turn while preserving read-only
# inspection and reversible project-local work. Expand only with an explicit
...
_INTERNAL_TURN_ALWAYS_BLOCKED = frozenset({
    "delegate_task", "hindsight_invalidate", "hindsight_restore", "hindsight_retain",
    "send_message", "skill_manage",
})
...
def _internal_turn_effect_block(agent, function_name: str, function_args: dict) -> str | None:
    ...
    if normalized_name == "memory" and (function_args or {}).get("operations"):
        blocked = True
    if not blocked:
        blocked = action in _INTERNAL_TURN_BLOCKED_ACTIONS.get(normalized_name, frozenset())
    if not blocked:
        return None
    ...
    logger.warning(
        ...
        normalized_name, detail, getattr(agent, "session_id", "") or "",
