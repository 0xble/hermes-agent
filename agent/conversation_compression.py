...
def _emit_compaction_terminal(
    ...
    if not status_callback:
        return
    with _swallow(
        f"status_callback error in compaction terminal outcome={outcome}",
        exc_info=True,
    ):
...
class CompressionCommitFence:
    ...
    def __init__(self, total_ceiling_seconds: float | None = None) -> None:
        ...
        # Worker-side deadline expiry is a route failure, not an explicit stop.
        # The host consults this when the Future settles immediately adjacent
        # to the shared deadline so scheduling order cannot bypass fallback.
    ...
    while True:
        ...
        try:
            ...
            if fence.worker_deadline_abort_observed:
                # The worker and host share the deadline. A cooperative worker
                # can unwind just before Future.result times out; route that
                # race through the ordinary fallback path (#102339).
                logger.info(
...
class _CompactionLifecycle:
    ...
    paths keep the terminal edge suppressed. ``terminal_message`` overrides the outcome's default text for
    outcomes that need a specific explanation (the would-grow refusal), keeping the count at exactly one
    terminal edge instead of a warning plus a generic abort."""
    ...
    def complete(self, *, force_terminal: bool = False) -> None:
        ...
        # Suppressed start → no terminal edge. Lock contenders and transient
        # blockers opt in so clients can retire an already-open phase.
        if not self.status_emitted and not force_terminal:
            return
        _emit_compaction_terminal(self._agent, self.commit_status, self.terminal_message)
...
def _sit_out_lock_contention(
    ...
    lifecycle.commit_status = "deferred"
...
def _adopt_if_parent_rotated(
    ...
    lease._lifecycle.commit_status = "committed" if recovered_messages is not None else "deferred"
    lease.release()
    _existing_sp = _existing_system_prompt(agent, system_message)
...
def _salvage_or_refuse_grown_transcript(
    ...
    if _rough_out > _rough_in:
        ...
        # No ``_emit_warning`` here: the refusal is reported once, as the compaction lifecycle's
        # single ``compaction_deferred`` terminal edge carrying COMPACTION_WOULD_GROW_STATUS (set by
        # the caller on the refusal path). A warning here would make it two notices for one outcome.
        _existing_sp = _existing_system_prompt(agent, system_message)
...
def _candidate_rejected(
    ...
    attempt_generation: Any, attempt_started_at: float, lifecycle: Any = None,
) -> bool:
    ...
    if getattr(agent.context_compressor, "_last_compress_aborted", False):
        ...
        # The reason rides the lifecycle's single ``compaction_aborted`` terminal edge rather than a
        # separate warning: one started compaction must produce exactly one outcome notice, and a
        # generic "Compression failed" edge alongside a detailed warning is two (HERMES-025).
        if lifecycle is not None:
            lifecycle.commit_status = "aborted"
            lifecycle.terminal_message = _abort_message
        else:
            agent._emit_warning(_abort_message)
...
def compress_context(
    ...
    try:
        ...
        if _candidate_rejected(
            ...
            attempt_started_at=attempt.started_at, lifecycle=lifecycle,
        ):
            return messages, _existing_system_prompt(agent, system_message)
        if commit_fence is not None:
        ...
        if commit.refused_prompt is not None:
            # Would-grow refusal (the only path that sets ``refused_prompt``): a DEFERRAL, not an
            # abort. Nothing failed and nothing was dropped, so the one terminal edge must say so.
            lifecycle.commit_status = "deferred"
            lifecycle.terminal_message = COMPACTION_WOULD_GROW_STATUS
...
def _compress_context_via_codex_app_server(
    ...
    if failed:
        # HERMES-025: one truthful terminal edge per compaction, carrying the detailed reason.
        # A direct ``_emit_warning`` here produced a user-facing failure notice with NO terminal
        # status event, so the lifecycle either looked unfinished or was later closed by a generic
        # edge — a failure followed by false success or a duplicate deferral, which is exactly what
        # this contract forbids. The Codex-native branch reports like every other abort path.
        with contextlib.suppress(Exception):
