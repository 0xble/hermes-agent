...
def _iteration_summary_chat_kwargs(agent, api_messages: list) -> dict:
    ...
    if not is_lmstudio and agent._supports_reasoning_extra_body():
        extra_body["reasoning"] = agent._current_reasoning_config() if agent._current_reasoning_config() is not None else {"enabled": True, "effort": "medium"}
    ...
    with contextlib.suppress(Exception):
        ...
        if provider_profile is not None:
            profile_extra_body = provider_profile.build_extra_body(
                ...
                model=agent.model, base_url=agent.base_url, reasoning_config=agent._current_reasoning_config())
