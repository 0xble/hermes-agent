...
def _maybe_title_session_at_turn_start(agent: Any, messages: List[Any]) -> None:
    ...
    try:
        ...
        # Preserve the native content parts for image-aware title generation while
        # still deriving text for the instant-title path.
        user_text = ""
        title_context = None
        for msg in reversed(messages or []):
            if isinstance(msg, dict) and msg.get("role") == "user":
                content = msg.get("content")
                user_text = flatten_message_text(content).strip()
                if isinstance(content, list):
                    title_context = content
                break
        if not user_text and title_context is None:
            return
...
def _capture_goal_control_revision(agent: Any) -> None:
    """Snapshot the persisted ``set_goal`` user-control fence for this turn.

    Busy slash commands may advance the revision while the model is reasoning.
    Dispatching the turn-start value lets the goal tool reject that stale model
    activation rather than undoing newer user control.
    """
    if "set_goal" not in (getattr(agent, "valid_tool_names", None) or ()):
        return
...
def reanchor_current_turn_user_idx(messages: List[Any], user_message: Any) -> int:
    ...
    surviving copy in the common case — so request-context injection and the #48677 persist override can't land on
...
def _persist_turn_start(
    ...
    """Crash-resilience: persist the clean inbound user turn before the first LLM
    call. Same critical section as CLI close persistence; retries the row create if
    the pre-compression attempt failed transiently."""
...
def _stamp_api_content_sidecar(
    ...
    # Fork guard: a non-dict or non-user entry at this index is not this turn's user row,
    # and stamping it would attach composed bytes to the wrong message.
...
def build_turn_context(
    ...
    _capture_goal_control_revision(agent)
