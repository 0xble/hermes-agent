...
def _db_flush_failed(agent, e: Exception, batch_rows: List[Dict[str, Any]], adoption_budget: int) -> bool:
    ...
    if isinstance(e, CompressionSessionClosedError):
        ...
        if adoption_budget > 0 and _db_flush_adopt_compression_tip(agent):
            return True
        # Fail closed rather than guess a target session; the flag lets the turn explanation name
        # rotation instead of the historical (misleading) full-disk advice.
        agent._compression_adoption_failed = True
        logger.warning("Session DB append_message failed: %s", e)
        return False
    # This failure destroys the user's turn (it triggers the deliberate ``session_persistence_failed``
    # stop), so retain enough metadata to diagnose SQLite and SessionDB ownership failures without
    # logging message content, SQL values, history, credentials, or the full database path.
    _thread = threading.current_thread()
    logger.warning(
        "Session DB append_message failed: %s: %s (sqlite_errorcode=%s sqlite_errorname=%s "
        "thread=%s/%s session_db=0x%x)",
        type(e).__name__, e, getattr(e, "sqlite_errorcode", None), getattr(e, "sqlite_errorname", None),
        _thread.name, _thread.ident, id(agent._session_db), exc_info=True,
    )
    return False
