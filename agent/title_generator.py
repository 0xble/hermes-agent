...
def _model_title_upgrade_enabled() -> bool:
    ...
    try:
        # Lazy imports for the same reason as _auto_title_enabled().
        from hermes_cli.config import load_config_readonly
        from utils import is_truthy_value
...
def _extract_title_text(content: str) -> str:
    ...
    A truncated structured payload is dropped rather than handed to the prose
    fallback: the fragment would otherwise be persisted as the session title.
    """
    if not content:
        return ""
    raw = content.strip()
...
def generate_title(
    ...
    def _request_title(content: Any):
        ...
        return call_llm(
            ...
            # temperature omitted from the wire rather than forced to 0.3: models that
            # accept only their server-side default (GPT-5.6) reject an explicit value and
            # the daemon title thread dies with "Unsupported value: 'temperature'"
            # (#72351, #51083, #51157). Titling wants determinism, not a specific value.
            temperature=None,
            timeout=timeout,
    ...
    try:
        ...
        # A reasoning model under ``json_schema`` may leave ``content`` empty and put the
        # payload in ``reasoning_content`` / ``reasoning`` (glm-5 / minimax, #82291).
        title = _clean_title(
            _extract_title_text(content) or _title_from_reasoning(choice.message),
            max_characters=None,
        )
