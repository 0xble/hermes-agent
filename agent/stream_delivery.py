...
class StreamDeliveryMixin:
    ...
    def _extract_codex_interim_visible_parts(self, assistant_msg: Dict[str, Any], *,
                                             respect_display_setting: bool = True) -> List[str]:
        ...
        commentary stays on the reasoning channel unless the caller opts out of that setting.
        """
        hidden = respect_display_setting and not getattr(self, "show_commentary", True)
        items = None if hidden else assistant_msg.get("codex_message_items")
        messages: List[str] = []
    ...
    def _deliver_interim(self, visible: str, *, already_streamed: bool, record: List[str],
                         callback: Any = None) -> None:
        """Hand ``visible`` to ``callback`` (default ``interim_assistant_callback``) and mark ``record``
        delivered; swallows callback errors."""
        cb = callback if callback is not None else getattr(self, "interim_assistant_callback", None)
        if cb is None:
            return
        try:
    ...
    def _emit_interim_assistant_message(self, assistant_msg: Dict[str, Any]) -> None:
        ...
        if not isinstance(assistant_msg, dict):
            return
        is_clarify_turn = self._assistant_message_calls_tool(assistant_msg, "clarify")
        cb = getattr(self, "interim_assistant_callback", None)
        if cb is None and is_clarify_turn:
            # A clarify prompt is not self-explanatory when the model put its decision brief right
            # before the tool call. Quiet chat profiles may hide ordinary interim narration but must
            # still see this decision-critical context — and for the same reason a clarify turn
            # ignores display.show_commentary below.
            cb = getattr(self, "clarify_context_callback", None)
        commentary_parts = self._extract_codex_interim_visible_parts(
            assistant_msg, respect_display_setting=not is_clarify_turn)
        ...
        self._deliver_interim(visible, already_streamed=already_streamed,
                              record=undelivered_parts or [visible], callback=cb)

    @classmethod
    def _assistant_message_calls_tool(cls, assistant_msg: Dict[str, Any], tool_name: str) -> bool:
        """Whether a persisted assistant turn invokes ``tool_name``."""
