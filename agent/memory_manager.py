...
def memory_provider_tools_exposed(agent: Any) -> bool:
    ...
    """
    # A read-only child never carries the built-in "memory" tool and runs with the
    # memory toolset disabled, so the ordinary gate would hide the very read-only
    # provider tools it is meant to have. Its filtered routing table is the gate.
...
class MemoryManager:
    ...
    def __init__(self, *, external_prefetch_timeout: Optional[float] = None,
        ...
        # retrieval are routed. Fail-closed — a provider with no read-only contract
        # is skipped entirely rather than trusted by default.
        self._read_only = bool(read_only)
        self._read_only_initialized_providers: set[MemoryProvider] = set()
        self._read_only_unavailable_providers: List[Dict[str, str]] = []
    ...
    def _read_only_active(self) -> List[MemoryProvider]:
        """Providers a read-only manager may read from: only those that actually
        initialized under the read-only contract (a skipped provider must not
        contribute a system-prompt block or recall)."""
        return [p for p in self._providers if p in self._read_only_initialized_providers]
    ...
    def build_system_prompt(self) -> str:
        ...
        blocks = self._each_provider("system_prompt_block() failed", lambda p: p.system_prompt_block(),
                                      ...
                                      providers=self._read_only_active() if self._read_only else None)
    ...
    def prefetch_all(self, query: str, *, session_id: str = "") -> str:
        ...
        readers = None
        if self._read_only:
            # supports_read_only_prefetch() is fail-closed on the base class, so a
            # provider that never opted in is not recalled from even when it
            # exposes read-only tools.
            readers = [p for p in self._read_only_active() if p.supports_read_only_prefetch()]
    ...
    def get_all_tool_schemas(self) -> List[Dict[str, Any]]:
        ...
        def _collect(provider: MemoryProvider) -> None:
            for raw_schema in provider.get_tool_schemas():
                ...
                elif (schema["name"] not in _HERMES_CORE_TOOLS
                      and self._tool_to_provider.get(schema["name"]) is provider
                      and schema["name"] not in seen):
    ...
    def handle_tool_call(self, tool_name: str, args: Dict[str, Any], **kwargs) -> str:
        ...
        if provider is None:
            # Withhold the routing table's shape from a read-only child: an
            # unrouted name there means "filtered out", not "does not exist".
            if self._read_only:
                return tool_error(f"Memory tool '{tool_name}' is unavailable in read-only mode")
            return tool_error(f"No memory provider handles tool '{tool_name}'")
        # Defence in depth: routing already filtered, but a provider that mutated
        # its schemas after initialize() must not win a stale route.
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        for provider in self._providers:
            if self._read_only and not self._has_read_only_contract(provider):
                self._read_only_unavailable_providers.append(
                    {"provider": provider.name, "reason": "no_read_only_contract"})
                logger.warning("Memory provider '%s' has no explicit read-only contract; skipping",
                               provider.name)
                continue
            try:
        ...
        self._rebuild_tool_routing(initialized_providers, reason="provider initialization")
        if self._read_only:
            self._read_only_initialized_providers = set(initialized_providers)

    @staticmethod
    def _has_read_only_contract(provider: MemoryProvider) -> bool:
        """Fail closed: a provider must positively declare pure-retrieval tools or
        read-only prefetch support before a delegated child may initialize it."""
        try:
            return bool(provider.get_read_only_tool_names()) or bool(provider.supports_read_only_prefetch())
        except Exception:
            return False
