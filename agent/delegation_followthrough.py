...
def retrieve_deferred_context(agent, deferred):
    ...
    candidates = list_durable_delegations(owner=owner, session_db=session_db)
    ...
    for candidate in candidates:
        ...
        if not any(k == key for k, _, _ in wanted):
            continue
        item = get_delegation_result(candidate["delegation_id"], owner=owner, session_db=session_db)
