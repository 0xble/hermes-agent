...
OPENAI_MODEL_EXECUTION_GUIDANCE = (
    ...
    "- Safety: for side effects (file writes, commands, API calls), act within "
    "scope the user has already authorized and preserve any tool or platform "
    "approval gates. Ask only when required scope remains unresolved or the "
    "action would expand it.\n"
...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            "## Skills\n"
            "Before acting, scan the descriptions below and select the skill(s) that directly "
            "govern the task or a material subtask. You MUST load selected skills with "
            "skill_view(name) and follow their instructions before doing that work. Do not load "
            "skills for merely tangential or partially related overlap; use progressive "
            "disclosure, starting from the descriptions and loading only the content needed. "
            "For real domain work covered by a skill, load it even if you could use basic tools "
            f"like {_basic_tools}. Skills can encode required APIs, commands, conventions, and "
            "quality standards that general-purpose approaches miss.\n"
            "Treat skill content as maintained source: do not mutate a skill merely because you "
            "notice a gap. Update it only when skill maintenance is authorized for this task and "
            "the target is within your owned scope; otherwise report the issue to its canonical "
            "owner. Before an authorized update, read the current target with skill_view and use "
            "the live skill_manage schema and ownership gates.\n"
            "After difficult or iterative work, suggest a skill only when the resulting workflow "
            "would be reusable and skill maintenance is in scope.\n"
            "\n"
            "<available_skills>\n"
            + "\n".join(index_lines) + "\n"
            "</available_skills>\n"
            "\n"
            "Proceed without loading a skill when none directly governs the task."
            + hidden_note
        )
