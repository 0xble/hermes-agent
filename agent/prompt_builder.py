...
TELEGRAM_RICH_MESSAGES_HINT = (
    ...
    "formatting over dense paragraphs for any comparison, set of steps, key/value summary, or tabular data. Use real "
    "Markdown tables for compact tabular data and real task lists instead of hand-built substitutes. Keep tables "
    "narrow. Never use Markdown tables with three or more columns. Instead, transpose or split that data into "
    "vertically stacked records with labeled fields. Rich tables degrade gracefully into readable bullet groups "
    "when rich rendering is unavailable, but advanced constructs like math and collapsible details may render as "
    "plain source text in that case. "
)
...
# Per-process probe cache is scoped to the profile, effective task cwd, and backend
# identity. The gateway can multiplex all three in one process.
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    try:
        ...
        if not isinstance(docker_env, dict):
            docker_env = {}
        cache_key = (
            str(get_hermes_home()), env_type, resolve_tool_cwd(),
            config.get("ssh_host"), config.get("ssh_user"), config.get("ssh_port"),
            config.get("docker_image"), config.get("singularity_image"),
            config.get("modal_image"), config.get("daytona_image"),
            config.get("vercel_runtime"), config.get("modal_mode"),
            config.get("docker_run_as_host_user"), docker_env.get("HOME"),
            docker_env.get("USER"),
        )
    except Exception as e:
        logger.debug("Backend probe unavailable (setup failed): %s", e)
        return None
...
# v3: root-level entries use ``general``; v2 snapshots may encode them as
# ``<name>/<name>`` and must be rebuilt.
