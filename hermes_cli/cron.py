...
def _job_rows(job: Dict[str, Any]) -> List[tuple[str, str]]:
    ...
    return [
        ...
        ("Timezone", f"{job['timezone']} (explicit)" if job.get("timezone")
...
def _print_job_details(job_data: Dict[str, Any]) -> None:
    """Print the optional Script/Verifier/Monitor/Mode/Continuity/Workdir lines of a job record."""
...
def cron_create(args):
    # The completion verifier is CLI-owned: validated and content-pinned here so the agent's
    # cronjob tool can never introduce or swap one.
    completion_script = getattr(args, "completion_script", None)
    if completion_error := _validate_completion_script_path(completion_script):
...
def cron_edit(args):
    ...
    if completion_error := _validate_completion_script_path(completion_script):
        ...
        return 1
    # A verifier only means anything when an agent runs; check the EFFECTIVE post-edit pair so
    # --no-agent alone cannot strand a job with an unreachable completion gate.
    requested_no_agent = getattr(args, "no_agent", None)
    effective_no_agent = requested_no_agent if requested_no_agent is not None else bool(job.get("no_agent"))
    effective_completion_script = (completion_script if completion_script is not None
                                   else job.get("completion_script"))
    if effective_completion_script and effective_no_agent:
        print(color("Failed to update job: completion_script requires an agent run and "
                    "cannot be combined with --no-agent.", Colors.RED))
        return 1
    if completion_script == "" and requested_no_agent is True:
        print(color("Failed to update job: clear --completion-script before enabling --no-agent.",
                    Colors.RED))
        return 1
