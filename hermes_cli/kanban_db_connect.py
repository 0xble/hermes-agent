...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    #
    # HERMES-002: refusal and raw reads are ONE atomic step under
    # ``offline_file_access``. A bare has_live_connection() followed by the
    # fingerprint/copy is a check/use race — a connection opened in the gap
    # (connect_tracked blocks on the same lock instead) would have its POSIX
    # locks cancelled by our close().
...
def _backup_corrupt_db_locked(resolved: Path, parent: Path, base_name: str) -> Optional[Path]:
    """Fingerprint + copy body of :func:`_backup_corrupt_db`.

    Must be called inside ``offline_file_access(resolved)`` — every byte-level read here is only
    safe while the connection-lifecycle lock is held."""
