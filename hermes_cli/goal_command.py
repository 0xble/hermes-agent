...
def _resume(mgr, arg, render):
    ...
    # HERMES-071: slash confirmations and the goal tool's notices share one renderer, so a
    # resume also carries the reassessment note and any recorded blocker, not just the headline.
    from hermes_cli.goal_display import format_goal_change
    return GoalCommandResult(format_goal_change("resume", state),
...
def _set(mgr, arg, *, drafting, last_user_message, render, progress):
    ...
    if not drafting:
        # HERMES-071: `/goal <text>` confirmations come from the same shared renderer.
        from hermes_cli.goal_display import format_goal_change
        return GoalCommandResult(format_goal_change("set", state),
                                 goals.goal_kick_prompt(state.goal, last_user_message), kickoff=True)
    # Drafting keeps upstream's own confirmation: it explains what the aux model produced.
