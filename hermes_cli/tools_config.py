...
_RECENTLY_SHIPPED_TOOLSETS: frozenset = frozenset({"goal", "review"})
...
def _get_platform_tools(config: dict, platform: str, *, include_default_mcp_servers: bool = True) -> Set[str]:
    ...
    # A recently shipped toolset can be legitimately absent from a saved list after the picker offered it.
    # Some of those (currently ``goal``) are not ordinary configurable entries, so the non-configurable
    # recovery pass must honour the same decline record as _enable_recently_shipped_toolsets instead of
    # silently adding them back.
    recovery_skip = configurable_keys | plugin_ts_keys | platform_default_keys
    _declined = (config.get("known_builtin_toolsets") or {}).get(platform)
    if isinstance(_declined, list):
        recovery_skip |= {str(ts) for ts in _declined} & _RECENTLY_SHIPPED_TOOLSETS - enabled_toolsets
    _recover_platform_native_toolsets(enabled_toolsets, platform, skip=recovery_skip)
    if plugin_ts_keys:
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    _cfg_section(config, "known_builtin_toolsets")[platform] = sorted(
        _configurable_keys() | _RECENTLY_SHIPPED_TOOLSETS)
