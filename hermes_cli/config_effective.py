...
def _effective(raw: Dict[str, Any]) -> Dict[str, Any]:
    expanded = _config._normalize_root_model_keys(_config._expand_env_vars(raw))
    # Share the authored-layer merge/preset ordering with load_config, but never
    # inject DEFAULT_CONFIG into presence-sensitive runtime readers.
    merged, _ = _config._merge_managed_overlay(expanded, include_defaults=False)
    return _config._normalize_root_model_keys(merged)
