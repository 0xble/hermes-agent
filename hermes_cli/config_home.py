...
def initialize_home(home: Path, subdirs: tuple[str, ...], ensured: set[str]) -> None:
    ...
    try:
        _ensure_directory(home, create=not managed, secure=not managed, boundary=home, home=home)
        required = ("cron", "sessions", "logs", "memories") if managed else subdirs
        for subdir in required:
            _ensure_directory(home / subdir, create=not managed, secure=not managed, boundary=home, home=home)
        if managed:
            _ensure_directory(home / "logs" / "curator", create=True, secure=False, boundary=home, home=home)
        try:
