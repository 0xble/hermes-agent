...
DEFAULT_CONFIG = {
    ...
    "auxiliary": {
        ...
        "title_generation": {
            ...
            "reasoning_effort": "",
            "language": "",
            # Native opening images reach an independently configured auxiliary provider only after
            # the operator opts into that boundary. The generator also defaults this off in code, so
            # the boundary stays closed even where these defaults are absent.
            "include_attachments": False,
            # Title-shaping preferences. The word range guides the model; both maxima are enforced
            # after generation. Defaults preserve the historical 3-7 word / 80 character behaviour.
            # An explicitly configured profile that OMITS ``max_characters`` expresses "no cap" —
            # that check reads the raw user config, so this default never suppresses it.
            "min_words": 3,
            "max_words": 7,
            "max_characters": 80,
        ...
        "background_review": {"enabled": True, "skill_mode": "direct", **_aux(120), "max_input_tokens": 600000},
    ...
    "tts": {
        ...
        "provider": "edge",
        "fallback_providers": [],  # Opt-in availability-only, whole-utterance retries.
        "edge": {
    ...
    "memory": {  # Persistent memory — bounded curated memory injected into the system prompt
        "background_policy": "approve_changes",
        "memory_enabled": True,
    ...
    "goals": {
        ...
        "max_turns": 20,
        # Agent-created goal receipt notices. Manual /goal confirmations and unexpected-stop
        # explanations remain visible regardless of this display preference.
        "auto_notices": True,
    },
    ...
    "skills": {
        ...
        "tier1_advisory": True,
        # Autonomous self-improvement may UPDATE existing skills, but new skill creation is opt-in.
        # Fail closed by default; foreground skill_manage(create) stays user-directed and unaffected.
        # Enforced at the skill_manage runtime boundary for the background_review origin only.
