...
def _collect_fleet_snapshot(restart, rows_expected: bool) -> list:
    ...
    _pre_restart_pid_set = _coerce_pid_set(pre_pids)
    while True:
        _time.sleep(2.0)
        snapshot = collect_fleet_versions(pre_restart_pids=pre_pids)
        # A stale row whose pid was alive before the restart is still DRAINING, not a failed
        # replacement (#102733) — keep polling for it exactly like identity_pending rows.
        settling_stale = any(
            row.get("state") == "stale"
        ...
        if (snapshot and not pending and not settling_stale
