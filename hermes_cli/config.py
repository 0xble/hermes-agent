...
# A legacy flat MoA layer names its slots directly instead of selecting a preset.
_LEGACY_FLAT_MOA_SLOT_KEYS = frozenset({"reference_models", "aggregator"})


def _merge_config_layer(base: dict, override: dict) -> dict:
    """Merge one config layer without shadowing an explicit flat MoA preset (HERMES-016).

    ``DEFAULT_CONFIG`` carries ``moa.presets.default``. A legacy flat MoA layer deliberately has no
    ``presets`` key, so an ordinary deep merge keeps that inherited preset and ``normalize_moa_config``
    then ignores the layer's explicit slots. Drop ONLY the inherited preset selectors before merging
    such a layer; named-preset layers keep normal deep-merge behaviour.
    """
    moa_override = override.get("moa")
    if (isinstance(moa_override, dict) and "presets" not in moa_override
        ...
        base_moa = dict(inherited_moa) if isinstance(inherited_moa, dict) else {}
        for selector in ("presets", "default_preset", "active_preset"):
            base_moa.pop(selector, None)
        base["moa"] = base_moa
...
def _read_raw_config_impl(*, want_deepcopy: bool, strict: bool = False) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if not strict and cached is not None and cached[:len(cache_key)] == cache_key:
...
def read_raw_config(*, strict: bool = False) -> Dict[str, Any]:
    ...
    Normal reads tolerate missing/unparseable files and cache on the file signature
    (mtime_ns, size, ino, ctime_ns). Security-sensitive readers use ``strict=True`` to
    bypass that cache and reject I/O, parse, or root-shape errors. A missing file still
    means omission.
    """
...
def read_config_mapping_strict(config_path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            config_path, "has a formatting error", exc,
            _FIX_YAML.format(backups=_backups_dir_display()), parse_failure=True) from exc
    if loaded is None:
        return {}
    if not isinstance(loaded, dict):
        ...
        raise ConfigReadError(
            ...
            exc, _FIX_YAML.format(backups=_backups_dir_display()), parse_failure=True) from exc
    return loaded
...
def _last_known_good_fallback(config_path: Path, path_key: str, cache_sig, exc: Exception) -> Optional[Dict[str, Any]]:
    ...
    if lkg is None:
        ...
        if raw_good is not None:
            # Preset references and flat MoA slots are authored-layer semantics:
            # expand them before defaults, just as on a successful normal load.
            normalized = _canonicalize_config(raw_good)
...
def _merge_managed_overlay(
    expanded: Dict[str, Any], *, strict: bool = False, include_defaults: bool = True,
    ...
    # Presence-sensitive callers share authored-layer expansion, but must not
    # gain default keys (or an empty agent section from full canonicalization).
    defaults_and_user = user_routes
    if include_defaults:
...
def _load_config_impl(*, want_deepcopy: bool, strict: bool = False) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if not strict and cached is not None and cache_sig is not None and cached[:8] == cache_sig:
