...
This module is surface-neutral. CLI, gateway, TUI, and configured aliases can
all feed the same expanded ``/reasoning`` argument text into it.
"""
...
def parse_reasoning_turn(raw_args: str) -> ReasoningTurnRequest | None:
    """Parse ``<level> <prompt>`` after alias expansion.

    Bare effort commands deliberately abstain so the existing session-scoped
    command handler remains authoritative. Unknown first tokens also abstain so
    existing status/display error handling remains unchanged.
    """
    ...
    if not prompt or prompt == "--global":
        return None
