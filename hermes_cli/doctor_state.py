...
def _session_retention_policy() -> dict:
    """Resolve the effective session-retention settings for Doctor (HERMES-032).

    Doctor must read the configured policy rather than infer it from rendered text. Any failure degrades to
    an explicit ``error`` so the advisory stays actionable instead of silently claiming retention is configured.
    """
    try:
        ...
        retention_days = None
        if not isinstance(raw_retention, bool):  # bools are ints; a bool retention is not a day count
            try:
...
def _render_state_db_stats(stats: dict, holders=None, *, retention_policy: dict | None = None) -> list:
    """Turn a collect_state_db_stats() dict into ``(kind, text, detail, issue)`` rows, kind 'info' / 'warn'.

    ``issue`` is the matching Doctor issue summary (None when the row needs no operator action) so callers
    never have to infer severity from rendered text. Pure formatting — no I/O — so it is unit-testable
    without the doctor CLI. Tolerates None in every field.
    """
    lines: list = []
    stats = stats or {}
    ...
    if fts:
        ...
        lines.append(("info", "FTS tables: " + (", ".join(present) if present else "none"), "", None))
    deferral = stats.get("fts_rebuild_deferral")
    if isinstance(deferral, dict):
        pids = deferral.get("holder_pids") or "unknown"
        # Fork: _render_state_db_stats returns 4-tuples (kind, text, detail, issue) — the caller at
        # doctor_state.py:290 unpacks four and appends `issue` to the doctor findings. Upstream's
        # rewrite emits 3-tuples; shipping those verbatim raises ValueError on every state.db warn.
        if deferral.get("futile"):
            lines.append(("warn", f"state.db FTS repair is blocked by the same holder(s) PID(s) {pids} for "
                          ...
                          "rebuilds within a minute of the holder leaving)", None))
        else:
            lines.append(("warn", f"state.db FTS repair is blocked after {deferral.get('attempts') or '?'} deferral(s) "
                          ...
                          "'hermes sessions optimize-storage' with every holder stopped)", None))
    # Oversized DB: report the configured retention state (HERMES-032 — size alone is not actionable when
    # pruning is already configured), plus the offline optimize-storage pass when the FTS rebuild is
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        valid_retention = (isinstance(retention_days, int) and not isinstance(retention_days, bool)
                           and retention_days > 0)
        if auto_prune is True and valid_retention:
            kind, issue = "info", None
            detail = (f"sessions.auto_prune is enabled with retention_days={retention_days}; it removes ended "
                      "inactive sessions, but does not cap active-session growth or itself shrink the SQLite file")
        elif auto_prune is False:
            kind = "warn"
            detail = ("sessions.auto_prune is disabled; enable it in config.yaml to remove ended inactive "
                      "sessions older than retention_days")
        ...
        if stats.get("fts_rebuild_pending") or stale_trigram:
            ...
            issue = "state.db is large — run 'hermes sessions optimize-storage' offline (gateway stopped)"
        lines.append((kind, f"state.db is large ({_human_bytes(logical)})", f"({detail})", issue))
...
def _state_db_stats(issues: list, state_db_path: Path) -> None:
    ...
    with warn_on_error("state.db stats unavailable ({e})", "", report=lambda t, _d: check_info(t)):
        ...
        for _kind, _text, _detail, _issue in rows:
            if _kind != "warn":
