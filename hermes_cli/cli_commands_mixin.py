...
def _parse_cron_flags(tokens):
    ...
    opts = {
        "name": None, "deliver": None, "repeat": None, "prompt": None, "schedule": None, "timezone": None,
...
class CLICommandsMixin:
    ...
    def _cron_list(self, subcommand: str, opts: dict) -> None:
        ...
        for job in jobs:
            _pr(f"  ID: {job['job_id']}", f"  Name: {job['name']}",
                ...
                f"  Timezone: {job['timezone']} (explicit)" if job.get("timezone")
    ...
    def _handle_reasoning_command(self, cmd: str):
        ...
        if raw:
            ...
            try:
                request = parse_reasoning_turn(raw)
            except ReasoningTurnError as exc:
                return _cp(_dim_line(str(exc)))
            if request is not None:
                self._pending_turn_reasoning_config = dict(request.reasoning_config)
                self._pending_agent_seed = request.prompt
                return _cp(request.notice)
