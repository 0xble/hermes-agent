...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    # The maintained fork is the release authority; upstream integration is maintenance,
    # never part of a runtime check. Preserve shallow checkout boundaries.
...
def _repair_venv_on_current_checkout(
    *, assume_yes, gateway_mode, pre_update_snapshot_id,
    ...
    return _repair_node_deps_on_current_checkout(
        _print_verified_update_completion if print_completion is None else print_completion,
...
def _reconcile_diverged_checkout(git_cmd, branch: str, pre_pull_sha) -> None:
    ...
    if _cur_branch and _cur_branch != branch:
        ...
        if _git_run(git_cmd, ["merge", "--no-edit", f"origin/{branch}"]).returncode != 0:
            ...
            sys.exit(1)
        return
    # Same branch: a true remote force-push/rebase; local changes are stashed, so reset.
...
def _prepare_checkout_for_update(
    ...
    upstream_checked = True  # Origin is the sole release authority in this fork.
...
def _finish_already_up_to_date(
    ...
    final_head_guard=None, no_gateway_restart: bool = False) -> None:
...
def _apply_pulled_update(
    ...
    _windows_gateway_resume, args, pinned_revision: str | None = None) -> None:
    ...
    # Fork (#133): origin is the sole release authority, so there is no upstream-sync prompt
    # here or in the post-swap tail. Upstream runs ``_sync_with_upstream_if_needed`` for forks
    # on ``main``; deliberately omitted.
    _hand_off_post_swap(
        ...
        _windows_gateway_resume=_windows_gateway_resume, pinned_revision=pinned_revision)
...
def _post_swap_payload(
    *, swap: str, branch: str, opts, gateway_mode: bool, had_desktop_app_before_update: bool,
    ...
    _windows_gateway_resume=None, pinned_revision: str | None = None) -> dict:
    ...
    return {
        ...
        # Immutable target SHA (fork ``--revision``/``--stable``): the post-swap tail must keep
        # re-proving checkout identity, so the pin has to cross the interpreter boundary.
...
def _execute_post_swap(payload: dict, args, gateway_mode: bool) -> None:
    ...
    try:
        ...
        _finish_pulled_update(
            ...
            pinned_revision=payload.get("pinned_revision"))
...
def _finish_pulled_update(
    ...
    # Fork (#133): origin is the sole release authority — no ``_sync_with_upstream_if_needed``
    # prompt for forks on ``main``.
...
def _run_pinned_revision_update(
    ...
    had_desktop_app_before_update, pre_update_snapshot_id, _windows_gateway_resume, args=None,
) -> None:
