...
def build_cron_parser(subparsers, *, cmd_cron: Callable) -> None:
    ...
    cron_create.add_argument("--completion-script", dest="completion_script",
        help="Trusted script under ~/.hermes/scripts/ run after the agent returns. "
            "Exit zero verifies observable completion; non-zero or timeout marks the "
            "run failed without constraining the agent's work.")
    ...
    _flag(cron_create, "--allow-messaging", dest="allow_messaging", default=False,
    ...
    cron_edit.add_argument("--timezone",
        help="Pin cron wall-clock schedules to an IANA timezone. Pass empty string to "
            "clear and inherit the profile timezone.")
    cron_edit.add_argument("--allow-messaging", dest="allow_messaging",
