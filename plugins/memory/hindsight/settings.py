...
def _normalize_observation_scopes(value: Any) -> Any:
    ...
    An explicit empty inner list (``[[]]``) is Hindsight's documented ``shared``
    scope — one global consolidation pass — and is preserved, not discarded."""
    if isinstance(value, str):
        text = value.strip()
    ...
    for entry in value:
        if isinstance(entry, (list, tuple)):
            # Keep explicitly-empty inner lists: [[]] is Hindsight's documented
            # equivalent of the "shared" scope. Dropping it silently reverted the
            # config to the "combined" default, fragmenting observations by
            # volatile session:<id> lineage tags (#74933). An inner list that
            # only becomes empty after stripping whitespace tags is malformed
