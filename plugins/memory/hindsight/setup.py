...
def run_setup(provider, hermes_home: str, config: dict) -> None:
    ...
    from . import _CLIENT_REQUIREMENT

    # hindsight-all allows a broad client range, so the exact client pin joins the
    # same solve instead of letting a transitive resolve drift off reviewed bytes.
    deps = ["hindsight-all", _CLIENT_REQUIREMENT] if mode == "local_embedded" else [_CLIENT_REQUIREMENT]
