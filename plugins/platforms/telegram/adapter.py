...
async def _first_completed(*futures: "asyncio.Future") -> None:
    """Return when the first of *futures* completes; the caller owns the losers' lifecycle.

    Used by the strict cold-start readiness gate to wait on "progress OR polling error" (#67498)."""
    await asyncio.wait(set(futures), return_when=asyncio.FIRST_COMPLETED)


def _flood_cap_result(wait: float, *, retryable: bool = False) -> "SendResult":
    """The shared fail-closed SendResult for an over-cap flood wait."""
    return SendResult(success=False, error=f"flood_control:{wait}", retryable=retryable, retry_after=float(wait))
...
# HERMES-095: Telegram Rich Message clients collapse an authored ``\n\n`` to no visible gap, so a prose
# paragraph boundary gets one explicit non-breaking-space row bounded by hard breaks. Structural regions
# (fences, tables, tilde fences, display math, <details>) keep their newlines byte for byte.
...
def _rich_is_prose_line(line: str) -> bool:
    """True when *line* can safely share one rich prose paragraph (no block semantics of its own)."""
    if line[:1].isspace():
        return False
    stripped = line.strip()
    if not stripped:
...
def _rich_materialize_prose_paragraphs(text: str) -> str:
    ...
    A non-breaking-space line bounded by Markdown hard breaks gives the boundary one stable visual row
    without the oversized spacer more raw blank lines would produce. Structural Markdown boundaries stay
    raw so headings, lists, blockquotes, details, and math keep their block semantics; fenced code and
    pipe tables are excluded by the caller's protected-region split."""
    if "\n\n" not in text:
        return text
    protected = [(m.start(), m.end()) for m in _RICH_PARAGRAPH_PROTECTED_REGION_RE.finditer(text)]
    protected_index = 0
    out: list[str] = []
    cursor = 0
    for match in re.finditer(r"\n{2,}", text):
        while protected_index < len(protected) and protected[protected_index][1] <= match.start():
            protected_index += 1
        inside_protected = protected_index < len(protected) and protected[protected_index][0] < match.end()
        previous_start = text.rfind("\n", 0, match.start()) + 1
        next_end = text.find("\n", match.end())
        if next_end < 0:
            next_end = len(text)
        previous_line, next_line = text[previous_start:match.start()], text[match.end():next_end]
        out.append(text[cursor:match.start()])
        if not inside_protected and _rich_is_prose_line(previous_line) and _rich_is_prose_line(next_line):
            out.append(f"\n{_RICH_VISUAL_PARAGRAPH_SPACER}\n")
        else:
...
def _rich_normalize_linebreaks(text: str) -> str:
    """Materialize prose paragraphs and harden lone ``\\n`` (a Markdown soft break) into hard breaks for
    sendRichMessage; every structural region is left byte for byte."""
    if not text or '\n' not in text:
        return text
    out: list[str] = []
    pos = 0
...
# Telegram Rich Markdown interprets paired literal dollar signs as inline LaTeX delimiters. Protect
# currency only on rich-rendered lines containing multiple amounts; leave ordinary prose, legitimate
# math, code, and shell variables unchanged.
...
# Link-target shaping for the rich path. Telegram renders only http(s):// and tg:// targets as
...
def _degrade_unsupported_markdown_links(text: str) -> str:
    ...
    the label text. Code spans/blocks and table blocks are left verbatim.
    """
    if '[' not in text:
        return text
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # ``auto`` keeps the adaptive routing, ``always`` sends every eligible final response rich, and
        # ``never`` keeps legacy MarkdownV2. Boolean config stays compatible: true=auto, false=never.
        ...
        # HERMES-004: concurrent components (typing, progress edits, final reply) each honour their own
        # retry-after, but the burst still crosses Telegram's per-chat threshold and escalates into
        # multi-thousand-second penalties. A cross-cutting per-chat gate forces a minimum gap between
        # every outbound call so the cumulative rate stays under the threshold. Keyed by chat so
        # unrelated chats never serialise on one lock.
        ...
        self._send_cooldown_max_wait: float = self._coerce_float_extra(
            "send_cooldown_max_wait_seconds", 5.0, min_value=0.5, max_value=60.0)
        ...
        self._send_path_recovery_generation: int = 0
        ...
        self._dm_topic_icon_observer: Optional[Callable[[str, str, Optional[str]], None]] = None
    ...
    def _replacement_telegram_adapter(self) -> Optional["TelegramAdapter"]:
        """Live adapter if the reconnect watcher replaced us (an in-flight ``send()`` still holds the old
        instance whose ``_bot`` stays None).

        The watcher installs the new adapter in the *owning profile's* registry, so a secondary profile
        resolves through ``_profile_adapters``; reading ``runner.adapters`` for it would route its replies
        through the primary profile's bot."""
        runner = getattr(self, "gateway_runner", None)
        owner_profile = getattr(self, "_owner_profile", None)
        if owner_profile:
            adapters = (getattr(runner, "_profile_adapters", None) or {}).get(owner_profile, {}) or {}
        else:
            adapters = getattr(runner, "adapters", None) or {}
        live = adapters.get(self.platform)
        if (
            live is not None and live is not self and getattr(live, "_bot", None)
    ...
    def _thread_kwargs_for_send(
        cls, chat_id: str, thread_id: Optional[str], metadata: Optional[Dict[str, Any]] = None,
        reply_to_message_id: Optional[int] = None, reply_to_mode: Optional[str] = None) -> Dict[str, Any]:
        """Topic routing plus Telegram Business routing, for every persistent send."""
        route = cls._topic_kwargs_for_send(
            chat_id, thread_id, metadata, reply_to_message_id=reply_to_message_id, reply_to_mode=reply_to_mode)
    ...
    def _thread_kwargs_for_draft(self, chat_id: str, metadata: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # PTB's send_message_draft does not accept this Bot API routing parameter; a Business turn relies
        # on the final persistent send, which does support it.
    ...
    def _mark_dm_topic_stale(self, chat_id: Any, thread_id: Any) -> bool:
        """Record a topic Telegram confirmed gone; True the first time this pair is seen."""
        if chat_id is None or thread_id is None:
            return False
        stale = getattr(self, "_stale_dm_topic_ids", None)
        if not isinstance(stale, dict):
            stale = self._stale_dm_topic_ids = {}
    ...
    def is_dm_topic_stale(self, chat_id: Any, thread_id: Any) -> bool:
        """True once a send proved this DM topic is deleted (read by gateway topic recovery)."""
        return (str(chat_id), str(thread_id)) in getattr(self, "_stale_dm_topic_ids", {})
    ...
    async def _recover_stale_subchat_delivery(
        ...
        """Re-deliver a completed response at chat root when its DM topic vanished mid-turn."""
        topic_metadata = dict(metadata or {})
        thread_id = self._metadata_thread_id(topic_metadata)
        if (
            not topic_metadata.get("notify") or not thread_id
        ...
        for key in (
            ...
            "telegram_direct_messages_topic_id", "telegram_reply_to_message_id", "reply_to_message_id",
            "telegram_dm_topic_reply_fallback", "telegram_dm_topic_created_for_send"):
        ...
        # A partial delivery must not repeat the visible prefix: point the user at the session instead.
        recovery_content = (
            ...
            if partial_delivery else "Recovered response from a deleted Telegram topic:\n\n" + content)
        logger.warning(
            "[%s] Recovering completed response at chat root after stale topic chat=%s thread=%s",
            self.name, chat_id, thread_id)
        return await self._send_with_retry(
            chat_id=chat_id, content=recovery_content, reply_to=None, metadata=topic_metadata)
    ...
    async def _send_with_dm_topic_reply_anchor_retry(
        ...
        try:
            ...
            # A flood rejection is not an anchor problem: surface it as a cooldown so the caller defers
            # instead of re-sending the media without its topic anchor.
    ...
    def _coerce_rich_message_mode(self) -> str:
        ...
        Existing boolean config is preserved deliberately: true means auto, false means never. An unknown
        value fails closed to auto rather than silently disabling the feature after a config typo."""
    ...
    async def _run_send_call(self, cooldown_chat_id: Any, send_fn: Any, *args: Any, _reserve_gap: bool = True, _expendable: Optional[bool] = None, **kwargs: Any):
        ...
        time before this request publishes a ``RetryAfter`` deadline."""
        cooldowns = getattr(self, "_send_cooldown_until", None)
        ...
        try:
            ...
            if wait > remaining_budget:
                logger.warning(
                    "[%s] send cooldown for chat %s exceeds remaining wait budget (%.1fs > %.1fs); yielding with retryable error",
                    self.name, chat_key, wait, remaining_budget)
    ...
    def _telegram_retry_after(error: Exception) -> Optional[float]:
        """Seconds Telegram asked us to wait, from the PTB attribute or the error text; None if absent."""
        value: Any = getattr(error, "retry_after", None)
        if value is None:
            match = re.search(r"retry\s+(?:after|in)\s+(\d+(?:\.\d+)?)", str(error), re.IGNORECASE)
    ...
    def _bounded_send_retry_after(self, retry_after: float) -> float:
        """The full finite deadline for upstream retry scheduling.

        ``_send_cooldown_max_wait`` bounds only inline sleeping; truncating this would wake the delivery
        ledger while Telegram still has the chat flood-controlled."""
        parsed = float(retry_after)
        if not math.isfinite(parsed):
            return max(0.001, float(getattr(self, "_send_cooldown_max_wait", 5.0)))
        return max(0.001, parsed)

    def _send_retry_after_outcome(self, error: Exception, attempt: int, *, prior_chunks_delivered: bool = False):
        """``(handled, result_or_None)`` for a flood rejection; ``result is None`` means retry in place."""
        retry_after = self._telegram_retry_after(error)
        if retry_after is None:
            return False, None
        if retry_after > float(getattr(self, "_send_cooldown_max_wait", 5.0)):
            # The caller owns the long wait. A first-chunk rejection is safe to retry later because
    ...
    def _send_cooldown_failure(self, error: "_TelegramSendCooldownExceeded") -> SendResult:
        return SendResult(
            success=False, error=str(error), retryable=True,
            retry_after=self._bounded_send_retry_after(error.retry_after))

    def _chat_in_send_cooldown(self, chat_id: Any) -> bool:
        """True while the chat's shared gate is still holding back outbound traffic."""
        cooldowns = getattr(self, "_send_cooldown_until", None)
        if not cooldowns:
            return False
        return float(cooldowns.get(str(chat_id), 0.0) or 0.0) > time.monotonic()

    async def _edit_message_text_with_cooldown(self, chat_id: Any, **kwargs: Any):
    ...
    def _business_connection_kwargs(metadata: Optional[Dict[str, Any]]) -> Dict[str, str]:
        """The PTB routing kwarg for a Telegram Business conversation, or ``{}`` outside one."""
    ...
    def _load_input_checklist_types() -> tuple[Any, Any]:
        """Load PTB checklist input types lazily so optional Telegram installs still import."""
        from telegram import InputChecklist, InputChecklistTask
        return InputChecklist, InputChecklistTask

    def _build_input_checklist(
        self, title: str, tasks: List[Dict[str, Any]], *, others_can_add_tasks: bool = False,
        others_can_mark_tasks_as_done: bool = False) -> Any:
        """Validate and construct the PTB checklist input objects (Bot API limits enforced locally)."""
        ...
        if not isinstance(others_can_add_tasks, bool) or not isinstance(others_can_mark_tasks_as_done, bool):
            raise ValueError("Checklist collaboration flags must be boolean")
        return InputChecklist(
            title=title, tasks=normalized_tasks, others_can_add_tasks=others_can_add_tasks,
            others_can_mark_tasks_as_done=others_can_mark_tasks_as_done)

    def _checklist_transport_failure(self, error: Exception, *, ambiguous_send: bool) -> SendResult:
        """Classify a checklist failure without authorizing a duplicate send."""
        retry_after = self._telegram_retry_after(error)
        error_kind = classify_send_error(error)
        retryable = bool(
            retry_after is not None or self._looks_like_connect_timeout(error)
            or self._looks_like_pool_timeout(error))
        if not ambiguous_send and error_kind in {"transient", "rate_limited"}:
            # Checklist edits are idempotent for one message ID, so retrying a classified
            # transport/rate-limit failure cannot create a duplicate.
            retryable = True
        return SendResult(
            success=False, error=_redact_telegram_error_text(error), retryable=retryable,
            retry_after=(self._bounded_send_retry_after(retry_after) if retry_after is not None else None),
            error_kind=error_kind)

    async def send_checklist(
        self, chat_id: str, title: str, tasks: List[Dict[str, Any]], *,
        ...
        if not connection_id:
            return SendResult(
                success=False, error="Telegram native checklists require business_connection_id", retryable=False)
        try:
            checklist = self._build_input_checklist(
                title, tasks, others_can_add_tasks=others_can_add_tasks,
                others_can_mark_tasks_as_done=others_can_mark_tasks_as_done)
        except (TypeError, ValueError) as error:
            return SendResult(success=False, error=str(error), retryable=False)
        kwargs: Dict[str, Any] = {
            "business_connection_id": connection_id, "chat_id": normalize_telegram_chat_id(chat_id),
            "checklist": checklist, **self._notification_kwargs(metadata)}
        if reply_to is not None:
            try:
                kwargs["reply_to_message_id"] = self._positive_message_id(reply_to, "reply_to")
            except ValueError as error:
                return SendResult(success=False, error=str(error), retryable=False)
        try:
            message = await self._run_send_call(chat_id, self._bot.send_checklist, **kwargs)
    ...
    async def edit_checklist(
        self, chat_id: str, message_id: str, title: str, tasks: List[Dict[str, Any]], *,
        business_connection_id: Optional[str] = None, others_can_add_tasks: bool = False,
    ...
    def _rich_message_payload(self, content: str, *, skip_entity_detection: bool = False) -> Dict[str, Any]:
        ...
        Paired ``$`` amounts are code-wrapped so Telegram's rich parser does not read them as inline
    ...
    async def _try_send_rich(
        ...
        try:
            ...
            msg = await self._run_send_call(chat_id, self._bot.do_api_request, "sendRichMessage", api_kwargs=payload)
            ...
            if self._rich_rejected(exc, "sendRichMessage", "MarkdownV2"):
                return None
            # Honor Telegram's flood-control retry_after over the base retry schedule; _run_send_call has
            # already published it to the chat's shared gate for every concurrent sender.
    ...
    def _record_polling_progress(self, generation: int) -> None:
        ...
        if was_degraded:
            self._send_path_recovery_generation = getattr(self, "_send_path_recovery_generation", 0) + 1
        if was_degraded and getattr(self, "gateway_runner", None) is not None:
            # Polling owns the health transition, not the gateway reconnect watcher: wake the durable
            # ledger on this edge without blocking getUpdates or spawning a task per healthy long-poll.
    ...
    def set_dm_topic_icon_observer(
        self,
        observer: Optional[Callable[[str, str, Optional[str]], None]],
    ) -> None:
        """Register the gateway's durable topic-icon observation sink."""
        self._dm_topic_icon_observer = observer
    ...
    def _remember_dm_topic_creation_icon(
        ...
        observer = self._dm_topic_icon_observer
        if observer is not None:
            try:
                observer(str(chat_id), str(thread_id), custom_id)
            except Exception:
                logger.debug(
                    "[%s] Failed to queue durable topic icon observation",
                    self.name,
                    exc_info=True,
                )

    def dm_topic_custom_icon_observation(
        self,
        chat_id: str,
        thread_id: str,
    ) -> Optional[str]:
        """Return observed custom emoji ID, empty default, or None if unknown."""
        key = self._dm_topic_icon_key(str(chat_id), str(thread_id))
        if key not in self._dm_topic_creation_icons:
            return None
        return self._dm_topic_creation_icons[key] or ""
    ...
    def dm_topic_custom_icon_state(
        ...
        """Return True for an observed custom icon, False for default, None if unknown."""
        observed = self.dm_topic_custom_icon_observation(chat_id, thread_id)
        if observed is None:
            return None
    ...
    def _register_handlers(self, app) -> None:
        ...
        app.add_handler(TelegramMessageHandler(topic_status_filter, self._handle_dm_topic_status_update))
        checklist_filter = (
            filters.CHECKLIST | filters.StatusUpdate.CHECKLIST_TASKS_ADDED | filters.StatusUpdate.CHECKLIST_TASKS_DONE)
        app.add_handler(TelegramMessageHandler(checklist_filter, self._handle_checklist_message))
        ...
        # PTB predates Bot API 10.1 RichMessage, so a rich-only message lives in ``Message.api_kwargs`` and
        # matches none of the built-in filters. After text/media so a mixed future payload still reaches
        # the richer handler first.
        app.add_handler(TelegramMessageHandler(self._rich_message_filter(), self._handle_rich_message))
        # Last in group 0: PTB dispatches only the first matching handler per group, so known families are
        # processed normally while future message-like payloads become observable instead of vanishing.
        app.add_handler(TelegramMessageHandler(filters.UpdateType.MESSAGES, self._handle_unmatched_message))
    ...
    async def _send_chunk_markdown_or_plain(self, chat_id: str, chunk: str, send_kwargs: Dict[str, Any]):
    ...
    async def _send_chunk_with_retries(
        ...
        thread_id: Optional[str], used_thread_fallback: bool, error_types: tuple, *, prior_chunks_delivered: bool = False):
        ...
        for _send_attempt in range(3):
            try:
                ...
                return await self._send_chunk_markdown_or_plain(chat_id, chunk, send_kwargs), used_thread_fallback
            except _NetErr as send_err:
                # A flood rejection is owned by the shared per-chat gate, not this loop's back-off.
                handled, retry_result = self._send_retry_after_outcome(
                    send_err, _send_attempt, prior_chunks_delivered=prior_chunks_delivered)
                if handled:
                ...
                if _BadReq and isinstance(send_err, _BadReq):
                    if self._is_thread_not_found_error(send_err) and effective_thread_id is not None:
                        # Telegram briefly rejects a newly created or existing topic before propagation
                        # settles. The request was rejected, so one same-thread retry is safe and never
                        # reroutes a created/private topic into All Messages.
                        ...
                        if private_dm_topic_send or (metadata and metadata.get("telegram_dm_topic_created_for_send")):
                            # Fail loud: a private topic must not fall back to the chat root here. The
                            # delivered-chunk counts let the caller recover the tail without duplicating.
                            ...
                            return SendResult(
                                ...
                                raw_response={
                                    "telegram_stale_topic_partial_delivery": prior_chunks_delivered,
                                    "delivered_chunks": index, "total_chunks": index + 1})
                ...
                # Never sleep a long server RetryAfter verbatim — a 97-minute penalty once pinned send()
                # and froze inbound on every platform (#91969). The shared gate re-paces the retry and an
                # over-cap wait fails closed so the delivery ledger owns it.
                handled, retry_result = self._send_retry_after_outcome(
                    send_err, _send_attempt, prior_chunks_delivered=prior_chunks_delivered)
                if handled:
                    if retry_result is not None:
    ...
    async def _send_impl(
        ...
        if getattr(self, "_send_path_degraded", False):
            # Let polling prove recovery before spending the generic send loop's small retry budget.
            return SendResult(
                success=False, error="send_path_degraded", retryable=True,
                retry_after=_POLLING_PROGRESS_TIMEOUT, error_kind="transient")
        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                try:
                    ...
                    return self._partial_text_delivery_failure(failure, message_ids, chunks) if message_ids else failure
    ...
    async def _edit_message_impl(
        ...
        guarded = await self._reconnect_guarded(
            "edit_message", chat_id, message_id, content, finalize=finalize, metadata=metadata)
        ...
        try:
            ...
            if retry_after is not None or "retry after" in err_str:
                ...
                if wait > _FLOOD_INLINE_WAIT_CAP_SECS:
                    # Telegram rejected the edit outright, so deferring it cannot duplicate anything.
    ...
    async def send_draft(self, chat_id: str, draft_id: int, content: str, metadata: Optional[Dict[str, Any]] = None) -> SendResult:
        ...
        if self._business_connection_kwargs(metadata):
            # PTB exposes no business_connection_id for send_message_draft, and calling it without the
            # discriminator can misroute or reject a Business turn. Fail closed so streaming uses the
            # persistent send/edit path, which is connection-aware.
            return SendResult(success=False, error="business_drafts_unsupported", retryable=False)
    ...
    async def _send_message_with_thread_fallback(self, **kwargs):
        ...
        try:
            ...
            # A flood rejection is not a stale-thread symptom: defer rather than retry without the topic.
            retry_after = self._telegram_retry_after(send_err)
            if retry_after is not None:
                raise _TelegramSendCooldownExceeded(retry_after) from send_err
            if (message_thread_id is not None and self._is_bad_request_error(send_err) and self._is_thread_not_found_error(send_err)):
                ...
                retry_kwargs.pop("message_thread_id", None)
                return await self._run_send_call(chat_id, self._bot.send_message, **retry_kwargs)
            raise
    ...
    async def _send_prompt(self, what: str, chat_id: str, metadata: Optional[Dict[str, Any]], build, *,
        ...
        try:
            ...
            # Control prompts are user-visible obligations: hand the caller a timed deferral, not a drop.
            return self._send_cooldown_failure(cooldown_error)
        except Exception as e:
    ...
    async def send_update_prompt(
        ...
        guarded = await self._reconnect_guarded(
            "send_update_prompt", chat_id, prompt, default, session_key=session_key, metadata=metadata)
        if guarded is not None:
            return guarded
    ...
    async def send_exec_approval(
        ...
        text ``/approve`` flow."""
        guarded = await self._reconnect_guarded(
            "send_exec_approval", chat_id, command, session_key, description=description, metadata=metadata,
    ...
    async def send_slash_confirm(
        ...
        guarded = await self._reconnect_guarded(
            "send_slash_confirm", chat_id, title, message, session_key, confirm_id, metadata=metadata)
        if guarded is not None:
            return guarded
    ...
    async def send_clarify(
        ...
        guarded = await self._reconnect_guarded(
            "send_clarify", chat_id, question, choices, clarify_id, session_key, metadata=metadata)
        if guarded is not None:
            return guarded
    ...
    async def send_model_picker(
        ...
        guarded = await self._reconnect_guarded(
            "send_model_picker", chat_id, providers, current_model, current_provider, session_key,
    ...
    async def send_choice_picker(
        ...
        guarded = await self._reconnect_guarded(
            "send_choice_picker", chat_id, title, choices, session_key, on_choice_selected, metadata=metadata)
        if guarded is not None:
            return guarded
    ...
    async def send_voice(
        ...
        guarded = await self._reconnect_guarded(
            "send_voice", chat_id, audio_path, caption=caption, reply_to=reply_to, metadata=metadata)
        if guarded is not None:
            return guarded
    ...
    async def send_multiple_images(
        ...
        human_delay: float = 0.0) -> List[SendResult]:
        ...
        try:
            ...
            results.extend(await super().send_multiple_images(chat_id, images, metadata, human_delay) or [])
            return results
        ...
        for chunk_idx, chunk in enumerate(chunks):
            ...
            try:
                ...
                # The chat is flood-gated: retrying the remaining chunks now would only deepen the penalty.
                ...
                results.extend(await super().send_multiple_images(chat_id, chunk, metadata, human_delay=human_delay) or [])
            finally:
                for fh in opened_files:
                    with contextlib.suppress(Exception):
                        fh.close()
    ...
    async def send_image_file(
        ...
        guarded = await self._reconnect_guarded(
            "send_image_file", chat_id, image_path, caption=caption, reply_to=reply_to, metadata=metadata)
        if guarded is not None:
            return guarded
    ...
    async def send_document(
        ...
        guarded = await self._reconnect_guarded(
            "send_document", chat_id, file_path, caption=caption, file_name=file_name, reply_to=reply_to,
            metadata=metadata)
        if guarded is not None:
            return guarded
        return await self._send_local_file(
    ...
    async def send_video(
        ...
        guarded = await self._reconnect_guarded(
            "send_video", chat_id, video_path, caption=caption, reply_to=reply_to, metadata=metadata)
        if guarded is not None:
            return guarded
        return await self._send_local_file(
    ...
    async def send_image(
        ...
        guarded = await self._reconnect_guarded(
            "send_image", chat_id, image_url, caption=caption, reply_to=reply_to, metadata=metadata)
        if guarded is not None:
            return guarded
    ...
    async def send_animation(
        ...
        guarded = await self._reconnect_guarded(
            "send_animation", chat_id, animation_url, caption=caption, reply_to=reply_to, metadata=metadata)
        if guarded is not None:
            return guarded
        try:
            msg = await self._send_media(
    ...
    async def send_typing(self, chat_id: str, metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        HERMES-084: the indicator shares the chat's send gate. A published flood window quiesces typing
        too — every active session used to keep firing ``sendChatAction`` through a multi-thousand-second
        penalty. ``_reserve_gap=False`` because a typing tick must not push real sends further out."""
        if not self._bot or self._typing_in_cooldown(chat_id) or self._chat_in_send_cooldown(chat_id):
            return
        _is_dm_topic: bool = False
        message_thread_id: Optional[int] = None

        async def _send_action(*, include_thread: bool) -> None:
            kwargs: Dict[str, Any] = {
    ...
    def _entity_sources(message: Message):
        """``(text, entities)`` pairs for the message text, caption, and structured (rich) payloads."""
        yield getattr(message, "text", None) or "", getattr(message, "entities", None) or []
        yield getattr(message, "caption", None) or "", getattr(message, "caption_entities", None) or []
        yield from TelegramAdapter._checklist_text_sources(message)
        yield from TelegramAdapter._rich_message_text_sources(message)
    ...
    def _checklist_from_message(message: Any) -> Any:
        """The structurally valid checklist a message carries or references, else None."""

        def _valid(value: Any) -> bool:
            return (
                value is not None and isinstance(getattr(value, "title", None), str)
        ...
        for attr in ("checklist_tasks_added", "checklist_tasks_done"):
            referenced = getattr(getattr(message, attr, None), "checklist_message", None)
    ...
    def _checklist_text_sources(cls, message: Any):
        """Yield checklist text/entity pairs so the existing mention gates can read them."""
        checklist = cls._checklist_from_message(message)
        if checklist is None:
            return
        yield str(getattr(checklist, "title", "") or ""), getattr(checklist, "title_entities", None) or []
        for task in list(getattr(checklist, "tasks", None) or []):
            yield str(getattr(task, "text", "") or ""), getattr(task, "text_entities", None) or []
    ...
    def _checklist_entity_metadata(entities: Any) -> List[Dict[str, Any]]:
        ...
        for entity in list(entities or [])[:100]:
            item: Dict[str, Any] = {}
            for key in ("type", "offset", "length", "url", "language", "custom_emoji_id"):
    ...
    def _checklist_actor_metadata(actor: Any) -> Optional[Dict[str, str]]:
        """``{"id", "name"}`` for the user/chat that completed a task, or None."""
        if actor is None or getattr(actor, "id", None) is None:
            return None
        name = getattr(actor, "full_name", None) or getattr(actor, "title", None) or getattr(actor, "username", None)
        return {"id": str(actor.id), "name": str(name) if name else ""}

    @staticmethod
    def _normalized_checklist_id(value: Any) -> Optional[int]:
        """A valid positive checklist task ID, otherwise ``None``."""
    ...
    def _checklist_task_metadata(cls, task: Any) -> Dict[str, Any]:
        ...
        if isinstance(completion_date, datetime):
            try:
                # PTB uses a zero Unix timestamp as the sentinel for an incomplete task, so a plain
                # truthiness check would mark that task done.
        ...
        metadata = {
            "id": task_id, "text": str(getattr(task, "text", "") or "")[:100],
            "text_entities": cls._checklist_entity_metadata(getattr(task, "text_entities", None)),
    ...
    def _checklist_task_line(cls, task: Any) -> str:
        ...
        return f"- [{'x' if item['completed'] else ' '}] #{task_id} {item['text']}".rstrip()

    def _build_checklist_event(self, message: Message, update_id: Optional[int] = None, *, edited: bool = False) -> MessageEvent:
        """One bounded text projection plus lossless checklist metadata."""
        event = self._build_message_event(message, MessageType.TEXT, update_id=update_id)
        ...
        metadata: Dict[str, Any] = {
            ...
            "title_entities": self._checklist_entity_metadata(getattr(checklist, "title_entities", None)),
            "tasks": [self._checklist_task_metadata(task) for task in tasks[:30]],
            "others_can_add_tasks": getattr(checklist, "others_can_add_tasks", None),
            "others_can_mark_tasks_as_done": getattr(checklist, "others_can_mark_tasks_as_done", None),
            "checklist_message_id": str(checklist_message_id) if checklist_message_id is not None else None}
        if kind == "checklist":
            ...
            raw_done_ids = list(getattr(done, "marked_as_done_task_ids", None) or [])[:30]
            raw_undone_ids = list(getattr(done, "marked_as_not_done_task_ids", None) or [])[:30]
            done_ids = [
                task_id for value in raw_done_ids
                if (task_id := self._normalized_checklist_id(value)) is not None]
            undone_ids = [
                task_id for value in raw_undone_ids
                if (task_id := self._normalized_checklist_id(value)) is not None]
            metadata["marked_as_done_task_ids"] = done_ids
            metadata["marked_as_not_done_task_ids"] = undone_ids
            invalid_counts = {
                "done": len(raw_done_ids) - len(done_ids), "not_done": len(raw_undone_ids) - len(undone_ids)}
            ...
            if undone_ids:
                lines.append("Marked not done: " + ", ".join(f"#{value}" for value in undone_ids))
        event.text = "\n".join(lines)
        # A projected checklist is never a slash command, whatever its task text says.
        event.allow_gateway_control = False
        event.metadata["telegram_checklist"] = metadata
        return event

    async def _handle_checklist_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Normalize an initial checklist and observe collaboration status changes."""
        message = self._effective_update_message(update)
        if message is None:
            return
        is_status = bool(
        ...
        if self._checklist_from_message(message) is None and not is_status:
            logger.warning(
                "[%s] Telegram checklist handler received no checklist payload update_id=%s message_id=%s",
                self.name, getattr(update, "update_id", None), getattr(message, "message_id", None))
            return
        if not self._is_user_authorized_from_message(message):
            ...
            return
        event = self._build_checklist_event(message, update_id=update.update_id, edited=is_edited)
        # A status change or an edit is collaboration context, never a fresh turn to answer.
        if is_status or is_edited:
            if self._should_process_message(message) or self._should_observe_unmentioned_group_message(message):
                self._observe_unmentioned_group_message(
                    message, MessageType.TEXT, update_id=update.update_id, event=event)
            return
    ...
    def _inbound_rich_message_payload(message: Any) -> tuple[Any, Optional[str]]:
        """The message's native/future or PTB-``api_kwargs`` RichMessage payload, with which one it was."""
        native = getattr(message, "rich_message", None)
        native_type = type(native)
        if isinstance(native, Mapping) or (
            native is not None and native_type.__module__.startswith("telegram")
    ...
    def _rich_message_text_sources(cls, message: Any):
        """Yield bounded Rich Message text so the existing group mention gates can read it."""
    ...
    def _is_rich_message_update(cls, message: Any) -> bool:
        """True only for rich-only messages no existing handler already owns."""
        if message is None:
            return False
        ...
        if isinstance(payload, dict):
            return payload.get("blocks") is not None
    ...
    def _rich_message_filter(cls):
        """Build the PTB filter lazily so optional imports stay import-safe."""
        adapter_cls = cls
    ...
    def _unmatched_message_fields(message: Any) -> List[str]:
        """Bounded field *names* for an unrecognized payload — never its values."""
        ignored = {
            "message_id", "date", "chat", "from_user", "sender_chat", "from", "business_connection_id",
            "message_thread_id", "is_topic_message", "reply_to_message", "quote", "entities",
            "caption_entities", "text", "caption", "forum_topic_created"}
        ...
        if callable(to_dict):
            ...
            if isinstance(serialized, dict):
                fields.update(
                    str(key) for key, value in serialized.items()
        ...
        try:
            ...
            instance_vars = {}
        fields.update(
            str(key) for key, value in instance_vars.items()
            if key not in ignored and key != "api_kwargs" and _present(value))
        # PTB TelegramObject subclasses are slotted with no __dict__: walk inherited slots so even a failed
        # to_dict() still yields a bounded shape without touching payload values.
    ...
    async def _handle_rich_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Normalize one inbound Bot API Rich Message into the ordinary text pipeline."""
        message = self._effective_update_message(update)
        ...
        if not projection.text:
            logger.warning(
                "[Telegram] Rich Message has no readable content update_id=%s message_id=%s chat_type=%s "
                "block_count=%s block_types=%s",
                getattr(update, "update_id", None), getattr(message, "message_id", None),
                str(getattr(getattr(message, "chat", None), "type", "unknown"))[:32],
                projection.block_count, list(projection.block_types))
            return
        should_process = self._should_process_message(message)
        should_observe = not should_process and self._should_observe_unmentioned_group_message(message)
        if not should_process and not should_observe:
            return
        event = self._build_message_event(message, MessageType.TEXT, update_id=getattr(update, "update_id", None))
        event.text = self._clean_bot_trigger_text(projection.text)
        event.metadata["telegram_rich_message"] = {
            "block_count": projection.block_count, "block_types": list(projection.block_types),
            "source": source, "truncated": projection.truncated}
        if should_observe:
            self._observe_unmentioned_group_message(
                message, MessageType.TEXT, update_id=getattr(update, "update_id", None), event=event)
            return
        await self._ensure_forum_commands(message)
    ...
    async def _handle_unmatched_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if message is None:
            return
        logger.warning(
            "Unhandled Telegram message-like update update_id=%s message_id=%s chat_type=%s payload_fields=%s",
            getattr(update, "update_id", None), getattr(message, "message_id", None),
            str(getattr(getattr(message, "chat", None), "type", "unknown"))[:32],
    ...
    def _message_matches_mention_patterns(self, message: Message) -> bool:
        if not self._mention_patterns:
            return False
        # Structured payloads (rich messages) carry prose no ``text``/``caption`` attribute exposes.
        structured = [
            text for source in (self._checklist_text_sources(message), self._rich_message_text_sources(message))
            for text, _entities in source]
        return any(
            pattern.search(candidate)
            for candidate in (getattr(message, "text", None), getattr(message, "caption", None), *structured) if candidate
    ...
    def _observe_unmentioned_group_message(
        ...
        try:
            ...
            # Observe-only structured messages never enter the normal dispatch path, so persist their
            # bounded metadata alongside the text or their source shape is lost.
            structured_metadata = {
                key: event.metadata[key] for key in ("telegram_checklist", "telegram_rich_message")
    ...
    def _resolve_topic_binding(self, message: Message, chat_type: str, thread_id_str: Optional[str]) -> tuple:
        ...
        if chat_type == "dm" and thread_id_str:
            ...
            # Dedicated observer normally records service updates; keep this
            # idempotent fallback for direct event builders.
            created_name = self._record_dm_topic_status_update(message, thread_id=thread_id_str)
    ...
    def _build_message_event(self, message: Message, msg_type: MessageType, update_id: Optional[int] = None) -> MessageEvent:
        ...
        # A Telegram Business conversation is a distinct routing identity: carry its discriminator on the
        # source (session keying) and in the event metadata (outbound stamping).
