...
_SLACK_REPLY_METADATA_FIELDS = ("reply_count", "reply_users_count", "reply_users", "latest_reply", "replies")
...
def _is_hidden_thread_parent_metadata_update(event: dict, message: dict) -> bool:
    """True when Slack changed only reply bookkeeping on a thread parent (HERMES-019).

    Slack emits hidden ``message_changed`` events when a thread receives a reply. Replaying the
    nested parent as user input is unsafe after a restart, when the process-local processed-message
    cache is empty. Real edits are preserved whenever Slack supplies a visible before/after delta;
    if the prior snapshot is malformed or absent, only an explicit edit whose edit timestamp
    identifies this event is accepted, and ambiguous hidden parent updates fail closed.
    """
    ...
    if not any(
        field in message or field in previous_for_metadata for field in _SLACK_REPLY_METADATA_FIELDS):
        return False
    if isinstance(previous, dict) and any(field in previous for field in _SLACK_VISIBLE_MESSAGE_FIELDS):
        return not any(
            _slack_visible_message_value(previous, field) != _slack_visible_message_value(message, field)
...
class SlackAdapter(SlackWisdomMixin, BasePlatformAdapter):
    ...
    async def send_multiple_images(
        ...
        Returns one SendResult per uploaded file so a caller can account for what actually landed
        ...
        for chunk_idx, chunk in enumerate(chunks):
            ...
            try:
                ...
                result = await self._client_for(chat_id, metadata).files_upload_v2(
                    ...
                    thread_ts=thread_ts)
                self._record_uploaded_file_thread(chat_id, thread_ts, metadata)
                results.extend(self._upload_batch_results(result, len(file_uploads)))
            except Exception as e:
                logger.warning(
                    "[Slack] Multi-image files_upload_v2 failed (chunk %d/%d), falling back to per-image: %s",
                    chunk_idx + 1, len(chunks), e, exc_info=True)
                results.extend(
                    await super().send_multiple_images(chat_id, chunk, metadata, human_delay=human_delay) or [])
        return results

    @staticmethod
    def _upload_batch_results(response: Any, uploaded: int) -> List[SendResult]:
        """One SendResult per uploaded file, identified by its share ``ts`` when Slack returns one.

        ``files_upload_v2`` reports share timestamps under ``files[].shares``; a workspace that does
        not share them still returns file ids, which are the only durable handle we get. Fewer
        identifiers than files means Slack collapsed the batch into one message — reuse the last."""
        ...
        return [
            SendResult(
                success=True,
                message_id=(
                    identifiers[index] if index < len(identifiers)
                    else identifiers[-1] if identifiers else None))
            for index in range(uploaded)]
    ...
    def _normalize_changed_message(self, event: dict) -> Optional[dict]:
        ...
        if _is_hidden_thread_parent_metadata_update(event, updated_message):
            logger.debug(
                "[Slack] Ignoring hidden thread-parent reply metadata update channel=%s parent_ts=%s event_ts=%s",
                event.get("channel", ""), updated_message.get("ts", ""),
