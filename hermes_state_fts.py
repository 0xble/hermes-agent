...
class SessionFtsSetupMixin:
    ...
    def _fts_structure_is_corrupt(self):
        ...
        through expensive rebuilds while the genuinely damaged object (a corrupt ephemeral lease
        btree, in the field incident) went undiagnosed.
        """
        now = time.monotonic()
        ...
        try:
            with self._lock:
                present = [row[0] for row in self._conn.execute(
                    ...
                    "('messages_fts', 'messages_fts_trigram', 'messages_fts_cjk')").fetchall()]
                result = False
                for table in present:
                    try:
                        ...
                        if is_malformed_db_error(probe_exc) or "corrupt" in str(probe_exc).lower():
                            ...
                            result = None  # the probe form itself was rejected: attribution unknown
                        break
