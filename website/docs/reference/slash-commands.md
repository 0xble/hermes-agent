...
| `/side <prompt>` | **Messaging only.** Fork the newest safe transcript checkpoint into a continuable side session. Replies to side messages stay in that side without changing the parent topic's active session. Use `/side close` in a side reply chain to close it. During an active turn, Hermes waits for the current tool batch to finish when necessary. See [Side Sessions](/user-guide/messaging/#side-sessions). |
| `/merge` (alias: `/fold`) | **Messaging only.** Reply to a side-session message to import its committed context into the originating main session. A compacted side contributes its canonical checkpoint first, unless main rewound copied fork rows, in which case merge fails closed. The side remains independently continuable; later side turns stay isolated until merged again. |
...
- `/sethome`, `/restart`, `/approve`, `/deny`, `/topic`, `/platform`, `/commands`, and `/side` are **messaging-only** commands.
