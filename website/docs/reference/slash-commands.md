...
| `/spawn <prompt>` (alias: `/side`) | **Messaging only.** Fork the newest safe transcript checkpoint into a durable one-shot child session, run the prompt asynchronously, and deliver the result without changing the parent transcript. During an active turn, Hermes waits for the current tool batch to finish when necessary. See [Contextual Spawns](/user-guide/messaging/#contextual-spawns). |
...
- `/sethome`, `/restart`, `/approve`, `/deny`, `/topic`, `/platform`, `/commands`, and `/spawn` (alias `/side`) are **messaging-only** commands.
