...
| `/spawn <prompt>` | **Messaging only.** Snapshot the current committed transcript into a durable one-shot child session, run the prompt asynchronously, and deliver the result without changing the parent transcript. See [Contextual Spawns](/user-guide/messaging/#contextual-spawns). |
...
- `/sethome`, `/restart`, `/approve`, `/deny`, `/topic`, `/platform`, `/commands`, and `/spawn` are **messaging-only** commands.
