...
`~/.hermes/browser-profile/<browser>/`, then launches the matching installed
stable browser against that snapshot and attaches automation over loopback CDP.
On macOS, keeping the browser bundle consistent also keeps copied cookies on the
same Safe Storage keychain identity; Hermes does not launch these snapshots with
...
so this does not reintroduce the mock-keychain authentication loss that the
matching-browser launch avoids. To watch the browser, enable
