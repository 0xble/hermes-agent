...
The same tool supports status, set/draft/edit, pause/resume/clear, waits, subgoals, and quality gates. `edit` refines the existing contract without resetting its status, budget, progress, wait barriers, or gates. Omitted contract fields are preserved. Routine lifecycle changes do not require a new instruction or special wording. The agent pauses for genuine blockers, resumes its own pauses when those blockers clear, and clears obsolete, duplicate, completed, or unnecessary agent-created tracking. It must preserve unfinished work you requested, including when replacing tracking or removing subgoals. It must not weaken completion criteria or remove gates just to bypass failure. Writes are reported as successful only after persistent readback.

Your explicit pause or stop remains binding until you direct continuation. The agent records that distinction with `user_requested=true` on user-directed pause/clear operations. It uses the same flag only when a fresh user message directs resume/restart. Natural-language intent is interpreted by the agent, not by phrase matching. Messaging supplies the authenticated message body separately from quoted and rendered context. CLI and TUI carry runtime-only continuation provenance and bind it on the model worker thread. Internal continuations cannot claim fresh user direction. The persisted `user_stopped` hold survives clearing, drafting, and restart. A user stop freezes autonomous execution, not housekeeping on the tracker record. The agent may retire obsolete or duplicate tracking while keeping your requested work covered. Instructions govern whether that work remains covered; clearing is never permission to abandon it.

`draft` saves paused state directly. Autonomous resume and replacement preserve the spent budget. After activation the agent must take a concrete step, not merely announce a goal. Blocked work is paused, not marked complete. The goal evaluator and quality gates still verify completion; clearing tracking is not a substitute for verification. Gate commands run synchronously through the configured terminal backend and its existing execution policy, so adding a verifier does not grant extra shell permissions.
