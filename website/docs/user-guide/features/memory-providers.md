...
The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud or local-external, `hindsight-all` plus the exact client constraint for local-embedded). Requires `hindsight-client==0.9.1` (re-pinned on session start if a different version is installed).
