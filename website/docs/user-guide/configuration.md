...
As of recent releases, editing `model.context_length` or any `compression.*` key in `config.yaml` on a running gateway takes effect on the next message — no gateway restart, no `/reset`, no session rotation required. The cached-agent signature includes these keys, so the gateway transparently rebuilds the agent when it sees a change. Named delegation roles and their schema-driving defaults also refresh on the next turn; see [delegation configuration](features/delegation.md). This does not mutate in-flight agents or existing children's frozen routes. API keys and other tool/skill configuration still require their usual reload paths.
:::
...
`interim_assistant_messages` is gateway-only. When enabled, Hermes sends completed mid-turn assistant updates as separate chat messages. This is independent from `tool_progress` and does not require gateway streaming. When disabled, ordinary mid-turn updates stay hidden; decision-critical prose emitted immediately before a `clarify` prompt is still delivered so the interactive choices remain understandable, even when `show_commentary` is disabled.

`show_commentary` (default `true`) controls Codex Responses models' commentary channel — the polished progress narration these models produce alongside their private reasoning. When enabled, each completed commentary message is delivered as a visible mid-turn update (on the gateway this also requires `interim_assistant_messages`). Set it to `false` if the extra narration annoys you: ordinary commentary then falls back to the reasoning channel and is only shown when `show_reasoning` is enabled; decision context immediately preceding `clarify` remains visible.
