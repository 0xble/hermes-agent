...
- `/spawn <prompt>` and its `/side` alias snapshot that topic's committed session context into a durable child and deliver the one-shot result back to the same topic without rebinding it
