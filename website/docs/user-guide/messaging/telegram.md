  ...
  platforms:
    telegram:
      extra:
        ...
        topic_icon_instructions: "Prefer specific visual metaphors"
...
- `/spawn <prompt>` and its `/side` alias fork that topic's newest safe session checkpoint into a durable child and deliver the one-shot result back to the same topic without rebinding it; while the parent is busy, Telegram edits one queued status bubble when the next complete tool checkpoint becomes available
...
**Rich Messages (Bot API 10.1).** In `auto` mode, final replies that contain constructs the legacy MarkdownV2 path degrades — tables, task lists, collapsible `<details>`, and block math — are sent with Telegram's native [`sendRichMessage`](https://core.telegram.org/bots/api#sendrichmessage). `always` mode attempts the rich endpoint for every eligible final response; `never` mode keeps legacy MarkdownV2. In DMs, `rich_drafts: false` keeps the animated preview on the editable legacy draft path, while `rich_drafts: true` makes the live preview use `sendRichMessageDraft` too. Rich draft rendering remains experimental because Telegram Desktop/macOS can visually overlay draft frames until the chat redraws. Edit-based streams can finalize an existing preview in place through `editMessageText`'s `rich_message` parameter.

The rich path is skipped automatically when content exceeds the 32,768-character rich text limit, when Telegram rejects the endpoint, or for known client-risk shapes. Those cases transparently fall back to the MarkdownV2 path; transient/network errors are not silently re-sent.
...
gateway:
  platforms:
    telegram:
      extra:
        rich_messages: auto    # auto (default), always, or never
        rich_drafts: false     # set true to enable rich draft previews
```

`rich_messages: true` remains accepted as a compatibility alias for `auto`; `false` maps to `never`.
