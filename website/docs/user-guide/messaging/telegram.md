...
When Hermes titles a session from its opening message, the Telegram topic itself is renamed to match — e.g. "New Topic" becomes "Database". Internally, an instant derived title is stored first; the title model upgrades it in the background, and Telegram waits for that upgrade to avoid two remote renames. Titles prefer 1–3 words, favor an explicitly named project or proper name, and avoid filler such as "Fixing", "Update", or "Analysis".
