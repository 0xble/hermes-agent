...
# HERMES-018 semantic topic icons (schema v4). Same profile-leading PK rule as _TOPIC_TABLES.
_TOPIC_ICON_TABLES = (
    (
...
class SessionTelegramTopicsMixin:
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        multiplexed gateways sharing one state.db isolate topic state per profile; v4 adds the
        HERMES-018 semantic topic-icon state and bounded per-chat selection history.
        ...
        def _do(conn):
            ...
            conn.executescript("""
                ...
                """)
            # v4 icon tables share the same pre-profile rebuild path as the binding tables.
            for table, columns, ddl in _TOPIC_ICON_TABLES:
                conn.execute(f"CREATE TABLE IF NOT EXISTS {table} ({ddl})")
                have = {row[1] for row in conn.execute(f"PRAGMA table_info('{table}')")}
                if "profile_name" in have:
                    continue
    ...
    # ── HERMES-018 semantic topic icons ──────────────────────────────────

    def get_telegram_topic_icon_state(self, *, chat_id: str, thread_id: str,
                                      profile_name: str = "default") -> Optional[Dict[str, Any]]:
        """Durable observed icon ownership. Never creates tables — an unmigrated DB reads as None."""
        with self._read_ctx() as conn:
            if conn is None:
                return None
            try:
    ...
    def record_telegram_topic_icon_observation(self, *, chat_id: str, thread_id: str,
                                               custom_emoji_id: Optional[str],
                                               profile_name: str = "default") -> str:
        """Persist an observed icon as ``auto``, ``manual`` or ``default`` and return that ownership.

        Only an icon matching what Hermes itself last set stays auto-owned; any other custom icon is the
        user's and therefore protected from automatic edits. An empty ID is Telegram's default bubble.
        """
        self.apply_telegram_topic_migration()
        chat_id, thread_id = str(chat_id), str(thread_id)
        ...
        def _do(conn):
            ...
            ownership = "default"
            if icon_id:
                same_auto = (current is not None and str(current["ownership"]) == "auto"
    ...
    def list_recent_telegram_topic_icons(self, *, chat_id: str, limit: int = 24,
                                         profile_name: str = "default") -> List[Dict[str, Any]]:
        """Durable most-recent-first icon history for one chat, used to keep selections diverse."""
        ...
        with self._read_ctx() as conn:
            ...
            try:
                rows = conn.execute(
                    "SELECT custom_emoji_id, emoji, selected_at FROM telegram_topic_icon_history "
                    "WHERE profile_name = ? AND chat_id = ? ORDER BY selected_at DESC LIMIT ?",
                    (_normalize_telegram_topic_profile_name(profile_name), str(chat_id), bounded_limit),
                ).fetchall()
            except (AttributeError, sqlite3.OperationalError):
                return []
        return [dict(row) for row in rows]

    def record_telegram_topic_icon_selection(self, *, chat_id: str, custom_emoji_id: str, emoji: str,
                                             limit: int = 24, profile_name: str = "default") -> None:
        """Record a selected icon, then prune the chat's history to a bounded LRU."""
        self.apply_telegram_topic_migration()
        chat_id = str(chat_id)
        icon_id, emoji = str(custom_emoji_id).strip(), str(emoji).strip()
        ...
        def _do(conn):
            conn.execute("""
                ...
                """, (profile_name, chat_id, icon_id, emoji, time.time()))
            conn.execute(
    ...
    def mark_telegram_topic_icon_auto(self, *, chat_id: str, thread_id: str, custom_emoji_id: str,
                                      profile_name: str = "default") -> None:
        """Claim auto ownership — call only after Telegram accepted the automatic edit."""
        self.apply_telegram_topic_migration()
