...
def _fsync_directory(path: Path) -> None:
    ...
    Renaming a file into place is only durable once the DIRECTORY entry is flushed; without this a
    crash can leave a forensic backup that the metadata claims exists. Windows has no directory
    handle to fsync, so the barrier is skipped rather than faked.
    """
    if os.name == "nt":
        return
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    See #69603.
    """
    # HERMES-002: the live-connection check and the raw copy must be ONE critical section. A bare
    # has_live_connection() followed by file I/O is a check/use race — a connection opened in that
    # window has its POSIX advisory locks cancelled by the copy's close(), the exact corruption this
    # guards against. offline_file_access holds the lifecycle lock across the whole bundle.
    try:  # scaffold/embed installs without hermes_cli track no connections
        from hermes_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    except ImportError:
        class LiveConnectionError(Exception):  # never raised on this path; keeps the handler well-typed
            pass
    ...
    try:
        with offline_file_access(db_path, what="raw-copy for forensic backup"):
            ...
            with contextlib.suppress(OSError):
                # Hash the source only when there is a candidate: hashing a multi-GB file before copying is waste.
        ...
        logger.error("Refusing to raw-copy %s for backup: %s", db_path, exc)
        return None, str(exc)
    except Exception as exc:  # pragma: no cover - best effort
...
# NOTE (fork): upstream's state_db_has_structural_damage below runs an UNCAPPED
# PRAGMA integrity_check. That is correct for its only caller, `hermes doctor`, where the
# user explicitly asked for a diagnosis and a full b-tree walk is the point. Do not reuse it
# on the startup path: _db_opens_cleanly gates its own walk on _integrity_check_is_affordable
# precisely because that walk cost minutes of startup on a 25GB store.
