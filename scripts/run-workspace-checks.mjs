...
//
// This also runs on a laptop: `node scripts/run-workspace-checks.mjs`.
...
async function main() {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === '--list') {
      ...
      const value = argv[++i]
      if (!value || value.startsWith('--')) throw new Error('--skip requires a substring argument')
      skips.push(value)
    } else if (arg === '--concurrency') {
      const value = argv[++i]
      if (requestedConcurrency !== undefined || !/^[1-9]\d*$/.test(value ?? '') ||
          !Number.isSafeInteger(Number(value))) {
        throw new Error('--concurrency requires one positive integer argument')
      }
      requestedConcurrency = Number(value)
    } else {
