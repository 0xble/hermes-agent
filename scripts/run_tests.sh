...
#   * HERMES_TEST_SCRATCH_ROOT selects an explicit disk-backed fixture root
#     when the sandbox makes the default /var/tmp read-only.
...
TEST_ENV=()
for _test_var in HERMES_PYTHON HERMES_TEST_IMAGE HERMES_TEST_WORKERS HERMES_TEST_PATHS \
  HERMES_TEST_FILE_TIMEOUT HERMES_TEST_FILE_RETRIES HERMES_TEST_SLICE \
  HERMES_TEST_SCRATCH_ROOT HERMES_GATEWAY_LOCK_DIR HERMES_E2E_REQUIRE_TUI CI GITHUB_ACTIONS; do
  if [ -n "${!_test_var:-}" ]; then
